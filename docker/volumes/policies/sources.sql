policy CREATE POLICY "Enable read access for authenticated users" ON "public"."sources" FOR
        ...
        (
          EXISTS (
            ...
            WHERE r.state_code > 0
              AND ((r.json->'data'->>'id'))::uuid = sources.id
              AND (r.json->'data'->>'version') = sources.version::text
