policy CREATE POLICY "Enable read access for authenticated users" ON "public"."processes" FOR
SELECT TO authenticated USING (
    ...
    (
      EXISTS (
        SELECT 1
        FROM roles
        WHERE roles.team_id = processes.team_id
        ...
        (
          EXISTS (
            ...
            WHERE r.state_code > 0
              AND ((r.json->'data'->>'id'))::uuid = processes.id
              AND (r.json->'data'->>'version') = processes.version::text
        ...
        (
          EXISTS (
            ...
            WHERE r.id IN (
                ...
                FROM jsonb_array_elements(processes.reviews) review_item(value)
              )
              AND r.reviewer_id @> jsonb_build_array(
                (
                  SELECT auth.uid()
