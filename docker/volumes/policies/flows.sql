policy CREATE POLICY "Enable read access for authenticated users" ON "public"."flows" FOR
SELECT TO authenticated USING (
    ...
    (
      EXISTS (
        SELECT 1
        FROM roles
        WHERE roles.team_id = flows.team_id
        ...
        (
          EXISTS (
            ...
            WHERE r.state_code > 0
              AND ((r.json->'data'->>'id'))::uuid = flows.id
              AND (r.json->'data'->>'version') = flows.version::text
        ...
        (
          EXISTS (
            ...
            WHERE r.id IN (
                ...
                FROM jsonb_array_elements(flows.reviews) review_item(value)
              )
              AND r.reviewer_id @> jsonb_build_array(
                (
                  SELECT auth.uid()
