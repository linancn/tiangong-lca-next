policy CREATE POLICY "Enable read access for authenticated users" ON "public"."unitgroups" FOR
SELECT TO authenticated USING (
    ...
    (
      EXISTS (
        SELECT 1
        FROM roles
        WHERE roles.team_id = unitgroups.team_id
        ...
        (
          EXISTS (
            ...
            WHERE r.state_code > 0
              AND ((r.json->'data'->>'id'))::uuid = unitgroups.id
              AND (r.json->'data'->>'version') = unitgroups.version::text
        ...
        (
          EXISTS (
            ...
            WHERE r.id IN (
                ...
                FROM jsonb_array_elements(unitgroups.reviews) review_item(value)
              )
              AND r.reviewer_id @> jsonb_build_array(
                (
                  SELECT auth.uid()
