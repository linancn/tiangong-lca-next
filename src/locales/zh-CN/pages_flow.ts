export default {
  'pages.flow.CASNumber': 'CAS号',
  'pages.flow.flowType': '类型',
  'pages.flow.locationOfSupply': '供应地点',
  'pages.flow.generalComment': '数据集一般性说明',
  'pages.flow.model.sourceOutputFlowName': '源输出流名称',
  'pages.flow.model.targetInputFlowName': '目标输入流名称',
  'pages.flows.editsuccess': '流更新成功。',
  'pages.flows.savesuccess': '保存成功。',

  'pages.flow.drawer.title.view': '查看流',
  'pages.flow.drawer.title.select': '选择流',
  'pages.flow.drawer.title.edit': '编辑流',
  'pages.flow.drawer.title.create': '创建流',
  'pages.flow.drawer.title.copy': '复制流',
  'pages.flow.drawer.title.createVersion': '创建版本',
  'pages.flow.model.drawer.title.refFlow': '基准流',
  'pages.flow.model.drawer.title.list': '模型列表',
  'pages.flow.model.drawer.title.edit': '编辑模型',
  'pages.flow.model.drawer.title.copy': '复制模型',
  'pages.flow.model.drawer.title.createVersion': '创建版本',
  'pages.flow.model.drawer.title.view': '查看模型',
  'pages.flow.model.drawer.title.info': '模型基本信息',
  'pages.flow.model.drawer.title.edge.port.select': '选择流连接桩',
  'pages.flow.model.drawer.title.edge.port.view': '查看流连接桩',
  'pages.flow.model.drawer.title.edge.exchange.list': '交换关系列表',
  'pages.flow.model.drawer.title.edge.exchange.edit': '编辑交换关系',
  'pages.flow.model.drawer.title.edge.exchange.view': '查看交换关系',
  'pages.flow.model.drawer.title.edge.exchange.create': '创建交换关系',

  'pages.flow.view.administrativeInformation.dataEntryBy': '数据输入方式',
  'pages.flow.view.administrativeInformation.dataSetVersion': '数据集版本',
  'pages.flow.view.administrativeInformation.referenceToPrecedingDataSetVersion': '数据集前一版本',
  'pages.flow.view.administrativeInformation.permanentDataSetURI': '永久数据集统一资源标识符',
  'pages.flow.view.administrativeInformation.permanentDataSetURI.default': '自动生成',
  'pages.flow.view.administrativeInformation.publicationAndOwnership': '出版和所有权',
  'pages.flow.view.administrativeInformation.referenceToDataSetFormat': '数据集格式',
  'pages.flow.view.administrativeInformation.timeStamp': '时间戳（最后保存）',
  'pages.flow.view.administrativeInformation.referenceToPersonOrEntityEnteringTheData': '数据录入人',
  'pages.flow.view.administrativeInformation.referenceToOwnershipOfDataSet': '数据集拥有者',
  'pages.flow.view.administrativeInformation': '管理信息',
  'pages.flow.view.flowProperties': '流属性',
  'pages.flow.view.flowInformation.name': '名称',
  'pages.flow.view.flowInformation.baseName': '基础名称',
  'pages.flow.view.flowInformation.treatmentStandardsRoutes': '处理、标准、路线',
  'pages.flow.view.flowInformation.mixAndLocationTypes': '混合和位置类型',
  'pages.flow.view.flowInformation.flowProperties': '定量流属性',
  'pages.flow.view.flowInformation.CASNumber': 'CAS号',
  'pages.flow.view.flowInformation.sumFormula': '求和公式',
  'pages.flow.view.flowInformation.classification': '分类',
  'pages.flow.view.flowInformation.ECNumber': 'EC登记号',
  'pages.flow.view.flowInformation.generalComment': '数据集一般性说明',
  'pages.flow.view.flowInformation.id': '身份信息',
  'pages.flow.view.flowInformation.synonyms': '同义词',
  'pages.flow.view.flowInformation.classificationInformation': '类别和分类信息',
  'pages.flow.view.flowInformation': '流信息',
  'pages.flow.view.flowInformation.locationOfSupply': '供应地点',
  'pages.flow.view.flowInformation.geography': '地理',
  'pages.flow.view.flowInformation.technology': '技术代表性',
  'pages.flow.view.flowInformation.technologicalApplicability': '产品或废物的技术目的',
  'pages.flow.view.flowInformation.referenceToTechnicalSpecification': '技术规范',
  'pages.flow.view.flowProperties.meanValue': '流属性的平均值',
  'pages.flow.view.flowProperties.minimumValue': '流属性的最小值',
  'pages.flow.view.flowProperties.maximumValue': '流属性的最大值',
  'pages.flow.view.flowProperties.uncertaintyDistributionType': '不确定性分布类型',
  'pages.flow.view.flowProperties.uncertaintyDistributionType.undefined': '未定义',
  'pages.flow.view.flowProperties.uncertaintyDistributionType.logNormal': '对数正态',
  'pages.flow.view.flowProperties.uncertaintyDistributionType.normal': '正态',
  'pages.flow.view.flowProperties.uncertaintyDistributionType.triangular': '三角形',
  'pages.flow.view.flowProperties.uncertaintyDistributionType.uniform': '均匀',
  'pages.flow.view.flowProperties.relativeStandardDeviation95In': '95% 相对标准偏差（%）',
  'pages.flow.view.flowProperties.dataDerivationType': '数据推导类型/状态',
  'pages.flow.view.flowProperties.dataDerivationType.measured': '测量值',
  'pages.flow.view.flowProperties.dataDerivationType.calculated': '计算值',
  'pages.flow.view.flowProperties.dataDerivationType.estimated': '估计值',
  'pages.flow.view.flowProperties.dataDerivationType.unknown': '未知推导',
  'pages.flow.view.flowProperties.referenceToFlowPropertyDataSet': '流属性',
  'pages.flow.view.flowProperties.referenceToDataSetFormat': '数据集格式',
  'pages.flow.view.flowProperties.refObjectId': '引用流属性数据集标识符',
  'pages.flow.view.flowProperties.shortDescription': '简述',
  'pages.flow.view.flowProperties.type': '类型',
  'pages.flow.view.flowProperties.uri': '统一资源标识符',
  'pages.flow.view.flowProperties.version': '版本',
  'pages.flow.view.flowProperties.generalComment': '备注',
  'pages.flow.view.flowProperty': '流属性',
  'pages.flow.view.modellingAndValidation.approvalOfOverallCompliance': '整体合规性批准',
  'pages.flow.view.modellingAndValidation.complianceDeclarations': '合规声明',
  'pages.flow.view.modellingAndValidation.typeOfDataSet': '流类型',
  'pages.flow.view.modellingAndValidation.referenceToComplianceSystem': '合规系统名称',
  'pages.flow.view.modellingAndValidation.typeOfDataSet.elementaryFlow': '基础流',
  'pages.flow.view.modellingAndValidation.typeOfDataSet.otherFlow': '其他流',
  'pages.flow.view.modellingAndValidation.typeOfDataSet.productFlow': '产品流',
  'pages.flow.view.modellingAndValidation.typeOfDataSet.wasteFlow': '废物流',
  'pages.flow.view.modellingAndValidation': '建模和验证',
  'pages.process.drawer.title.addProcess': '添加过程',

  'pages.flow.view.modellingAndValidation.approvalOfOverallCompliance.fullyCompliant': '完全符合',
  'pages.flow.view.modellingAndValidation.approvalOfOverallCompliance.notCompliant': '不符合',
  'pages.flow.view.modellingAndValidation.approvalOfOverallCompliance.notDefined': '未定义',

  // 验证器消息
  'pages.flow.validator.version.required': '请输入版本',
  'pages.flow.validator.uuid.required': '请输入UUID',
  'pages.flow.validator.name.required': '请输入基础名称',
  'pages.flow.validator.treatmentStandardsRoutes.required': '请输入处理方式、标准或路线。',
  'pages.flow.validator.mixAndLocationTypes.required': '请输入混合和位置类型',
  'pages.flow.validator.category.required': '请输入基础流分类',
  'pages.flow.validator.class.required': '请输入分类',
  'pages.flow.validator.referenceToReferenceFlowProperty.required': '请输入参考流属性',
  'pages.flow.validator.typeOfDataSet.required': '请输入流类型',
  'pages.flow.validator.referenceToComplianceSystem.required': '请输入合规系统名称',
  'pages.flow.validator.approvalOfOverallCompliance.required': '请输入整体合规性批准',
  'pages.flow.validator.timeStamp.required': '请输入时间戳',
  'pages.flow.validator.referenceToDataSetFormat.required': '请输入数据集格式',
  'pages.flow.validator.dataSetVersion.required': '请输入数据集版本',
  'pages.flow.validator.referenceToOwnershipOfDataSet.required': '请输入数据集拥有者',
  'pages.flow.validator.dataSetInternalID.required': '请输入数据集内部ID',
  'pages.flow.validator.referenceToFlowPropertyDataSet.required': '请输入参考流属性',
  'pages.flow.validator.meanValue.required': '请输入流属性的平均值',
  'pages.flow.validator.flowProperties.required': '请选择流属性',
  'pages.flow.validator.flowProperties.quantitativeReference.required': '必须且只能将一个流属性指定为定量参考。',
};
