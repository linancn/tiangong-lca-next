export default {
  'teams.members.email': '邮箱',
  'teams.members.role': '角色',
  'teams.members.actions': '操作',
  'teams.members.add': '添加成员',
  'teams.members.delete': '移除',
  'teams.members.setAdmin': '设为管理员',
  'teams.members.deleteConfirm.title': '移除',
  'teams.members.deleteConfirm.content': '确定要移除该成员吗？',
  'teams.members.role.admin': '管理员',
  'teams.members.role.member': '成员',
  'pages.team.members.getError': '获取成员失败!',
  'teams.members.role.owner': '所有者',
  'teams.members.role.rejected': '已拒绝',
  'teams.members.reInvite': '重新邀请',
  'teams.members.setMember': '设为成员',
  'teams.members.actionError': '操作失败!',
  'teams.members.actionSuccess': '操作成功!',
  'teams.members.add.title': '添加团队成员',
  'teams.members.add.success': '添加成员成功！',
  'teams.members.add.error': '添加成员失败！',
  'teams.members.email.required': '请输入邮箱！',
  'teams.members.email.invalid': '请输入有效的邮箱地址！',
  'teams.members.add.exists': '该用户已在团队中！',
  'teams.members.add.notRegistered': '该用户未注册！',
  'teams.members.add.alreadyInTeam': '该用户已加入其他团队，暂时不能再次邀请。',
  'teams.members.add.alreadyInvitedToTeam': '该用户已有其他团队的待处理邀请，暂时不能再次邀请。',
  'teams.members.add.forbidden': '你没有权限邀请成员加入该团队。',
  'teams.members.add.reinviteRequired': '该用户已拒绝过邀请，请在成员列表中重新邀请。',
  'teams.notification.team.invite.title': '团队邀请',
  'teams.notification.team.invite.content': '您已受邀加入 {teamName}。',
  'teams.notification.team.invite.accept': '同意',
  'teams.notification.team.invite.reject': '拒绝',
  'teams.members.role.empty': '暂无信息',
  'teams.members.role.invited': '邀请中',
  'teams.logo.typeError': '只能上传图片文件!',
  'teams.logo.uploadSuccess': '上传成功!',
  'teams.logo.uploadError': '团队 Logo 上传失败。',
  'teams.logo.nameError': '文件名不能包含中文字符!',
  'teams.members.memberName': '成员名称',
  'teams.notifications.teamName': '团队名称',
  'teams.notifications.status': '状态',
  'teams.notifications.status.pending': '待接受',
  'teams.notifications.status.accepted': '已接受',
  'teams.modal.noTeam.title': '您当前不属于任何团队',
  'teams.modal.noTeam.content': '您可以创建团队或加入已有团队。',
  'teams.modal.noTeam.create': '创建团队',
  'teams.modal.noTeam.join': '加入团队',
  'pages.team.info.public': '公开团队信息',
  'pages.team.info.public.tooltip': '开启后，允许他人通过邮箱联系团队负责人',
  'pages.team.info.showInHome': '申请首页展示',
  'pages.team.info.showInHome.tooltip': '开启后，允许在首页展示Logo及团队简介',
  'pages.team.info.section.basic': '基本信息',
  'pages.team.info.section.visibility': '团队可见性与展示',
  'pages.team.info.section.logo': '团队标志',
  'pages.team.info.logo.helper': '建议使用透明背景的 PNG 或 SVG 格式，以获得最佳展示效果。',
  'pages.team.info.lightLogo.required': '请上传亮色Logo!',
  'pages.team.info.darkLogo.required': '请上传暗色Logo!',
};
