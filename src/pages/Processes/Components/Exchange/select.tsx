...
const ExchangeSelect: FC<Props> = ({ buttonType, lang, sourceProcessId, targetProcessId, sourceRowKeys, targetRowKeys, optionType, onData }) => {
    ...
    useEffect(() => {
        ...
        getProcessDetail(sourceProcessId).then(async (result) => {
            ...
            await setExchangeDataSourceTable(genProcessExchangeTableData([
                ...(genProcessFromData(result.data?.json?.processDataSet ?? {})?.exchanges?.exchange ?? []),
            ]?.map((item) => {
        ...
        getProcessDetail(targetProcessId).then(async (result) => {
            ...
            await setExchangeDataTargetTable(genProcessExchangeTableData([
                ...(genProcessFromData(result.data?.json?.processDataSet ?? {})?.exchanges?.exchange ?? []),
            ]?.map((item) => {
    ...
    return (
        <>
            ...
            <Drawer
                title={
                    optionType === 'create' ? (
                        <FormattedMessage id="pages.source.drawer.title.create.edge.exchange" defaultMessage="Create Edge Exchange" />
                    ) : (
                        <FormattedMessage id="pages.source.drawer.title.edit.edge.exchange" defaultMessage="Edit  Edge Exchange" />
                    )}
