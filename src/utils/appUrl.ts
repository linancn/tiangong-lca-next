import { appBasePath, normalizeAppBasePath } from './appBasePath';
...
const buildHashHistoryPath = (path: string, basePath: string) =>
  `${normalizeAppBasePath(basePath)}#${normalizeAppPath(path)}`;
...
) => `${normalizeOrigin(origin)}${buildHashHistoryPath(path, basePath)}`;
...
export const buildAuthCallbackUrl = (
  origin: string = getAppOrigin(),
  basePath: string = appBasePath,
) => `${normalizeOrigin(origin)}${normalizeAppBasePath(basePath)}`;
