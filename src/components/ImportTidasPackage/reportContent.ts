...
type ImportReportLocaleContent = Readonly<{
...
export const IMPORT_REPORT_CONTENT_BY_APP_LOCALE = {
  'zh-CN': {
    humanSummaryTemplate:
      '导入结果：{code}。总记录 {total_entries} 条，开放数据跳过 {filtered_open_data_count} 条，用户数据冲突 {user_conflict_count} 条，成功导入 {imported_count} 条，校验问题 {validation_issue_count} 条。',
...
`,
  ...
  'en-US': {
    humanSummaryTemplate:
      'Import result: {code}. Total records: {total_entries}, skipped open-data records: {filtered_open_data_count}, user conflicts: {user_conflict_count}, imported: {imported_count}, validation issues: {validation_issue_count}.',
...
`,
  ...
  'de-DE': {
    humanSummaryTemplate:
      'Importergebnis: {code}. Datensätze insgesamt: {total_entries}, übersprungene Open-Data-Datensätze: {filtered_open_data_count}, Konflikte mit benutzereigenen Daten: {user_conflict_count}, importiert: {imported_count}, Validierungsprobleme: {validation_issue_count}.',
...
`,
  ...
  'fr-FR': {
    humanSummaryTemplate:
      "Résultat de l'importation : {code}. Nombre total d'enregistrements : {total_entries}, enregistrements de données ouvertes ignorés : {filtered_open_data_count}, conflits avec les données utilisateur : {user_conflict_count}, importés : {imported_count}, problèmes de validation : {validation_issue_count}.",
