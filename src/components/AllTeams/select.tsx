...
const SelectTeams: FC<SelectTeamsProps> = ({ actionRef: parentActionRef, buttonType = 'default' }) => {
    const intl = useIntl();
    ...
    const teamColumns: ProColumns<TeamTable>[] = [
        ...
        {
            title: <FormattedMessage id="component.allTeams.table.description" defaultMessage="Description" />,
    ...
    return (
        <>
            {buttonType === 'default' ? (
                ...
                <Tooltip
                    title={<FormattedMessage id="component.allTeams.table.select.tooltip" defaultMessage="Select Team" />}
                >
                    <Button type="text" shape="circle" size="small" icon={<SelectOutlined />} onClick={showDrawer} />
                </Tooltip>
            )}

            <Drawer
                title={<FormattedMessage id="component.allTeams.select.title" defaultMessage="Select Team" />}
                width={800}
                onClose={onClose}
                open={visible}
                footer={
            ...
            >
                <ProTable<TeamTable>
                    rowKey="id"
                    headerTitle={
                        <FormattedMessage id="component.allTeams.select.tableTitle" defaultMessage="Unshown Teams" />
                    }
                    actionRef={actionRef}
                    search={false}
