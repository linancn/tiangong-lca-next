...
const validateDeliveryEvidence = (resource, locale, overlay) => {
  ...
  if (!asArray(overlay.evidence?.tests).includes('pnpm reference-data:check')) {
    throw new Error(`${description} evidence must include pnpm reference-data:check.`);
  }
};
...
const generatedHeader = `// Generated by scripts/reference-data/reference-resource-pipeline.mjs.\n// Do not edit; update reference-resource-manifest.json or source/overlay JSON and run pnpm reference-data:write.\n\n`;
...
export async function buildReferenceResources({ repoRoot, mode, supportedLanguages }) {
  ...
  if (drift.length > 0) {
    throw new Error(
      `Reference-resource generated files are stale: ${drift.join(', ')}. Run pnpm reference-data:write.`,
    );
  }
