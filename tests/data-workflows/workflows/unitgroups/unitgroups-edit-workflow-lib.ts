...
import {
  DEFAULT_UNITGROUP_CHECK_DATA_EXPECTED_PATH,
  DEFAULT_UNITGROUP_CHECK_DATA_FIXTURE_PATH,
  buildValidationExpectationResults,
  getExpectedRuleVerification,
  getFixtureOrderedJson,
  loadUnitGroupCheckDataFixture,
} from './unitgroups-check-data-workflow-lib';
import {
...
export const DEFAULT_UNITGROUP_EDIT_FAILURE_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/unitgroups/003_edit_data_validate_false.json';
export const DEFAULT_UNITGROUP_EDIT_FAILURE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/unitgroups/003_edit_data_validate_false.md';
...
export type UnitGroupEditCliOptions = {
...
export type UnitGroupEditResult = {
...
export type UnitGroupEditRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type UnitGroupEditDependencies = {
...
Usage:
  pnpm test:workflows --unitgroups:edit --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --unitgroups:edit --role user --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one unit group from tests/data-workflows/fixtures/data/unitgroups/001_create.json
  2. Edit the same unit group into a validation-passing record from tests/data-workflows/fixtures/data/unitgroups/002_check_data_success.json
...
Flags:
  ...
  --create-data-file <path>        Defaults to tests/data-workflows/fixtures/data/unitgroups/001_create.json
  --success-data-file <path>       Defaults to tests/data-workflows/fixtures/data/unitgroups/002_check_data_success.json
  --edit-data-file <path>          Defaults to tests/data-workflows/fixtures/data/unitgroups/003_edit_data_validate_false.json
  ...
  --keep-data                      Keep the workflow unit group after validation (default)
  --keep-created                   Alias of --keep-data
  --no-keep-data                   Delete the workflow unit group after validation
...
export function parseEditCliArgs(argv: string[], cwd = process.cwd()): UnitGroupEditCliOptions {
  let runtimeRecordFileExplicit = false;
  const options: UnitGroupEditCliOptions = {
    createDataFile: path.resolve(cwd, DEFAULT_UNITGROUP_FIXTURE_PATH),
    createExpectedFile: path.resolve(cwd, DEFAULT_UNITGROUP_EXPECTED_PATH),
    editDataFile: path.resolve(cwd, DEFAULT_UNITGROUP_EDIT_FAILURE_FIXTURE_PATH),
    editExpectedFile: path.resolve(cwd, DEFAULT_UNITGROUP_EDIT_FAILURE_EXPECTED_PATH),
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_UNITGROUP_ROLE,
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_UNITGROUP_EDIT_FAILURE_FIXTURE_PATH),
      cwd,
    ),
    successDataFile: path.resolve(cwd, DEFAULT_UNITGROUP_CHECK_DATA_FIXTURE_PATH),
    successExpectedFile: path.resolve(cwd, DEFAULT_UNITGROUP_CHECK_DATA_EXPECTED_PATH),
...
export function buildUnitGroupEditRuntimeRecord(
  options: Pick<
    UnitGroupEditCliOptions,
    | 'createDataFile'
    | 'createExpectedFile'
    | 'editDataFile'
    | 'editExpectedFile'
...
export async function runUnitGroupEditSmoke(
  options: UnitGroupEditCliOptions,
  dependencies: UnitGroupEditDependencies = {},
): Promise<UnitGroupEditResult> {
  ...
  const successFixture = await loadUnitGroupCheckDataFixture(options.successDataFile);
  const editFixture = await loadUnitGroupCheckDataFixture(options.editDataFile);
  ...
  if (!rawSuccessJsonOrdered) {
    throw new Error(`Fixture ${options.successDataFile} does not contain ordered unit group JSON.`);
  }

  if (!rawEditJsonOrdered) {
    throw new Error(`Fixture ${options.editDataFile} does not contain ordered unit group JSON.`);
  }
  ...
  try {
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildUnitGroupEditRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        createStep,
...
async function runEditStage(input: {
  ...
  const preparedJsonOrdered = prepareUnitGroupJsonOrderedForRuntime(input.rawJsonOrdered, {
  ...
  if (updateResult.error) {
    throw new Error(`Update unit group failed: ${updateResult.error.message}.`);
  }

  const record = await queryUnitGroupRecord(input.modules.supabase, input.runtimeId, input.version);
