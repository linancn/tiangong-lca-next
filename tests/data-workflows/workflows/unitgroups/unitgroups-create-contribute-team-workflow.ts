...
import {
  UNITGROUP_CREATE_CONTRIBUTE_TEAM_DATA_WORKFLOW_HELP,
  parseCreateContributeTeamCliArgs,
  runUnitGroupCreateContributeTeamSmoke,
} from './unitgroups-create-contribute-team-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(UNITGROUP_CREATE_CONTRIBUTE_TEAM_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runUnitGroupCreateContributeTeamSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('UnitGroup create-contribute-team data workflow');
    ...
    console.log(`Runtime unit group version: ${result.runtimeFixture.version}`);
  ...
  if (!result.passed) {
    throw new Error('UnitGroup create-contribute-team data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('UnitGroup create-contribute-team data workflow passed.');
  }
}
