...
export const DEFAULT_UNITGROUP_CHECK_DATA_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/unitgroups/002_check_data_success.json';
export const DEFAULT_UNITGROUP_CHECK_DATA_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/unitgroups/002_check_data_success.md';
...
export type UnitGroupCheckDataFixture = {
...
export type UnitGroupCheckDataResult = {
...
export type UnitGroupCheckDataRuntimeRecord = {
  check: {
    expectedFile: string;
    expectationResults: ExpectationResult[];
    fixtureFile: string;
...
export type UnitGroupCheckDataDependencies = {
...
Usage:
  pnpm test:workflows --unitgroups:check-data --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --unitgroups:check-data --role user --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one unit group from tests/data-workflows/fixtures/data/unitgroups/001_create.json
  2. Update the same unit group with tests/data-workflows/fixtures/data/unitgroups/002_check_data_success.json
...
Flags:
  ...
  --check-data-file <path>         Defaults to tests/data-workflows/fixtures/data/unitgroups/002_check_data_success.json
...
export function parseCheckDataCliArgs(argv: string[], cwd = process.cwd()): CheckDataCliOptions {
  ...
  const options: CheckDataCliOptions = {
    checkDataFile: path.resolve(cwd, DEFAULT_UNITGROUP_CHECK_DATA_FIXTURE_PATH),
    checkExpectedFile: path.resolve(cwd, DEFAULT_UNITGROUP_CHECK_DATA_EXPECTED_PATH),
    ...
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_UNITGROUP_CHECK_DATA_FIXTURE_PATH),
      cwd,
    ),
...
export async function loadUnitGroupCheckDataFixture(
  filePath: string,
): Promise<UnitGroupCheckDataFixture> {
  const fixture = (await importJsonFile<UnitGroupCheckDataFixture>(
    filePath,
  )) as UnitGroupCheckDataFixture | null;

  if (!fixture || typeof fixture !== 'object' || Array.isArray(fixture)) {
    throw new Error(`Invalid unit group check-data fixture: ${filePath}`);
  }

  if (fixture.table && fixture.table !== 'unitgroups') {
    throw new Error(`Check-data fixture table must be "unitgroups": ${filePath}`);
  }
...
export function getFixtureOrderedJson(fixture: UnitGroupCheckDataFixture) {
...
export function buildUnitGroupCheckDataRuntimeRecord(
  options: Pick<
    CheckDataCliOptions,
    | 'checkDataFile'
    | 'checkExpectedFile'
  ...
  result: Pick<
    ...
    | 'validation'
  > & { passed: boolean },
): UnitGroupCheckDataRuntimeRecord {
...
export async function runUnitGroupCheckDataSmoke(
  options: CheckDataCliOptions,
  dependencies: UnitGroupCheckDataDependencies = {},
): Promise<UnitGroupCheckDataResult> {
  ...
  const checkFixture = await loadUnitGroupCheckDataFixture(options.checkDataFile);
  const rawCheckOrderedJson = getFixtureOrderedJson(checkFixture);

  if (!rawCheckOrderedJson) {
    throw new Error(`Fixture ${options.checkDataFile} does not contain ordered unit group JSON.`);
  }
  ...
  try {
    ...
    const preparedCheckJsonOrdered = prepareUnitGroupJsonOrderedForRuntime(rawCheckOrderedJson, {
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildUnitGroupCheckDataRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        createStep,
        frontendProbe,
