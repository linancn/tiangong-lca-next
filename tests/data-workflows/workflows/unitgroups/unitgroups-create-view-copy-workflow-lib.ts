...
export const DEFAULT_UNITGROUP_CREATE_VIEW_COPY_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/unitgroups/001_create_view_copy.json';
export const DEFAULT_UNITGROUP_CREATE_VIEW_COPY_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/unitgroups/001_create_view_copy.md';
...
export type UnitGroupCreateViewCopyCliOptions = {
...
export type UnitGroupCreateViewCopyResult = {
...
type StepRuntimeRecord = {
  expectationResults: ExpectationResult[];
  persistedRecord: Omit<UnitGroupWorkflowRecord, 'json' | 'json_ordered'>;
  submittedRuleVerification: boolean | null;
};

export type UnitGroupCreateViewCopyRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type UnitGroupCreateViewCopyDependencies = {
...
Usage:
  pnpm test:workflows --unitgroups:create-view-copy --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --unitgroups:create-view-copy --role user --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one unit group from tests/data-workflows/fixtures/data/unitgroups/001_create_view_copy.json
  2. View the created unit group through getUnitGroupDetail
  3. Copy that unit group through the real createUnitGroup flow
...
Flags:
  ...
  --create-data-file <path>        Defaults to tests/data-workflows/fixtures/data/unitgroups/001_create_view_copy.json
  ...
  --keep-data                      Keep both source and copied unitgroups after validation (default)
  --keep-created                   Alias of --keep-data
  --no-keep-data                   Delete the workflow unitgroups after validation
...
export function parseCreateViewCopyCliArgs(
  argv: string[],
  cwd = process.cwd(),
): UnitGroupCreateViewCopyCliOptions {
  let runtimeRecordFileExplicit = false;
  const defaultFollowUpExpectedFile = path.resolve(
    cwd,
    DEFAULT_UNITGROUP_CREATE_VIEW_COPY_EXPECTED_PATH,
  );
  const options: UnitGroupCreateViewCopyCliOptions = {
    createDataFile: path.resolve(cwd, DEFAULT_UNITGROUP_CREATE_VIEW_COPY_FIXTURE_PATH),
    createExpectedFile: path.resolve(cwd, DEFAULT_UNITGROUP_EXPECTED_PATH),
...
export function buildCreateViewCopyExpectations(): ParsedWorkflowExpectation[] {
  return [
    ...
    buildExpectation(
      'copy.ruleVerificationMatchesSource',
      true,
      'Copied rule_verification matches source',
    ),
    buildExpectation(
      'copy.sameContentExceptUuidTimestampPermanentUri',
      true,
    ...
    buildExpectation('copy.permanentUriAbsent', true, 'Copied permanentDataSetURI is absent'),
...
export function normalizeJsonForCopyComparison(jsonOrdered: Record<string, unknown>) {
  ...
  deleteNestedValue(clonedJson, UNITGROUP_UUID_PATH);
...
export function buildUnitGroupCreateViewCopyRuntimeRecord(
  options: Pick<
    UnitGroupCreateViewCopyCliOptions,
    'createDataFile' | 'createExpectedFile' | 'followUpExpectedFile' | 'frontendUrl' | 'keepData'
  >,
  result: Pick<
    UnitGroupCreateViewCopyResult,
    | 'cleanupAttempted'
    | 'cleanupPassed'
    | 'copy'
    | 'copyStep'
    ...
    | 'viewCopyStep'
  > & { passed: boolean },
): UnitGroupCreateViewCopyRuntimeRecord {
  const {
    json: _createJson,
...
export async function runUnitGroupCreateViewCopySmoke(
  options: UnitGroupCreateViewCopyCliOptions,
  dependencies: UnitGroupCreateViewCopyDependencies = {},
): Promise<UnitGroupCreateViewCopyResult> {
  const supabaseTarget = normalizeSupabaseTarget(options);
  const runtimeFixture = prepareRuntimeFixture(await loadUnitGroupFixture(options.createDataFile), {
  ...
  try {
    const submittedCreateRuleVerification = await (
      dependencies.computeCreateRuleVerificationFn ?? computeUnitGroupRuleVerification
    ...
    const sourceDetailResponse = await modules.unitgroupsApi.getUnitGroupDetail(
      createdRecord.id,
      createdRecord.version,
    );

    if (!sourceDetailResponse?.success || !sourceDetailResponse.data?.json) {
      throw new Error('View source unit group failed: getUnitGroupDetail returned no data.');
    }

    const sourceFormData = modules.unitgroupsUtil.genUnitGroupFromData(
      (sourceDetailResponse.data.json as Record<string, any>)?.unitGroupDataSet ?? {},
    );

    if (!sourceFormData) {
      throw new Error('Failed to derive unit group form data from the viewed source detail.');
    }

    const copiedId = (dependencies.generateIdFn ?? randomUUID)();
    if (copiedId === createdRecord.id) {
      throw new Error('Copy unit group failed: generated id matches the source unit group id.');
    }

    const copyCreateResult = await modules.unitgroupsApi.createUnitGroup(copiedId, sourceFormData);
    if (copyCreateResult.error) {
      throw new Error(
        `Copy unit group failed: ${copyCreateResult.error.message ?? 'unknown error'}`,
      );
    }
    ...
    const copiedRecord = await queryUnitGroupRecord(modules.supabase, copiedId, copiedVersion);
    createdRecords.push({
      id: copiedRecord.id,
      version: copiedRecord.version,
    });

    const copiedDetailResponse = await modules.unitgroupsApi.getUnitGroupDetail(
      copiedRecord.id,
      copiedRecord.version,
    );

    if (!copiedDetailResponse?.success || !copiedDetailResponse.data?.json) {
      throw new Error('View copied unit group failed: getUnitGroupDetail returned no data.');
    }
    ...
    const actuals: WorkflowActuals = {
      ...
      'copy.ruleVerificationMatchesSource':
        copiedRecord.rule_verification === createdRecord.rule_verification,
      'copy.sameContentExceptUuidTimestampPermanentUri': comparableJsonMatchesSource,
      'copy.uuidMatchesCopiedId':
        getNestedValue(copiedRecord.json_ordered as Record<string, any>, UNITGROUP_UUID_PATH) ===
        copiedId,
      'copy.permanentUriAbsent': extractPermanentDataSetUri(copiedRecord.json_ordered) == null,
    ...
    if (!options.keepData) {
      ...
      cleanupPassed = await cleanupCreatedUnitGroups(modules.supabase, accessToken, createdRecords);
    }
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildUnitGroupCreateViewCopyRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        copy,
        copyStep,
...
async function loadWorkflowModules(supabaseTarget: SupabaseTarget): Promise<LoadedWorkflowModules> {
  ...
  const unitgroupsApiModule = await import('../../../../src/services/unitgroups/api');
  ...
  const unitgroupsApiExports =
    extractModuleExports<LoadedWorkflowModules['unitgroupsApi']>(unitgroupsApiModule);
  const unitgroupsUtilExports =
    extractModuleExports<LoadedWorkflowModules['unitgroupsUtil']>(unitgroupsUtilModule);

  return {
    unitgroupsApi: {
      createUnitGroup: unitgroupsApiExports.createUnitGroup,
      getUnitGroupDetail: unitgroupsApiExports.getUnitGroupDetail,
    },
    unitgroupsUtil: {
...
async function cleanupCreatedUnitGroups(
  supabase: SupabaseClient,
  accessToken: string,
