...
import {
  UNITGROUP_CHECK_DATA_WORKFLOW_HELP,
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(UNITGROUP_CHECK_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runUnitGroupCheckDataSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('UnitGroup check-data data workflow');
  ...
  if (!result.passed) {
    throw new Error('UnitGroup check-data data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('UnitGroup check-data data workflow passed.');
  }
}
