...
export const DEFAULT_UNITGROUP_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/unitgroups/001_create.json';
export const DEFAULT_UNITGROUP_EXPECTED_PATH =
...
const UNITGROUP_PERMANENT_URI_BASE = 'https://lcdn.tiangong.earth/unitgroups';
...
export type UnitGroupCreateFixture = {
  id: string;
...
export type RuntimeFixture = {
  fixture: UnitGroupCreateFixture;
  sourceFixtureId: string;
  runtimeId: string;
  version: string;
};

export type PreparedUnitGroupJsonOrdered = {
  jsonOrdered: Record<string, any>;
  version: string;
};
...
Usage:
  pnpm test:workflows --unitgroups:create --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --unitgroups:create --role user --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna --no-keep-data
...
Flags:
  ...
  --data-file <path>               Defaults to tests/data-workflows/fixtures/data/unitgroups/001_create.json
  ...
  --keep-data                      Keep the created unit group after validation (default)
  --keep-created                   Alias of --keep-data
  --no-keep-data                   Delete the created unit group after validation
...
export function parseCliArgs(argv: string[], cwd = process.cwd()): CliOptions {
  ...
  const options: CliOptions = {
    dataFile: path.resolve(cwd, DEFAULT_UNITGROUP_FIXTURE_PATH),
    expectedFile: path.resolve(cwd, DEFAULT_UNITGROUP_EXPECTED_PATH),
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_UNITGROUP_ROLE,
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_UNITGROUP_FIXTURE_PATH),
      cwd,
    ),
...
export async function loadUnitGroupFixture(filePath: string): Promise<UnitGroupCreateFixture> {
  const fixture = await loadJsonFile<UnitGroupCreateFixture>(filePath);

  if (!fixture || typeof fixture !== 'object') {
    throw new Error(`Invalid unit group fixture: ${filePath}`);
  }
  ...
  if (fixture.table !== 'unitgroups') {
    throw new Error(`Fixture table must be "unitgroups": ${filePath}`);
  }
...
export function prepareRuntimeFixture(
  ...
  const preparedJsonOrdered = prepareUnitGroupJsonOrderedForRuntime(clonedFixture.jsonOrdered, {
    runtimeId,
  });
...
export function prepareUnitGroupJsonOrderedForRuntime(
  jsonOrdered: Record<string, any>,
  options: {
    runtimeId: string;
    version?: string;
  },
): PreparedUnitGroupJsonOrdered {
  const clonedJsonOrdered = replaceRuntimeUnitGroupIdPlaceholder(
    deepClone(jsonOrdered),
    options.runtimeId,
  );
  setNestedValue(clonedJsonOrdered, UNITGROUP_UUID_PATH, options.runtimeId);

  const version = options.version ?? getUnitGroupVersion(clonedJsonOrdered);
  if (!version) {
    throw new Error('Unable to resolve unit group dataset version from jsonOrdered.');
  }

  setNestedValue(clonedJsonOrdered, UNITGROUP_VERSION_PATH, version);
...
export async function computeUnitGroupRuleVerification(
  jsonOrdered: Record<string, any>,
): Promise<boolean> {
  const sdkValidation = createTidasUnitGroup(jsonOrdered).validateEnhanced();
...
export async function runUnitGroupCreateSmoke(
  options: CliOptions,
  ...
  let submittedRuleVerification = await computeUnitGroupRuleVerification(
  ...
  submittedRuleVerification = await computeUnitGroupRuleVerification(
    runtimeFixture.fixture.jsonOrdered,
  );
  ...
  if (recordResult.error || !recordResult.data) {
    throw new Error(
      `Failed to query created unit group: ${recordResult.error?.message ?? 'record not found'}`,
    );
  }
...
function getUnitGroupVersion(jsonOrdered: Record<string, any>) {
  const version = getNestedValue<string>(jsonOrdered, UNITGROUP_VERSION_PATH);
...
export function buildPermanentDataSetUri(id: string, version: string) {
  return `${UNITGROUP_PERMANENT_URI_BASE}/${id}?version=${version}`;
}
...
function replaceRuntimeUnitGroupIdPlaceholder<T>(value: T, runtimeId: string): T {
  if (typeof value === 'string') {
    return value.split(RUNTIME_UNITGROUP_ID_PLACEHOLDER).join(runtimeId) as T;
  }

  if (Array.isArray(value)) {
    return value.map((item) => replaceRuntimeUnitGroupIdPlaceholder(item, runtimeId)) as T;
  }
  ...
  return Object.entries(value as Record<string, unknown>).reduce<Record<string, unknown>>(
    (accumulator, [key, nestedValue]) => {
      accumulator[key] = replaceRuntimeUnitGroupIdPlaceholder(nestedValue, runtimeId);
      return accumulator;
    },
    {},
  ) as T;
