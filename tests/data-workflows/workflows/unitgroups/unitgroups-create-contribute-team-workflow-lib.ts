...
import {
  DEFAULT_UNITGROUP_CONTRIBUTE_TEAM_ROLE,
  DEFAULT_USERS_PATH,
...
export const DEFAULT_UNITGROUP_CREATE_CONTRIBUTE_TEAM_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/unitgroups/005_create_contribute_team.json';
export const DEFAULT_UNITGROUP_CREATE_CONTRIBUTE_TEAM_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/unitgroups/005_create_contribute_team.md';
export const DEFAULT_UNITGROUP_CREATE_CONTRIBUTE_TEAM_CREATE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/unitgroups/005_create_contribute_team_create.md';
...
export type UnitGroupCreateContributeTeamCliOptions = {
...
export type UnitGroupCreateContributeTeamResult = {
...
export type UnitGroupCreateContributeTeamRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type UnitGroupCreateContributeTeamDependencies = {
...
Usage:
  pnpm test:workflows --unitgroups:create-contribute-team --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --unitgroups:create-contribute-team --role user --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna
...
Workflow:
  ...
  3. Contribute that unit group through the real contributeSource('unitgroups', id, version) flow
...
Flags:
  ...
  --create-data-file <path>        Defaults to tests/data-workflows/fixtures/data/unitgroups/005_create_contribute_team.json
  ...
  --keep-data                      Keep the contributed unit group after validation (default)
...
export function parseCreateContributeTeamCliArgs(
  argv: string[],
  cwd = process.cwd(),
): UnitGroupCreateContributeTeamCliOptions {
  let runtimeRecordFileExplicit = false;
  const defaultFollowUpExpectedFile = path.resolve(
    cwd,
    DEFAULT_UNITGROUP_CREATE_CONTRIBUTE_TEAM_EXPECTED_PATH,
  );
  const options: UnitGroupCreateContributeTeamCliOptions = {
    createDataFile: path.resolve(cwd, DEFAULT_UNITGROUP_CREATE_CONTRIBUTE_TEAM_FIXTURE_PATH),
    createExpectedFile: path.resolve(
      cwd,
      DEFAULT_UNITGROUP_CREATE_CONTRIBUTE_TEAM_CREATE_EXPECTED_PATH,
    ),
    followUpExpectedFile: defaultFollowUpExpectedFile,
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_UNITGROUP_CONTRIBUTE_TEAM_ROLE,
...
export function buildUnitGroupCreateContributeTeamRuntimeRecord(
  options: Pick<
    UnitGroupCreateContributeTeamCliOptions,
    'createDataFile' | 'createExpectedFile' | 'followUpExpectedFile' | 'frontendUrl' | 'keepData'
  >,
  result: Pick<
    UnitGroupCreateContributeTeamResult,
    | 'cleanupAttempted'
    | 'cleanupPassed'
    | 'contribute'
    | 'contributeStep'
    ...
    | 'supabaseTarget'
  > & { passed: boolean },
): UnitGroupCreateContributeTeamRuntimeRecord {
...
export async function runUnitGroupCreateContributeTeamSmoke(
  options: UnitGroupCreateContributeTeamCliOptions,
  dependencies: UnitGroupCreateContributeTeamDependencies = {},
): Promise<UnitGroupCreateContributeTeamResult> {
  ...
  try {
    ...
    if (contributeError) {
      throw new Error(`Contribute unit group failed: ${extractErrorMessage(contributeError)}`);
    }

    const contributedRecord = await queryUnitGroupRecord(
      modules.supabase,
      createdRecord.id,
      createdRecord.version,
    );
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildUnitGroupCreateContributeTeamRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        contribute,
        contributeStep,
