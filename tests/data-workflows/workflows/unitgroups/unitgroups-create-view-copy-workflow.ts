...
import {
  UNITGROUP_CREATE_VIEW_COPY_DATA_WORKFLOW_HELP,
  parseCreateViewCopyCliArgs,
  runUnitGroupCreateViewCopySmoke,
} from './unitgroups-create-view-copy-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(UNITGROUP_CREATE_VIEW_COPY_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runUnitGroupCreateViewCopySmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('UnitGroup create-view-copy data workflow');
    ...
    console.log(`Runtime source unit group ID: ${result.runtimeFixture.runtimeId}`);
    console.log(`Copied unit group ID: ${result.copy.copiedId}`);
    console.log(`Copied unit group version: ${result.copy.copiedVersion}`);
    console.log(
  ...
  if (!result.passed) {
    throw new Error('UnitGroup create-view-copy data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('UnitGroup create-view-copy data workflow passed.');
  }
}
