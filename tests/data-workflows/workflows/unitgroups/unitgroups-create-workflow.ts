...
import {
  UNITGROUP_CREATE_DATA_WORKFLOW_HELP,
  parseCliArgs,
  runUnitGroupCreateSmoke,
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(UNITGROUP_CREATE_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runUnitGroupCreateSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('UnitGroup create data workflow');
  ...
  if (!result.passed) {
    throw new Error('UnitGroup create data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('UnitGroup create data workflow passed.');
  }
}
