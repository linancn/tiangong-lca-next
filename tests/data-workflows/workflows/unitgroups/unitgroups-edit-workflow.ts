...
import {
  UNITGROUP_EDIT_DATA_WORKFLOW_HELP,
  parseEditCliArgs,
  runUnitGroupEditSmoke,
} from './unitgroups-edit-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(UNITGROUP_EDIT_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runUnitGroupEditSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('UnitGroup edit data workflow');
  ...
  if (!result.passed) {
    throw new Error('UnitGroup edit data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('UnitGroup edit data workflow passed.');
  }
}
