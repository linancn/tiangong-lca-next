...
} from './flows-check-data-workflow-lib';
import {
...
export const DEFAULT_FLOW_EDIT_FAILURE_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/flows/003_edit_data_validate_false.json';
export const DEFAULT_FLOW_EDIT_FAILURE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/flows/003_edit_data_validate_false.md';
...
export type FlowEditCliOptions = {
...
export type FlowEditRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type FlowEditDependencies = {
...
Usage:
  pnpm test:workflows --flows:edit --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --flows:edit --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one flow from tests/data-workflows/fixtures/data/flows/001_create.json
  2. Edit the same flow into a validation-passing record from tests/data-workflows/fixtures/data/flows/002_check_data_success.json
  3. Edit the same flow into a validation-failing record from tests/data-workflows/fixtures/data/flows/003_edit_data_validate_false.json
...
Flags:
  ...
  --create-data-file <path>        Defaults to tests/data-workflows/fixtures/data/flows/001_create.json
  --success-data-file <path>       Defaults to tests/data-workflows/fixtures/data/flows/002_check_data_success.json
  --edit-data-file <path>          Defaults to tests/data-workflows/fixtures/data/flows/003_edit_data_validate_false.json
  ...
  --keep-data                      Keep the workflow flow after validation (default)
  --keep-created                   Alias of --keep-data
  --no-keep-data                   Delete the workflow flow after validation
...
export function parseEditCliArgs(argv: string[], cwd = process.cwd()): FlowEditCliOptions {
  let runtimeRecordFileExplicit = false;
  const options: FlowEditCliOptions = {
    createDataFile: path.resolve(cwd, DEFAULT_FLOW_FIXTURE_PATH),
    createExpectedFile: path.resolve(cwd, DEFAULT_FLOW_EXPECTED_PATH),
    editDataFile: path.resolve(cwd, DEFAULT_FLOW_EDIT_FAILURE_FIXTURE_PATH),
    editExpectedFile: path.resolve(cwd, DEFAULT_FLOW_EDIT_FAILURE_EXPECTED_PATH),
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_FLOW_ROLE,
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_FLOW_EDIT_FAILURE_FIXTURE_PATH),
      cwd,
    ),
    successDataFile: path.resolve(cwd, DEFAULT_FLOW_CHECK_DATA_FIXTURE_PATH),
    successExpectedFile: path.resolve(cwd, DEFAULT_FLOW_CHECK_DATA_EXPECTED_PATH),
...
export function buildFlowEditRuntimeRecord(
  options: Pick<
    FlowEditCliOptions,
    | 'createDataFile'
    | 'createExpectedFile'
...
export async function runFlowEditSmoke(
  options: FlowEditCliOptions,
  dependencies: FlowEditDependencies = {},
  ...
  const successFixture = await loadFlowCheckDataFixture(options.successDataFile);
  const editFixture = await loadFlowCheckDataFixture(options.editDataFile);
  ...
  if (!rawSuccessJsonOrdered) {
    throw new Error(`Fixture ${options.successDataFile} does not contain ordered flow JSON.`);
  }

  if (!rawEditJsonOrdered) {
    throw new Error(`Fixture ${options.editDataFile} does not contain ordered flow JSON.`);
  }
  ...
  try {
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildFlowEditRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        createStep,
...
async function runEditStage(input: {
  ...
  const preparedJsonOrdered = prepareFlowJsonOrderedForRuntime(input.rawJsonOrdered, {
  ...
  if (updateResult.error) {
    throw new Error(`Update flow failed: ${updateResult.error.message}.`);
  }

  const record = await queryFlowRecord(input.modules.supabase, input.runtimeId, input.version);
