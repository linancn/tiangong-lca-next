...
export const DEFAULT_FLOW_CREATE_VIEW_COPY_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/flows/001_create_view_copy.json';
export const DEFAULT_FLOW_CREATE_VIEW_COPY_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/flows/001_create_view_copy.md';
...
export type FlowCreateViewCopyCliOptions = {
...
export type FlowCreateViewCopyResult = {
...
type StepRuntimeRecord = {
  expectationResults: ExpectationResult[];
  persistedRecord: Omit<FlowWorkflowRecord, 'json' | 'json_ordered'>;
  submittedRuleVerification: boolean | null;
};

export type FlowCreateViewCopyRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type FlowCreateViewCopyDependencies = {
...
Usage:
  pnpm test:workflows --flows:create-view-copy --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --flows:create-view-copy --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one flow from tests/data-workflows/fixtures/data/flows/001_create_view_copy.json
  2. View the created flow through getFlowDetail
  3. Copy that flow through the real createFlows flow
  4. View the copied flow through getFlowDetail
...
Flags:
  ...
  --create-data-file <path>        Defaults to tests/data-workflows/fixtures/data/flows/001_create_view_copy.json
  ...
  --keep-data                      Keep both source and copied flows after validation (default)
  --keep-created                   Alias of --keep-data
  --no-keep-data                   Delete the workflow flows after validation
...
export function parseCreateViewCopyCliArgs(
  argv: string[],
  cwd = process.cwd(),
): FlowCreateViewCopyCliOptions {
  let runtimeRecordFileExplicit = false;
  const defaultFollowUpExpectedFile = path.resolve(
    cwd,
    DEFAULT_FLOW_CREATE_VIEW_COPY_EXPECTED_PATH,
  );
  const options: FlowCreateViewCopyCliOptions = {
    createDataFile: path.resolve(cwd, DEFAULT_FLOW_CREATE_VIEW_COPY_FIXTURE_PATH),
    createExpectedFile: path.resolve(cwd, DEFAULT_FLOW_EXPECTED_PATH),
...
export function normalizeJsonForCopyComparison(jsonOrdered: Record<string, unknown>) {
  ...
  deleteNestedValue(clonedJson, FLOW_UUID_PATH);
...
export function buildFlowCreateViewCopyRuntimeRecord(
  options: Pick<
    FlowCreateViewCopyCliOptions,
    'createDataFile' | 'createExpectedFile' | 'followUpExpectedFile' | 'frontendUrl' | 'keepData'
  >,
  result: Pick<
    FlowCreateViewCopyResult,
    | 'cleanupAttempted'
    | 'cleanupPassed'
    | 'copy'
    | 'copyStep'
    ...
    | 'viewCopyStep'
  > & { passed: boolean },
): FlowCreateViewCopyRuntimeRecord {
  const {
    json: _createJson,
...
export async function runFlowCreateViewCopySmoke(
  options: FlowCreateViewCopyCliOptions,
  dependencies: FlowCreateViewCopyDependencies = {},
): Promise<FlowCreateViewCopyResult> {
  const supabaseTarget = normalizeSupabaseTarget(options);
  const runtimeFixture = prepareRuntimeFixture(await loadFlowFixture(options.createDataFile), {
  ...
  try {
    ...
    const sourceDetailResponse = await modules.flowsApi.getFlowDetail(
      createdRecord.id,
      createdRecord.version,
    );

    if (!sourceDetailResponse?.success || !sourceDetailResponse.data?.json) {
      throw new Error('View source flow failed: getFlowDetail returned no data.');
    }

    const sourceFormData = modules.flowsUtil.genFlowFromData(
      (sourceDetailResponse.data.json as Record<string, any>)?.flowDataSet ?? {},
    );

    if (!sourceFormData) {
      throw new Error('Failed to derive flow form data from the viewed source detail.');
    }

    const copiedId = (dependencies.generateIdFn ?? randomUUID)();
    if (copiedId === createdRecord.id) {
      throw new Error('Copy flow failed: generated id matches the source flow id.');
    }

    const copyCreateResult = await modules.flowsApi.createFlows(copiedId, sourceFormData);
    if (copyCreateResult.error) {
      throw new Error(`Copy flow failed: ${copyCreateResult.error.message ?? 'unknown error'}`);
    }
    ...
    const copiedRecord = await queryFlowRecord(modules.supabase, copiedId, copiedVersion);
    createdRecords.push({
      id: copiedRecord.id,
      version: copiedRecord.version,
    });

    const copiedDetailResponse = await modules.flowsApi.getFlowDetail(
      copiedRecord.id,
      copiedRecord.version,
    );

    if (!copiedDetailResponse?.success || !copiedDetailResponse.data?.json) {
      throw new Error('View copied flow failed: getFlowDetail returned no data.');
    }
    ...
    const actuals: WorkflowActuals = {
      ...
      'copy.uuidMatchesCopiedId':
        getNestedValue(copiedRecord.json_ordered as Record<string, any>, FLOW_UUID_PATH) ===
        copiedId,
      'copy.permanentUriMatchesCopiedIdAndVersion':
    ...
    if (!options.keepData) {
      ...
      cleanupPassed = await cleanupCreatedFlows(modules.supabase, accessToken, createdRecords);
    }
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildFlowCreateViewCopyRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        copy,
        copyStep,
...
async function loadWorkflowModules(supabaseTarget: SupabaseTarget): Promise<LoadedWorkflowModules> {
  ...
  const flowsApiModule = await import('../../../../src/services/flows/api');
  ...
  const flowsApiExports = extractModuleExports<LoadedWorkflowModules['flowsApi']>(flowsApiModule);
  const flowsUtilExports =
    extractModuleExports<LoadedWorkflowModules['flowsUtil']>(flowsUtilModule);

  return {
    flowsApi: {
      createFlows: flowsApiExports.createFlows,
      getFlowDetail: flowsApiExports.getFlowDetail,
    },
    flowsUtil: {
...
async function cleanupCreatedFlows(
  supabase: SupabaseClient,
  accessToken: string,
