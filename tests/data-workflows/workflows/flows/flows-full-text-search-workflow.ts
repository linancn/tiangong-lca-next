...
import {
  FLOW_FULL_TEXT_SEARCH_DATA_WORKFLOW_HELP,
  parseFlowFullTextSearchCliArgs,
  runFlowFullTextSearchSmoke,
} from './flows-full-text-search-workflow-lib';

export async function main(argv: string[] = process.argv.slice(2)) {
  const detailResultState = extractDetailResultFlag(argv);
  const options = parseFlowFullTextSearchCliArgs(detailResultState.argv);

  if (options.help) {
    printDataWorkflowHelp(FLOW_FULL_TEXT_SEARCH_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runFlowFullTextSearchSmoke(options);
  ...
  if (!showDetailedResult) {
    printCompactSmokeSummary({
      ...
      dataType: 'flow full-text search',
      passed: result.passed,
      version: result.runtimeFixture.version,
    });
  } else {
    console.log('Flow full-text-search data workflow');
  ...
  if (!result.passed) {
    throw new Error('Flow full-text-search data workflow failed.');
  }
}
