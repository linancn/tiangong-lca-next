...
import {
  FLOW_CREATE_VERSION_UPDATE_REFERENCE_DATA_WORKFLOW_HELP,
  parseCreateVersionUpdateReferenceCliArgs,
  runFlowCreateVersionUpdateReferenceSmoke,
} from './flows-create-version-update-reference-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(FLOW_CREATE_VERSION_UPDATE_REFERENCE_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runFlowCreateVersionUpdateReferenceSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Flow create-version-update-reference data workflow');
  ...
  if (!result.passed) {
    throw new Error('Flow create-version-update-reference data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Flow create-version-update-reference data workflow passed.');
  }
}
