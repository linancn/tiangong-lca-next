...
export const DEFAULT_FLOW_CHECK_DATA_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/flows/002_check_data_success.json';
export const DEFAULT_FLOW_CHECK_DATA_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/flows/002_check_data_success.md';
...
export type FlowCheckDataFixture = {
...
export type FlowCheckDataResult = {
...
export type FlowCheckDataRuntimeRecord = {
  check: {
    expectedFile: string;
    expectationResults: ExpectationResult[];
    fixtureFile: string;
...
export type FlowCheckDataDependencies = {
...
Usage:
  pnpm test:workflows --flows:check-data --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --flows:check-data --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one flow from tests/data-workflows/fixtures/data/flows/001_create.json
  2. Update the same flow with tests/data-workflows/fixtures/data/flows/002_check_data_success.json
...
Flags:
  ...
  --check-data-file <path>         Defaults to tests/data-workflows/fixtures/data/flows/002_check_data_success.json
...
export function parseCheckDataCliArgs(argv: string[], cwd = process.cwd()): CheckDataCliOptions {
  ...
  const options: CheckDataCliOptions = {
    checkDataFile: path.resolve(cwd, DEFAULT_FLOW_CHECK_DATA_FIXTURE_PATH),
    checkExpectedFile: path.resolve(cwd, DEFAULT_FLOW_CHECK_DATA_EXPECTED_PATH),
    ...
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_FLOW_CHECK_DATA_FIXTURE_PATH),
      cwd,
    ),
...
export async function loadFlowCheckDataFixture(filePath: string): Promise<FlowCheckDataFixture> {
  const fixture = (await importJsonFile<FlowCheckDataFixture>(
    filePath,
  )) as FlowCheckDataFixture | null;

  if (!fixture || typeof fixture !== 'object' || Array.isArray(fixture)) {
    throw new Error(`Invalid flow check-data fixture: ${filePath}`);
  }

  if (fixture.table && fixture.table !== 'flows') {
    throw new Error(`Check-data fixture table must be "flows": ${filePath}`);
  }
...
export function getFixtureOrderedJson(fixture: FlowCheckDataFixture) {
...
export function buildFlowCheckDataRuntimeRecord(
  options: Pick<
    CheckDataCliOptions,
    | 'checkDataFile'
    | 'checkExpectedFile'
...
export async function runFlowCheckDataSmoke(
  options: CheckDataCliOptions,
  dependencies: FlowCheckDataDependencies = {},
): Promise<FlowCheckDataResult> {
  ...
  const checkFixture = await loadFlowCheckDataFixture(options.checkDataFile);
  const rawCheckOrderedJson = getFixtureOrderedJson(checkFixture);

  if (!rawCheckOrderedJson) {
    throw new Error(`Fixture ${options.checkDataFile} does not contain ordered flow JSON.`);
  }
  ...
  try {
    ...
    const preparedCheckJsonOrdered = prepareFlowJsonOrderedForRuntime(rawCheckOrderedJson, {
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildFlowCheckDataRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        createStep,
        frontendProbe,
