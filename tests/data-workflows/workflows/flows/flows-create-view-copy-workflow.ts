...
import {
  FLOW_CREATE_VIEW_COPY_DATA_WORKFLOW_HELP,
  parseCreateViewCopyCliArgs,
  runFlowCreateViewCopySmoke,
} from './flows-create-view-copy-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(FLOW_CREATE_VIEW_COPY_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runFlowCreateViewCopySmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Flow create-view-copy data workflow');
    ...
    console.log(`Runtime source flow ID: ${result.runtimeFixture.runtimeId}`);
    console.log(`Copied flow ID: ${result.copy.copiedId}`);
    console.log(`Copied flow version: ${result.copy.copiedVersion}`);
    console.log(
  ...
  if (!result.passed) {
    throw new Error('Flow create-view-copy data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Flow create-view-copy data workflow passed.');
  }
}
