...
export const DEFAULT_FLOW_FIXTURE_PATH = 'tests/data-workflows/fixtures/data/flows/001_create.json';
export const DEFAULT_FLOW_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/flows/001_create.md';
...
export const DEFAULT_FLOW_ROLE = 'user';
export const DEFAULT_FLOW_CONTRIBUTE_TEAM_ROLE = 'team-member';
export const TEST_USERS_JSON_ENV = SHARED_TEST_USERS_JSON_ENV;
export const RUNTIME_FLOW_ID_PLACEHOLDER = '__RUNTIME_FLOW_ID__';

const FLOW_PERMANENT_URI_BASE = 'https://lcdn.tiangong.earth/datasetdetail/productFlow.xhtml';
...
export type RuntimeFixture = {
  fixture: FlowCreateFixture;
  sourceFixtureId: string;
  runtimeId: string;
  version: string;
};

export type PreparedFlowJsonOrdered = {
  jsonOrdered: Record<string, any>;
  version: string;
};
...
Usage:
  pnpm test:workflows --flows:create --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --flows:create --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna --no-keep-data
...
Flags:
  ...
  --data-file <path>               Defaults to tests/data-workflows/fixtures/data/flows/001_create.json
  ...
  --keep-data                      Keep the created flow after validation (default)
  --keep-created                   Alias of --keep-data
  --no-keep-data                   Delete the created flow after validation
...
export function parseCliArgs(argv: string[], cwd = process.cwd()): CliOptions {
  ...
  const options: CliOptions = {
    dataFile: path.resolve(cwd, DEFAULT_FLOW_FIXTURE_PATH),
    expectedFile: path.resolve(cwd, DEFAULT_FLOW_EXPECTED_PATH),
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_FLOW_ROLE,
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_FLOW_FIXTURE_PATH),
      cwd,
    ),
...
export async function loadFlowFixture(filePath: string): Promise<FlowCreateFixture> {
  const fixture = await loadJsonFile<FlowCreateFixture>(filePath);

  if (!fixture || typeof fixture !== 'object') {
    throw new Error(`Invalid flow fixture: ${filePath}`);
  }
  ...
  if (fixture.table !== 'flows') {
    throw new Error(`Fixture table must be "flows": ${filePath}`);
  }
...
export function prepareRuntimeFixture(
  ...
  const preparedJsonOrdered = prepareFlowJsonOrderedForRuntime(clonedFixture.jsonOrdered, {
    runtimeId,
  });
...
export function prepareFlowJsonOrderedForRuntime(
  jsonOrdered: Record<string, any>,
  options: {
    runtimeId: string;
    version?: string;
  },
): PreparedFlowJsonOrdered {
  const clonedJsonOrdered = replaceRuntimeFlowIdPlaceholder(
    deepClone(jsonOrdered),
    options.runtimeId,
  );
  setNestedValue(clonedJsonOrdered, FLOW_UUID_PATH, options.runtimeId);

  const version = options.version ?? getFlowVersion(clonedJsonOrdered);
  if (!version) {
    throw new Error('Unable to resolve flow dataset version from jsonOrdered.');
  }

  setNestedValue(clonedJsonOrdered, FLOW_VERSION_PATH, version);
  setNestedValue(
    clonedJsonOrdered,
    FLOW_PERMANENT_URI_PATH,
...
export async function computeFlowRuleVerification(
  jsonOrdered: Record<string, any>,
): Promise<boolean> {
  const sdkValidation = createTidasFlow(jsonOrdered).validateEnhanced();
...
export async function runFlowCreateSmoke(
  options: CliOptions,
  ...
  let submittedRuleVerification = await computeFlowRuleVerification(
  ...
  submittedRuleVerification = await computeFlowRuleVerification(runtimeFixture.fixture.jsonOrdered);
  ...
  if (recordResult.error || !recordResult.data) {
    throw new Error(
      `Failed to query created flow: ${recordResult.error?.message ?? 'record not found'}`,
    );
  }
...
function getFlowVersion(jsonOrdered: Record<string, any>) {
  const version = getNestedValue<string>(jsonOrdered, FLOW_VERSION_PATH);
...
export function buildPermanentDataSetUri(id: string, version: string) {
  return `${FLOW_PERMANENT_URI_BASE}?uuid=${id}&version=${version}`;
}
...
function replaceRuntimeFlowIdPlaceholder<T>(value: T, runtimeId: string): T {
  if (typeof value === 'string') {
    return value.split(RUNTIME_FLOW_ID_PLACEHOLDER).join(runtimeId) as T;
  }

  if (Array.isArray(value)) {
    return value.map((item) => replaceRuntimeFlowIdPlaceholder(item, runtimeId)) as T;
  }
  ...
  return Object.entries(value as Record<string, unknown>).reduce<Record<string, unknown>>(
    (accumulator, [key, nestedValue]) => {
      accumulator[key] = replaceRuntimeFlowIdPlaceholder(nestedValue, runtimeId);
      return accumulator;
    },
    {},
  ) as T;
