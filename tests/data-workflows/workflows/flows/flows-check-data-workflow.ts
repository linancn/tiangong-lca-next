...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(FLOW_CHECK_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runFlowCheckDataSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Flow check-data data workflow');
  ...
  if (!result.passed) {
    throw new Error('Flow check-data data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Flow check-data data workflow passed.');
  }
}
