...
import {
  FLOW_CREATE_DATA_WORKFLOW_HELP,
  parseCliArgs,
  runFlowCreateSmoke,
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(FLOW_CREATE_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runFlowCreateSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Flow create data workflow');
  ...
  if (!result.passed) {
    throw new Error('Flow create data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Flow create data workflow passed.');
  }
}
