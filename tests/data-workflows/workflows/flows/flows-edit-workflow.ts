...
import {
  FLOW_EDIT_DATA_WORKFLOW_HELP,
  parseEditCliArgs,
  runFlowEditSmoke,
} from './flows-edit-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(FLOW_EDIT_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runFlowEditSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Flow edit data workflow');
  ...
  if (!result.passed) {
    throw new Error('Flow edit data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Flow edit data workflow passed.');
  }
}
