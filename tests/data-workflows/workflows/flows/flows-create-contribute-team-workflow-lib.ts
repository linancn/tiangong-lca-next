...
import {
  DEFAULT_FLOW_CONTRIBUTE_TEAM_ROLE,
  DEFAULT_USERS_PATH,
...
export const DEFAULT_FLOW_CREATE_CONTRIBUTE_TEAM_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/flows/005_create_contribute_team.json';
export const DEFAULT_FLOW_CREATE_CONTRIBUTE_TEAM_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/flows/005_create_contribute_team.md';
export const DEFAULT_FLOW_CREATE_CONTRIBUTE_TEAM_CREATE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/flows/005_create_contribute_team_create.md';
...
export type FlowCreateContributeTeamCliOptions = {
...
export type FlowCreateContributeTeamResult = {
...
export type FlowCreateContributeTeamRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type FlowCreateContributeTeamDependencies = {
...
Usage:
  pnpm test:workflows --flows:create-contribute-team --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --flows:create-contribute-team --role user --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna
...
Workflow:
  ...
  3. Contribute that flow through the real contributeSource('flows', id, version) flow
...
Flags:
  ...
  --create-data-file <path>        Defaults to tests/data-workflows/fixtures/data/flows/005_create_contribute_team.json
  ...
  --keep-data                      Keep the contributed flow after validation (default)
...
export function parseCreateContributeTeamCliArgs(
  argv: string[],
  cwd = process.cwd(),
): FlowCreateContributeTeamCliOptions {
  let runtimeRecordFileExplicit = false;
  const defaultFollowUpExpectedFile = path.resolve(
    cwd,
    DEFAULT_FLOW_CREATE_CONTRIBUTE_TEAM_EXPECTED_PATH,
  );
  const options: FlowCreateContributeTeamCliOptions = {
    createDataFile: path.resolve(cwd, DEFAULT_FLOW_CREATE_CONTRIBUTE_TEAM_FIXTURE_PATH),
    createExpectedFile: path.resolve(cwd, DEFAULT_FLOW_CREATE_CONTRIBUTE_TEAM_CREATE_EXPECTED_PATH),
    followUpExpectedFile: defaultFollowUpExpectedFile,
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_FLOW_CONTRIBUTE_TEAM_ROLE,
...
export function buildFlowCreateContributeTeamRuntimeRecord(
  options: Pick<
    FlowCreateContributeTeamCliOptions,
    'createDataFile' | 'createExpectedFile' | 'followUpExpectedFile' | 'frontendUrl' | 'keepData'
  >,
  result: Pick<
    FlowCreateContributeTeamResult,
    | 'cleanupAttempted'
    | 'cleanupPassed'
    | 'contribute'
    | 'contributeStep'
    ...
    | 'supabaseTarget'
  > & { passed: boolean },
): FlowCreateContributeTeamRuntimeRecord {
...
export async function runFlowCreateContributeTeamSmoke(
  options: FlowCreateContributeTeamCliOptions,
  dependencies: FlowCreateContributeTeamDependencies = {},
): Promise<FlowCreateContributeTeamResult> {
  ...
  try {
    ...
    if (contributeError) {
      throw new Error(`Contribute flow failed: ${extractErrorMessage(contributeError)}`);
    }

    const contributedRecord = await queryFlowRecord(
      modules.supabase,
      createdRecord.id,
      createdRecord.version,
    );
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildFlowCreateContributeTeamRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        contribute,
        contributeStep,
