...
import {
  FLOWPROPERTY_CREATE_VERSION_UPDATE_REFERENCE_DATA_WORKFLOW_HELP,
  parseCreateVersionUpdateReferenceCliArgs,
  runFlowpropertyCreateVersionUpdateReferenceSmoke,
} from './flowproperties-create-version-update-reference-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(FLOWPROPERTY_CREATE_VERSION_UPDATE_REFERENCE_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runFlowpropertyCreateVersionUpdateReferenceSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Flowproperty create-version-update-reference data workflow');
  ...
  if (!result.passed) {
    throw new Error('Flowproperty create-version-update-reference data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Flowproperty create-version-update-reference data workflow passed.');
  }
}
