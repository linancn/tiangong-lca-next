...
export const DEFAULT_FLOWPROPERTY_CREATE_VIEW_COPY_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/flowProperties/001_create_view_copy.json';
export const DEFAULT_FLOWPROPERTY_CREATE_VIEW_COPY_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/flowProperties/001_create_view_copy.md';
...
type FlowpropertyWorkflowRecord = {
  id: string;
...
type CreateStepResult = {
  ...
  record: FlowpropertyWorkflowRecord;
...
type LoadedWorkflowModules = {
  ...
  flowpropertiesUtil: {
    genFlowpropertyFromData: (data: any) => any;
  };
...
export type FlowpropertyCreateViewCopyCliOptions = {
...
export type FlowpropertyCreateViewCopyResult = {
...
type StepRuntimeRecord = {
  expectationResults: ExpectationResult[];
  persistedRecord: Omit<FlowpropertyWorkflowRecord, 'json' | 'json_ordered'>;
  submittedRuleVerification: boolean | null;
};

export type FlowpropertyCreateViewCopyRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type FlowpropertyCreateViewCopyDependencies = {
...
const FLOWPROPERTY_TIMESTAMP_PATH = [
  'flowPropertyDataSet',
...
Usage:
  pnpm test:workflows --flowproperties:create-view-copy --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --flowproperties:create-view-copy --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one flowproperty from tests/data-workflows/fixtures/data/flowProperties/001_create_view_copy.json
  2. View the created flowproperty through getFlowpropertyDetail
  3. Copy that flowproperty through the real createFlowproperties flow
  4. View the copied flowproperty through getFlowpropertyDetail
...
Flags:
  ...
  --create-data-file <path>        Defaults to tests/data-workflows/fixtures/data/flowProperties/001_create_view_copy.json
  ...
  --keep-data                      Keep both source and copied flowproperties after validation (default)
  --keep-created                   Alias of --keep-data
  --no-keep-data                   Delete the workflow flowproperties after validation
...
export function parseCreateViewCopyCliArgs(
  argv: string[],
  cwd = process.cwd(),
): FlowpropertyCreateViewCopyCliOptions {
  let runtimeRecordFileExplicit = false;
  const defaultFollowUpExpectedFile = path.resolve(
    cwd,
    DEFAULT_FLOWPROPERTY_CREATE_VIEW_COPY_EXPECTED_PATH,
  );
  const options: FlowpropertyCreateViewCopyCliOptions = {
    createDataFile: path.resolve(cwd, DEFAULT_FLOWPROPERTY_CREATE_VIEW_COPY_FIXTURE_PATH),
    createExpectedFile: path.resolve(cwd, DEFAULT_FLOWPROPERTY_EXPECTED_PATH),
...
export function normalizeJsonForCopyComparison(jsonOrdered: Record<string, unknown>) {
  ...
  deleteNestedValue(clonedJson, FLOWPROPERTY_UUID_PATH);
  deleteNestedValue(clonedJson, FLOWPROPERTY_TIMESTAMP_PATH);
  deleteNestedValue(clonedJson, FLOWPROPERTY_PERMANENT_URI_PATH);
  return deepSortJson(removeEmptyStructuresForComparison(clonedJson) ?? {});
}

export function buildFlowpropertyCreateViewCopyRuntimeRecord(
  options: Pick<
    FlowpropertyCreateViewCopyCliOptions,
    'createDataFile' | 'createExpectedFile' | 'followUpExpectedFile' | 'frontendUrl' | 'keepData'
  >,
  result: Pick<
    FlowpropertyCreateViewCopyResult,
    | 'cleanupAttempted'
    | 'cleanupPassed'
    | 'copy'
    | 'copyStep'
    ...
    | 'viewCopyStep'
  > & { passed: boolean },
): FlowpropertyCreateViewCopyRuntimeRecord {
  const {
    json: _createJson,
...
export async function runFlowpropertyCreateViewCopySmoke(
  options: FlowpropertyCreateViewCopyCliOptions,
  dependencies: FlowpropertyCreateViewCopyDependencies = {},
): Promise<FlowpropertyCreateViewCopyResult> {
  const supabaseTarget = normalizeSupabaseTarget(options);
  const runtimeFixture = prepareRuntimeFixture(
    await loadFlowpropertyFixture(options.createDataFile),
    {
  ...
  try {
    const submittedCreateRuleVerification = await (
      dependencies.computeCreateRuleVerificationFn ?? computeFlowpropertyRuleVerification
    ...
    const createdRecord = await queryFlowpropertyRecord(
      modules.supabase,
    ...
    const sourceDetailResponse = await modules.flowpropertiesApi.getFlowpropertyDetail(
      createdRecord.id,
      createdRecord.version,
    );

    if (!sourceDetailResponse?.success || !sourceDetailResponse.data?.json) {
      throw new Error('View source flowproperty failed: getFlowpropertyDetail returned no data.');
    }

    const sourceFormData = modules.flowpropertiesUtil.genFlowpropertyFromData(
      (sourceDetailResponse.data.json as Record<string, any>)?.flowPropertyDataSet ?? {},
    );

    if (!sourceFormData) {
      throw new Error('Failed to derive flowproperty form data from the viewed source detail.');
    }

    const copiedId = (dependencies.generateIdFn ?? randomUUID)();
    if (copiedId === createdRecord.id) {
      throw new Error('Copy flowproperty failed: generated id matches the source flowproperty id.');
    }

    const copyCreateResult = await modules.flowpropertiesApi.createFlowproperties(
      copiedId,
      sourceFormData,
    );
    if (copyCreateResult.error) {
      throw new Error(
        `Copy flowproperty failed: ${copyCreateResult.error.message ?? 'unknown error'}`,
      );
    }
    ...
    const copiedRecord = await queryFlowpropertyRecord(modules.supabase, copiedId, copiedVersion);
    createdRecords.push({
      id: copiedRecord.id,
      version: copiedRecord.version,
    });

    const copiedDetailResponse = await modules.flowpropertiesApi.getFlowpropertyDetail(
      copiedRecord.id,
      copiedRecord.version,
    );

    if (!copiedDetailResponse?.success || !copiedDetailResponse.data?.json) {
      throw new Error('View copied flowproperty failed: getFlowpropertyDetail returned no data.');
    }
    ...
    const actuals: WorkflowActuals = {
      ...
      'copy.uuidMatchesCopiedId':
        getNestedValue(copiedRecord.json_ordered as Record<string, any>, FLOWPROPERTY_UUID_PATH) ===
        copiedId,
      'copy.permanentUriMatchesCopiedIdAndVersion':
    ...
    if (!options.keepData) {
      ...
      cleanupPassed = await cleanupCreatedFlowproperties(
        modules.supabase,
        accessToken,
        createdRecords,
      );
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildFlowpropertyCreateViewCopyRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        copy,
        copyStep,
...
async function queryFlowpropertyRecord(
  supabase: SupabaseClient,
  id: string,
  version: string,
  ...
  const result = await supabase
    ...
    .eq('id', id)
    .eq('version', version)
    .maybeSingle<FlowpropertyWorkflowRecord>();

  if (result.error || !result.data) {
    throw new Error(
      `Failed to query flowproperty by id/version: ${result.error?.message ?? 'record not found'}`,
    );
  }
...
async function loadWorkflowModules(supabaseTarget: SupabaseTarget): Promise<LoadedWorkflowModules> {
  ...
  const flowpropertiesApiModule = await import('../../../../src/services/flowproperties/api');
  const flowpropertiesUtilModule = await import('../../../../src/services/flowproperties/util');

  const supabaseExports = extractModuleExports<{ supabase: SupabaseClient }>(supabaseModule);
  const flowpropertiesApiExports =
    extractModuleExports<LoadedWorkflowModules['flowpropertiesApi']>(flowpropertiesApiModule);
  const flowpropertiesUtilExports =
    extractModuleExports<LoadedWorkflowModules['flowpropertiesUtil']>(flowpropertiesUtilModule);

  return {
    flowpropertiesApi: {
      createFlowproperties: flowpropertiesApiExports.createFlowproperties,
      getFlowpropertyDetail: flowpropertiesApiExports.getFlowpropertyDetail,
    },
    flowpropertiesUtil: {
      genFlowpropertyFromData: flowpropertiesUtilExports.genFlowpropertyFromData,
    },
...
function extractPermanentDataSetUri(jsonOrdered: Record<string, unknown>) {
  const uri = getNestedValue(jsonOrdered, FLOWPROPERTY_PERMANENT_URI_PATH);
  return typeof uri === 'string' ? uri : undefined;
}

async function cleanupCreatedFlowproperties(
  supabase: SupabaseClient,
  accessToken: string,
