...
export const DEFAULT_FLOWPROPERTY_EDIT_FAILURE_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/flowProperties/003_edit_data_validate_false.json';
export const DEFAULT_FLOWPROPERTY_EDIT_FAILURE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/flowProperties/003_edit_data_validate_false.md';
...
export type FlowpropertyEditCliOptions = {
...
export type FlowpropertyEditResult = {
...
export type FlowpropertyEditRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type FlowpropertyEditDependencies = {
...
Usage:
  pnpm test:workflows --flowproperties:edit --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --flowproperties:edit --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one flowproperty from tests/data-workflows/fixtures/data/flowProperties/001_create.json
  2. Edit the same flowproperty into a validation-passing record from tests/data-workflows/fixtures/data/flowProperties/002_check_data_success.json
  3. Edit the same flowproperty into a validation-failing record from tests/data-workflows/fixtures/data/flowProperties/003_edit_data_validate_false.json
...
Flags:
  ...
  --create-data-file <path>        Defaults to tests/data-workflows/fixtures/data/flowProperties/001_create.json
  --success-data-file <path>       Defaults to tests/data-workflows/fixtures/data/flowProperties/002_check_data_success.json
  --edit-data-file <path>          Defaults to tests/data-workflows/fixtures/data/flowProperties/003_edit_data_validate_false.json
  ...
  --keep-data                      Keep the workflow flowproperty after validation (default)
  --keep-created                   Alias of --keep-data
  --no-keep-data                   Delete the workflow flowproperty after validation
...
export function parseEditCliArgs(argv: string[], cwd = process.cwd()): FlowpropertyEditCliOptions {
  let runtimeRecordFileExplicit = false;
  const options: FlowpropertyEditCliOptions = {
    createDataFile: path.resolve(cwd, DEFAULT_FLOWPROPERTY_FIXTURE_PATH),
    createExpectedFile: path.resolve(cwd, DEFAULT_FLOWPROPERTY_EXPECTED_PATH),
    editDataFile: path.resolve(cwd, DEFAULT_FLOWPROPERTY_EDIT_FAILURE_FIXTURE_PATH),
    editExpectedFile: path.resolve(cwd, DEFAULT_FLOWPROPERTY_EDIT_FAILURE_EXPECTED_PATH),
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_FLOWPROPERTY_ROLE,
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_FLOWPROPERTY_EDIT_FAILURE_FIXTURE_PATH),
      cwd,
    ),
    successDataFile: path.resolve(cwd, DEFAULT_FLOWPROPERTY_CHECK_DATA_FIXTURE_PATH),
    successExpectedFile: path.resolve(cwd, DEFAULT_FLOWPROPERTY_CHECK_DATA_EXPECTED_PATH),
...
export function buildFlowpropertyEditRuntimeRecord(
  options: Pick<
    FlowpropertyEditCliOptions,
    | 'createDataFile'
    | 'createExpectedFile'
    | 'editDataFile'
    | 'editExpectedFile'
...
export async function runFlowpropertyEditSmoke(
  options: FlowpropertyEditCliOptions,
  dependencies: FlowpropertyEditDependencies = {},
): Promise<FlowpropertyEditResult> {
  ...
  const successFixture = await loadFlowpropertyCheckDataFixture(options.successDataFile);
  const editFixture = await loadFlowpropertyCheckDataFixture(options.editDataFile);
  ...
  if (!rawSuccessJsonOrdered) {
    throw new Error(
      `Fixture ${options.successDataFile} does not contain ordered flowproperty JSON.`,
    );
  }

  if (!rawEditJsonOrdered) {
    throw new Error(`Fixture ${options.editDataFile} does not contain ordered flowproperty JSON.`);
  }
  ...
  try {
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildFlowpropertyEditRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        createStep,
...
async function runEditStage(input: {
  ...
  const preparedJsonOrdered = prepareFlowpropertyJsonOrderedForRuntime(input.rawJsonOrdered, {
  ...
  if (updateResult.error) {
    throw new Error(`Update flowproperty failed: ${updateResult.error.message}.`);
  }

  const record = await queryFlowpropertyRecord(
