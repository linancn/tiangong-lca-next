...
import {
  FLOWPROPERTY_CREATE_DATA_WORKFLOW_HELP,
  parseCliArgs,
  runFlowpropertyCreateSmoke,
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(FLOWPROPERTY_CREATE_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runFlowpropertyCreateSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Flowproperty create data workflow');
  ...
  if (!result.passed) {
    throw new Error('Flowproperty create data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Flowproperty create data workflow passed.');
  }
}
