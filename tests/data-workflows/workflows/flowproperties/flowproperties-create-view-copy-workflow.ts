...
import {
  FLOWPROPERTY_CREATE_VIEW_COPY_DATA_WORKFLOW_HELP,
  parseCreateViewCopyCliArgs,
  runFlowpropertyCreateViewCopySmoke,
} from './flowproperties-create-view-copy-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(FLOWPROPERTY_CREATE_VIEW_COPY_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runFlowpropertyCreateViewCopySmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Flowproperty create-view-copy data workflow');
    ...
    console.log(`Runtime source flowproperty ID: ${result.runtimeFixture.runtimeId}`);
    console.log(`Copied flowproperty ID: ${result.copy.copiedId}`);
    console.log(`Copied flowproperty version: ${result.copy.copiedVersion}`);
    console.log(
  ...
  if (!result.passed) {
    throw new Error('Flowproperty create-view-copy data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Flowproperty create-view-copy data workflow passed.');
  }
}
