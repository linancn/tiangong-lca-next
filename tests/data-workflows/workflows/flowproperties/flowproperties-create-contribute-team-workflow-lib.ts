...
import {
  DEFAULT_FLOWPROPERTY_CONTRIBUTE_TEAM_ROLE,
  DEFAULT_USERS_PATH,
...
export const DEFAULT_FLOWPROPERTY_CREATE_CONTRIBUTE_TEAM_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/flowProperties/005_create_contribute_team.json';
export const DEFAULT_FLOWPROPERTY_CREATE_CONTRIBUTE_TEAM_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/flowProperties/005_create_contribute_team.md';
export const DEFAULT_FLOWPROPERTY_CREATE_CONTRIBUTE_TEAM_CREATE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/flowProperties/005_create_contribute_team_create.md';
...
export type FlowpropertyCreateContributeTeamCliOptions = {
...
export type FlowpropertyCreateContributeTeamResult = {
...
export type FlowpropertyCreateContributeTeamRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type FlowpropertyCreateContributeTeamDependencies = {
...
Usage:
  pnpm test:workflows --flowproperties:create-contribute-team --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --flowproperties:create-contribute-team --role user --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna
...
Workflow:
  ...
  3. Contribute that flowproperty through the real contributeSource('flowproperties', id, version) flow
...
Flags:
  ...
  --create-data-file <path>        Defaults to tests/data-workflows/fixtures/data/flowProperties/005_create_contribute_team.json
  ...
  --keep-data                      Keep the contributed flowproperty after validation (default)
...
export function parseCreateContributeTeamCliArgs(
  argv: string[],
  cwd = process.cwd(),
): FlowpropertyCreateContributeTeamCliOptions {
  let runtimeRecordFileExplicit = false;
  const defaultFollowUpExpectedFile = path.resolve(
    cwd,
    DEFAULT_FLOWPROPERTY_CREATE_CONTRIBUTE_TEAM_EXPECTED_PATH,
  );
  const options: FlowpropertyCreateContributeTeamCliOptions = {
    createDataFile: path.resolve(cwd, DEFAULT_FLOWPROPERTY_CREATE_CONTRIBUTE_TEAM_FIXTURE_PATH),
    createExpectedFile: path.resolve(
      cwd,
      DEFAULT_FLOWPROPERTY_CREATE_CONTRIBUTE_TEAM_CREATE_EXPECTED_PATH,
    ),
    followUpExpectedFile: defaultFollowUpExpectedFile,
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_FLOWPROPERTY_CONTRIBUTE_TEAM_ROLE,
...
export function buildFlowpropertyCreateContributeTeamRuntimeRecord(
  options: Pick<
    FlowpropertyCreateContributeTeamCliOptions,
    'createDataFile' | 'createExpectedFile' | 'followUpExpectedFile' | 'frontendUrl' | 'keepData'
  >,
  result: Pick<
    FlowpropertyCreateContributeTeamResult,
    | 'cleanupAttempted'
    | 'cleanupPassed'
    | 'contribute'
    | 'contributeStep'
    ...
    | 'supabaseTarget'
  > & { passed: boolean },
): FlowpropertyCreateContributeTeamRuntimeRecord {
...
export async function runFlowpropertyCreateContributeTeamSmoke(
  options: FlowpropertyCreateContributeTeamCliOptions,
  dependencies: FlowpropertyCreateContributeTeamDependencies = {},
): Promise<FlowpropertyCreateContributeTeamResult> {
  ...
  try {
    ...
    if (contributeError) {
      throw new Error(`Contribute flowproperty failed: ${extractErrorMessage(contributeError)}`);
    }

    const contributedRecord = await queryFlowpropertyRecord(
      modules.supabase,
      createdRecord.id,
      createdRecord.version,
    );
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildFlowpropertyCreateContributeTeamRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        contribute,
        contributeStep,
