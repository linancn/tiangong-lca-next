...
export const DEFAULT_FLOWPROPERTY_CREATE_VERSION_UPDATE_REFERENCE_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/flowProperties/004_create_version_update_reference.json';
export const DEFAULT_FLOWPROPERTY_CREATE_VERSION_UPDATE_REFERENCE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/flowProperties/004_create_version_update_reference.md';
...
type LoadedWorkflowModules = {
  flowpropertiesUtil: {
    ...
    genFlowpropertyJsonOrdered: (id: string, data: any) => Record<string, any>;
  };
  generalApi: {
...
export type FlowpropertyCreateVersionUpdateReferenceCliOptions = {
  dataFile: string;
  expectedFile: string;
  frontendUrl?: string;
  generateId: boolean;
...
export type FlowpropertyCreateVersionUpdateReferenceResult = {
...
type StepRuntimeRecord = {
  expectationResults: ExpectationResult[];
  persistedRecord: Omit<FlowpropertyWorkflowRecord, 'json_ordered'>;
  submittedRuleVerification: boolean;
};

export type FlowpropertyCreateVersionUpdateReferenceRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type FlowpropertyCreateVersionUpdateReferenceDependencies = {
...
Usage:
  pnpm test:workflows --flowproperties:create-version-update-reference --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --flowproperties:create-version-update-reference --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna
...
Workflow:
  ...
  2. Create a new version on the same flowproperty id
...
Flags:
  ...
  --data-file <path>               Defaults to tests/data-workflows/fixtures/data/flowProperties/004_create_version_update_reference.json
...
export function parseCreateVersionUpdateReferenceCliArgs(
  argv: string[],
  cwd = process.cwd(),
): FlowpropertyCreateVersionUpdateReferenceCliOptions {
  let runtimeRecordFileExplicit = false;
  const options: FlowpropertyCreateVersionUpdateReferenceCliOptions = {
    dataFile: path.resolve(cwd, DEFAULT_FLOWPROPERTY_CREATE_VERSION_UPDATE_REFERENCE_FIXTURE_PATH),
    expectedFile: path.resolve(
      cwd,
      DEFAULT_FLOWPROPERTY_CREATE_VERSION_UPDATE_REFERENCE_EXPECTED_PATH,
    ),
    generateId: true,
    help: false,
    keepData: true,
    ...
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_FLOWPROPERTY_CREATE_VERSION_UPDATE_REFERENCE_FIXTURE_PATH),
      cwd,
    ),
...
export function normalizeJsonForCreateVersionComparison(jsonOrdered: Record<string, unknown>) {
  ...
  deleteNestedValue(clonedJson, FLOWPROPERTY_VERSION_PATH);
...
export function buildFlowpropertyCreateVersionUpdateReferenceRuntimeRecord(
  options: Pick<
    FlowpropertyCreateVersionUpdateReferenceCliOptions,
    'dataFile' | 'expectedFile' | 'frontendUrl' | 'keepData'
  >,
  result: Pick<
    FlowpropertyCreateVersionUpdateReferenceResult,
    | 'cleanupAttempted'
    | 'cleanupPassed'
    | 'createStep'
    | 'createVersionStep'
    ...
    | 'versionCreation'
  > & { passed: boolean },
): FlowpropertyCreateVersionUpdateReferenceRuntimeRecord {
...
export async function runFlowpropertyCreateVersionUpdateReferenceSmoke(
  options: FlowpropertyCreateVersionUpdateReferenceCliOptions,
  dependencies: FlowpropertyCreateVersionUpdateReferenceDependencies = {},
): Promise<FlowpropertyCreateVersionUpdateReferenceResult> {
  ...
  try {
    ...
    const createVersionFormData = modules.flowpropertiesUtil.genFlowpropertyFromData(
      (createdRecord.json_ordered as Record<string, any>)?.flowPropertyDataSet ?? {},
    );

    if (!createVersionFormData) {
      throw new Error('Failed to derive flowproperty form data from the created record.');
    }
    ...
    const createVersionJsonOrdered = prepareFlowpropertyJsonOrderedForRuntime(
      modules.flowpropertiesUtil.genFlowpropertyJsonOrdered(
        runtimeFixture.runtimeId,
    ...
    if (createVersionResult.error) {
      throw new Error(`Create flowproperty version failed: ${createVersionResult.error.message}.`);
    }

    const createdVersionRecord = await queryFlowpropertyRecord(
      modules.supabase,
      runtimeFixture.runtimeId,
      createVersion,
    );
    ...
    const updateReferenceFormData = modules.flowpropertiesUtil.genFlowpropertyFromData(
      (createdVersionRecord.json_ordered as Record<string, any>)?.flowPropertyDataSet ?? {},
    );

    if (!updateReferenceFormData) {
      throw new Error('Failed to derive flowproperty form data from the created version record.');
    }
    ...
    const updatedJsonOrdered = prepareFlowpropertyJsonOrderedForRuntime(
      modules.flowpropertiesUtil.genFlowpropertyJsonOrdered(
        runtimeFixture.runtimeId,
    ...
    if (updateReferenceResult.error) {
      throw new Error(
        `Update flowproperty reference failed: ${updateReferenceResult.error.message}.`,
      );
    }

    const updatedRecord = await queryFlowpropertyRecord(
      modules.supabase,
      runtimeFixture.runtimeId,
    ...
    const actuals: WorkflowActuals = {
      ...
      'createVersion.ownerRefUriMatchesRuntimeId':
        createVersionOwnerReference.uri === buildFlowpropertyReferenceUri(runtimeFixture.runtimeId),
      'createVersion.ownerRefVersionStillEqualsCreateVersion':
      ...
      'updateReference.ownerRefUriMatchesRuntimeId':
        updatedOwnerReference.uri === buildFlowpropertyReferenceUri(runtimeFixture.runtimeId),
    ...
    if (!options.keepData) {
      ...
      cleanupPassed = await cleanupCreatedFlowpropertyVersions(
        modules.supabase,
        accessToken,
        runtimeFixture.runtimeId,
        createdVersions,
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildFlowpropertyCreateVersionUpdateReferenceRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        createStep,
        createVersionStep,
...
async function loadWorkflowModules(supabaseTarget: SupabaseTarget): Promise<LoadedWorkflowModules> {
  ...
  return {
    flowpropertiesUtil: {
      ...
      genFlowpropertyJsonOrdered: flowpropertiesUtilExports.genFlowpropertyJsonOrdered,
    },
    generalApi: {
...
function buildFlowpropertyReferenceUri(id: string) {
  return `../flowproperties/${id}.xml`;
}
...
async function cleanupCreatedFlowpropertyVersions(
  supabase: SupabaseClient,
  accessToken: string,
  runtimeId: string,
  createdVersions: string[],
