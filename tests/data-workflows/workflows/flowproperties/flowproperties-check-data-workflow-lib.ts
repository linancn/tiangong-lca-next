...
export const DEFAULT_FLOWPROPERTY_CHECK_DATA_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/flowProperties/002_check_data_success.json';
export const DEFAULT_FLOWPROPERTY_CHECK_DATA_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/flowProperties/002_check_data_success.md';
...
export type FlowpropertyCheckDataFixture = {
...
export type FlowpropertyCheckDataResult = {
...
export type FlowpropertyCheckDataRuntimeRecord = {
  check: {
    expectedFile: string;
    expectationResults: ExpectationResult[];
    fixtureFile: string;
...
export type FlowpropertyCheckDataDependencies = {
...
Usage:
  pnpm test:workflows --flowproperties:check-data --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --flowproperties:check-data --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one flowproperty from tests/data-workflows/fixtures/data/flowProperties/001_create.json
  2. Update the same flowproperty with tests/data-workflows/fixtures/data/flowProperties/002_check_data_success.json
...
Flags:
  ...
  --check-data-file <path>         Defaults to tests/data-workflows/fixtures/data/flowProperties/002_check_data_success.json
...
export function parseCheckDataCliArgs(argv: string[], cwd = process.cwd()): CheckDataCliOptions {
  ...
  const options: CheckDataCliOptions = {
    checkDataFile: path.resolve(cwd, DEFAULT_FLOWPROPERTY_CHECK_DATA_FIXTURE_PATH),
    checkExpectedFile: path.resolve(cwd, DEFAULT_FLOWPROPERTY_CHECK_DATA_EXPECTED_PATH),
    ...
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_FLOWPROPERTY_CHECK_DATA_FIXTURE_PATH),
      cwd,
    ),
...
export async function loadFlowpropertyCheckDataFixture(
  filePath: string,
): Promise<FlowpropertyCheckDataFixture> {
  const fixture = (await importJsonFile<FlowpropertyCheckDataFixture>(
    filePath,
  )) as FlowpropertyCheckDataFixture | null;

  if (!fixture || typeof fixture !== 'object' || Array.isArray(fixture)) {
    throw new Error(`Invalid flowproperty check-data fixture: ${filePath}`);
  }

  if (fixture.table && fixture.table !== 'flowproperties') {
    throw new Error(`Check-data fixture table must be "flowproperties": ${filePath}`);
  }
...
export function getFixtureOrderedJson(fixture: FlowpropertyCheckDataFixture) {
...
export function buildFlowpropertyCheckDataRuntimeRecord(
  options: Pick<
    CheckDataCliOptions,
    | 'checkDataFile'
    | 'checkExpectedFile'
  ...
  result: Pick<
    ...
    | 'validation'
  > & { passed: boolean },
): FlowpropertyCheckDataRuntimeRecord {
...
export async function runFlowpropertyCheckDataSmoke(
  options: CheckDataCliOptions,
  dependencies: FlowpropertyCheckDataDependencies = {},
): Promise<FlowpropertyCheckDataResult> {
  ...
  const checkFixture = await loadFlowpropertyCheckDataFixture(options.checkDataFile);
  const rawCheckOrderedJson = getFixtureOrderedJson(checkFixture);

  if (!rawCheckOrderedJson) {
    throw new Error(`Fixture ${options.checkDataFile} does not contain ordered flowproperty JSON.`);
  }
  ...
  try {
    ...
    const preparedCheckJsonOrdered = prepareFlowpropertyJsonOrderedForRuntime(rawCheckOrderedJson, {
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildFlowpropertyCheckDataRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        createStep,
        frontendProbe,
