...
export const DEFAULT_CONTACT_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/contacts/001_create.json';
export const DEFAULT_CONTACT_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/contacts/001_create.md';
...
export const DEFAULT_CONTACT_ROLE = 'user';
export const DEFAULT_CONTACT_CONTRIBUTE_TEAM_ROLE = 'team-member';
export const TEST_USERS_JSON_ENV = SHARED_TEST_USERS_JSON_ENV;
export const RUNTIME_CONTACT_ID_PLACEHOLDER = '__RUNTIME_CONTACT_ID__';

const CONTACT_PERMANENT_URI_BASE = 'https://lcdn.tiangong.earth/datasetdetail/contact.xhtml';
...
export type ContactCreateFixture = {
  id: string;
...
export type RuntimeFixture = {
  fixture: ContactCreateFixture;
  sourceFixtureId: string;
  runtimeId: string;
  version: string;
};

export type PreparedContactJsonOrdered = {
  jsonOrdered: Record<string, any>;
  version: string;
};
...
Usage:
  pnpm test:workflows --contacts:create --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --contacts:create --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna --no-keep-data
...
Flags:
  ...
  --data-file <path>               Defaults to tests/data-workflows/fixtures/data/contacts/001_create.json
  ...
  --keep-data                      Keep the created contact after validation (default)
  --keep-created                   Alias of --keep-data
  --no-keep-data                   Delete the created contact after validation
...
export function parseCliArgs(argv: string[], cwd = process.cwd()): CliOptions {
  ...
  const options: CliOptions = {
    dataFile: path.resolve(cwd, DEFAULT_CONTACT_FIXTURE_PATH),
    expectedFile: path.resolve(cwd, DEFAULT_CONTACT_EXPECTED_PATH),
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_CONTACT_ROLE,
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_CONTACT_FIXTURE_PATH),
      cwd,
    ),
...
export async function loadContactFixture(filePath: string): Promise<ContactCreateFixture> {
  const fixture = await loadJsonFile<ContactCreateFixture>(filePath);

  if (!fixture || typeof fixture !== 'object') {
    throw new Error(`Invalid contact fixture: ${filePath}`);
  }
  ...
  if (fixture.table !== 'contacts') {
    throw new Error(`Fixture table must be "contacts": ${filePath}`);
  }
...
export function prepareRuntimeFixture(
  ...
  const preparedJsonOrdered = prepareContactJsonOrderedForRuntime(clonedFixture.jsonOrdered, {
    runtimeId,
  });
...
export function prepareContactJsonOrderedForRuntime(
  jsonOrdered: Record<string, any>,
  options: {
    runtimeId: string;
    version?: string;
  },
): PreparedContactJsonOrdered {
  const clonedJsonOrdered = replaceRuntimeContactIdPlaceholder(
    deepClone(jsonOrdered),
    options.runtimeId,
  );
  setNestedValue(clonedJsonOrdered, CONTACT_UUID_PATH, options.runtimeId);

  const version = options.version ?? getContactVersion(clonedJsonOrdered);
  if (!version) {
    throw new Error('Unable to resolve contact dataset version from jsonOrdered.');
  }

  setNestedValue(clonedJsonOrdered, CONTACT_VERSION_PATH, version);
  setNestedValue(
    clonedJsonOrdered,
    CONTACT_PERMANENT_URI_PATH,
...
export async function computeContactRuleVerification(
  jsonOrdered: Record<string, any>,
): Promise<boolean> {
  const sdkValidation = createTidasContact(jsonOrdered).validateEnhanced();
...
export async function runContactCreateSmoke(
  options: CliOptions,
  ...
  let submittedRuleVerification = await computeContactRuleVerification(
  ...
  submittedRuleVerification = await computeContactRuleVerification(
    runtimeFixture.fixture.jsonOrdered,
  );
  ...
  if (recordResult.error || !recordResult.data) {
    throw new Error(
      `Failed to query created contact: ${recordResult.error?.message ?? 'record not found'}`,
    );
  }
...
function getContactVersion(jsonOrdered: Record<string, any>) {
  const version = getNestedValue<string>(jsonOrdered, CONTACT_VERSION_PATH);
...
export function buildPermanentDataSetUri(id: string, version: string) {
  return `${CONTACT_PERMANENT_URI_BASE}?uuid=${id}&version=${version}`;
}
...
function replaceRuntimeContactIdPlaceholder<T>(value: T, runtimeId: string): T {
  if (typeof value === 'string') {
    return value.split(RUNTIME_CONTACT_ID_PLACEHOLDER).join(runtimeId) as T;
  }

  if (Array.isArray(value)) {
    return value.map((item) => replaceRuntimeContactIdPlaceholder(item, runtimeId)) as T;
  }
  ...
  return Object.entries(value as Record<string, unknown>).reduce<Record<string, unknown>>(
    (accumulator, [key, nestedValue]) => {
      accumulator[key] = replaceRuntimeContactIdPlaceholder(nestedValue, runtimeId);
      return accumulator;
    },
    {},
  ) as T;
