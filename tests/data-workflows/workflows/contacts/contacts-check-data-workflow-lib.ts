...
export const DEFAULT_CONTACT_CHECK_DATA_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/contacts/002_check_data_success.json';
export const DEFAULT_CONTACT_CHECK_DATA_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/contacts/002_check_data_success.md';
...
export type ContactCheckDataFixture = {
...
export type ContactCheckDataResult = {
...
export type ContactCheckDataRuntimeRecord = {
  check: {
    expectedFile: string;
    expectationResults: ExpectationResult[];
    fixtureFile: string;
...
export type ContactCheckDataDependencies = {
...
Usage:
  pnpm test:workflows --contacts:check-data --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --contacts:check-data --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one contact from tests/data-workflows/fixtures/data/contacts/001_create.json
  2. Update the same contact with tests/data-workflows/fixtures/data/contacts/002_check_data_success.json
...
Flags:
  ...
  --check-data-file <path>         Defaults to tests/data-workflows/fixtures/data/contacts/002_check_data_success.json
...
export function parseCheckDataCliArgs(argv: string[], cwd = process.cwd()): CheckDataCliOptions {
  ...
  const options: CheckDataCliOptions = {
    checkDataFile: path.resolve(cwd, DEFAULT_CONTACT_CHECK_DATA_FIXTURE_PATH),
    checkExpectedFile: path.resolve(cwd, DEFAULT_CONTACT_CHECK_DATA_EXPECTED_PATH),
    ...
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_CONTACT_CHECK_DATA_FIXTURE_PATH),
      cwd,
    ),
...
export async function loadContactCheckDataFixture(
  filePath: string,
): Promise<ContactCheckDataFixture> {
  const fixture = (await importJsonFile<ContactCheckDataFixture>(
    filePath,
  )) as ContactCheckDataFixture | null;

  if (!fixture || typeof fixture !== 'object' || Array.isArray(fixture)) {
    throw new Error(`Invalid contact check-data fixture: ${filePath}`);
  }

  if (fixture.table && fixture.table !== 'contacts') {
    throw new Error(`Check-data fixture table must be "contacts": ${filePath}`);
  }
...
export function getFixtureOrderedJson(fixture: ContactCheckDataFixture) {
...
export function buildContactCheckDataRuntimeRecord(
  options: Pick<
    CheckDataCliOptions,
    | 'checkDataFile'
    | 'checkExpectedFile'
...
export async function runContactCheckDataSmoke(
  options: CheckDataCliOptions,
  dependencies: ContactCheckDataDependencies = {},
): Promise<ContactCheckDataResult> {
  ...
  const checkFixture = await loadContactCheckDataFixture(options.checkDataFile);
  const rawCheckOrderedJson = getFixtureOrderedJson(checkFixture);

  if (!rawCheckOrderedJson) {
    throw new Error(`Fixture ${options.checkDataFile} does not contain ordered contact JSON.`);
  }
  ...
  try {
    ...
    const preparedCheckJsonOrdered = prepareContactJsonOrderedForRuntime(rawCheckOrderedJson, {
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildContactCheckDataRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        createStep,
        frontendProbe,
