...
import {
  DEFAULT_CONTACT_CONTRIBUTE_TEAM_ROLE,
  DEFAULT_USERS_PATH,
...
export const DEFAULT_CONTACT_CREATE_CONTRIBUTE_TEAM_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/contacts/005_create_contribute_team.json';
export const DEFAULT_CONTACT_CREATE_CONTRIBUTE_TEAM_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/contacts/005_create_contribute_team.md';
export const DEFAULT_CONTACT_CREATE_CONTRIBUTE_TEAM_CREATE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/contacts/005_create_contribute_team_create.md';
...
export type ContactCreateContributeTeamCliOptions = {
...
export type ContactCreateContributeTeamResult = {
...
export type ContactCreateContributeTeamRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type ContactCreateContributeTeamDependencies = {
...
Usage:
  pnpm test:workflows --contacts:create-contribute-team --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --contacts:create-contribute-team --role user --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna
...
Workflow:
  ...
  3. Contribute that contact through the real contributeSource('contacts', id, version) flow
...
Flags:
  ...
  --create-data-file <path>        Defaults to tests/data-workflows/fixtures/data/contacts/005_create_contribute_team.json
  ...
  --keep-data                      Keep the contributed contact after validation (default)
...
export function parseCreateContributeTeamCliArgs(
  argv: string[],
  cwd = process.cwd(),
): ContactCreateContributeTeamCliOptions {
  let runtimeRecordFileExplicit = false;
  const defaultFollowUpExpectedFile = path.resolve(
    cwd,
    DEFAULT_CONTACT_CREATE_CONTRIBUTE_TEAM_EXPECTED_PATH,
  );
  const options: ContactCreateContributeTeamCliOptions = {
    createDataFile: path.resolve(cwd, DEFAULT_CONTACT_CREATE_CONTRIBUTE_TEAM_FIXTURE_PATH),
    createExpectedFile: path.resolve(
      cwd,
      DEFAULT_CONTACT_CREATE_CONTRIBUTE_TEAM_CREATE_EXPECTED_PATH,
    ),
    followUpExpectedFile: defaultFollowUpExpectedFile,
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_CONTACT_CONTRIBUTE_TEAM_ROLE,
...
export function buildContactCreateContributeTeamRuntimeRecord(
  options: Pick<
    ContactCreateContributeTeamCliOptions,
    'createDataFile' | 'createExpectedFile' | 'followUpExpectedFile' | 'frontendUrl' | 'keepData'
  >,
  result: Pick<
    ContactCreateContributeTeamResult,
    | 'cleanupAttempted'
    | 'cleanupPassed'
    | 'contribute'
    | 'contributeStep'
    ...
    | 'supabaseTarget'
  > & { passed: boolean },
): ContactCreateContributeTeamRuntimeRecord {
...
export async function runContactCreateContributeTeamSmoke(
  options: ContactCreateContributeTeamCliOptions,
  dependencies: ContactCreateContributeTeamDependencies = {},
): Promise<ContactCreateContributeTeamResult> {
  ...
  try {
    ...
    if (contributeError) {
      throw new Error(`Contribute contact failed: ${extractErrorMessage(contributeError)}`);
    }

    const contributedRecord = await queryContactRecord(
      modules.supabase,
      createdRecord.id,
      createdRecord.version,
    );
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildContactCreateContributeTeamRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        contribute,
        contributeStep,
