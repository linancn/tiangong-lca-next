...
import {
  CONTACT_EDIT_DATA_WORKFLOW_HELP,
  parseEditCliArgs,
  runContactEditSmoke,
} from './contacts-edit-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(CONTACT_EDIT_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runContactEditSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Contact edit data workflow');
  ...
  if (!result.passed) {
    throw new Error('Contact edit data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Contact edit data workflow passed.');
  }
}
