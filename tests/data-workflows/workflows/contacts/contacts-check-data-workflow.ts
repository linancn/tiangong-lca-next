...
import {
  CONTACT_CHECK_DATA_WORKFLOW_HELP,
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(CONTACT_CHECK_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runContactCheckDataSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Contact check-data data workflow');
  ...
  if (!result.passed) {
    throw new Error('Contact check-data data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Contact check-data data workflow passed.');
  }
}
