...
export const DEFAULT_CONTACT_CREATE_VERSION_UPDATE_REFERENCE_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/contacts/004_create_version_update_reference.json';
export const DEFAULT_CONTACT_CREATE_VERSION_UPDATE_REFERENCE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/contacts/004_create_version_update_reference.md';
...
type LoadedWorkflowModules = {
  contactsUtil: {
    ...
    genContactJsonOrdered: (id: string, data: any) => Record<string, any>;
  };
  generalApi: {
...
export type ContactCreateVersionUpdateReferenceCliOptions = {
  dataFile: string;
  expectedFile: string;
  frontendUrl?: string;
  generateId: boolean;
...
export type ContactCreateVersionUpdateReferenceResult = {
...
type StepRuntimeRecord = {
  expectationResults: ExpectationResult[];
  persistedRecord: Omit<ContactWorkflowRecord, 'json_ordered'>;
  submittedRuleVerification: boolean;
};

export type ContactCreateVersionUpdateReferenceRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type ContactCreateVersionUpdateReferenceDependencies = {
...
Usage:
  pnpm test:workflows --contacts:create-version-update-reference --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --contacts:create-version-update-reference --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one contact from tests/data-workflows/fixtures/data/contacts/004_create_version_update_reference.json
  2. Create a new version on the same contact id
...
Flags:
  ...
  --data-file <path>               Defaults to tests/data-workflows/fixtures/data/contacts/004_create_version_update_reference.json
...
export function parseCreateVersionUpdateReferenceCliArgs(
  argv: string[],
  cwd = process.cwd(),
): ContactCreateVersionUpdateReferenceCliOptions {
  let runtimeRecordFileExplicit = false;
  const options: ContactCreateVersionUpdateReferenceCliOptions = {
    dataFile: path.resolve(cwd, DEFAULT_CONTACT_CREATE_VERSION_UPDATE_REFERENCE_FIXTURE_PATH),
    expectedFile: path.resolve(cwd, DEFAULT_CONTACT_CREATE_VERSION_UPDATE_REFERENCE_EXPECTED_PATH),
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_CONTACT_ROLE,
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_CONTACT_CREATE_VERSION_UPDATE_REFERENCE_FIXTURE_PATH),
      cwd,
    ),
...
export function normalizeJsonForCreateVersionComparison(jsonOrdered: Record<string, unknown>) {
  ...
  deleteNestedValue(clonedJson, CONTACT_VERSION_PATH);
...
export function buildContactCreateVersionUpdateReferenceRuntimeRecord(
  options: Pick<
    ContactCreateVersionUpdateReferenceCliOptions,
    'dataFile' | 'expectedFile' | 'frontendUrl' | 'keepData'
  >,
  result: Pick<
    ContactCreateVersionUpdateReferenceResult,
    | 'cleanupAttempted'
    | 'cleanupPassed'
    | 'createStep'
    | 'createVersionStep'
    ...
    | 'versionCreation'
  > & { passed: boolean },
): ContactCreateVersionUpdateReferenceRuntimeRecord {
...
export async function runContactCreateVersionUpdateReferenceSmoke(
  options: ContactCreateVersionUpdateReferenceCliOptions,
  dependencies: ContactCreateVersionUpdateReferenceDependencies = {},
): Promise<ContactCreateVersionUpdateReferenceResult> {
  const supabaseTarget = normalizeSupabaseTarget(options);
  const runtimeFixture = prepareRuntimeFixture(await loadContactFixture(options.dataFile), {
  ...
  try {
    ...
    const createVersionFormData = modules.contactsUtil.genContactFromData(
      (createdRecord.json_ordered as Record<string, any>)?.contactDataSet ?? {},
    );

    if (!createVersionFormData) {
      throw new Error('Failed to derive contact form data from the created record.');
    }
    ...
    const createVersionJsonOrdered = prepareContactJsonOrderedForRuntime(
      modules.contactsUtil.genContactJsonOrdered(runtimeFixture.runtimeId, createVersionFormData),
      {
    ...
    if (createVersionResult.error) {
      throw new Error(`Create contact version failed: ${createVersionResult.error.message}.`);
    }

    const createdVersionRecord = await queryContactRecord(
      modules.supabase,
      runtimeFixture.runtimeId,
      createVersion,
    );
    ...
    const updateReferenceFormData = modules.contactsUtil.genContactFromData(
      (createdVersionRecord.json_ordered as Record<string, any>)?.contactDataSet ?? {},
    );

    if (!updateReferenceFormData) {
      throw new Error('Failed to derive contact form data from the created version record.');
    }
    ...
    const updatedJsonOrdered = prepareContactJsonOrderedForRuntime(
      modules.contactsUtil.genContactJsonOrdered(
        runtimeFixture.runtimeId,
    ...
    if (updateReferenceResult.error) {
      throw new Error(`Update contact reference failed: ${updateReferenceResult.error.message}.`);
    }

    const updatedRecord = await queryContactRecord(
      modules.supabase,
      runtimeFixture.runtimeId,
      createdVersionRecord.version,
    );
    ...
    const actuals: WorkflowActuals = {
      ...
      'create.ownerRefUriMatchesRuntimeId':
        createOwnerReference.uri === buildContactReferenceUri(runtimeFixture.runtimeId),
      ...
      'createVersion.ownerRefUriMatchesRuntimeId':
        createVersionOwnerReference.uri === buildContactReferenceUri(runtimeFixture.runtimeId),
      'createVersion.ownerRefVersionStillEqualsCreateVersion':
      ...
      'updateReference.ownerRefUriMatchesRuntimeId':
        updatedOwnerReference.uri === buildContactReferenceUri(runtimeFixture.runtimeId),
    ...
    if (!options.keepData) {
      ...
      cleanupPassed = await cleanupCreatedContactVersions(
        modules.supabase,
        accessToken,
        runtimeFixture.runtimeId,
        createdVersions,
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildContactCreateVersionUpdateReferenceRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        createStep,
        createVersionStep,
...
async function loadWorkflowModules(supabaseTarget: SupabaseTarget): Promise<LoadedWorkflowModules> {
  ...
  return {
    contactsUtil: {
      ...
      genContactJsonOrdered: contactsUtilExports.genContactJsonOrdered,
    },
    generalApi: {
...
function buildContactReferenceUri(id: string) {
  ...
  return (
    newRefs.find(
      (reference) =>
        ...
        reference.type === 'contact data set' &&
...
async function cleanupCreatedContactVersions(
  supabase: SupabaseClient,
  accessToken: string,
  runtimeId: string,
  createdVersions: string[],
