...
import {
  CONTACT_CREATE_CONTRIBUTE_TEAM_DATA_WORKFLOW_HELP,
  parseCreateContributeTeamCliArgs,
  runContactCreateContributeTeamSmoke,
} from './contacts-create-contribute-team-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(CONTACT_CREATE_CONTRIBUTE_TEAM_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runContactCreateContributeTeamSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Contact create-contribute-team data workflow');
    ...
    console.log(`Runtime contact ID: ${result.runtimeFixture.runtimeId}`);
    console.log(`Runtime contact version: ${result.runtimeFixture.version}`);
  ...
  if (!result.passed) {
    throw new Error('Contact create-contribute-team data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Contact create-contribute-team data workflow passed.');
  }
}
