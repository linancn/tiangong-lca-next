...
} from './contacts-check-data-workflow-lib';
import {
...
export const DEFAULT_CONTACT_EDIT_FAILURE_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/contacts/003_edit_data_validate_false.json';
export const DEFAULT_CONTACT_EDIT_FAILURE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/contacts/003_edit_data_validate_false.md';
...
export type ContactEditCliOptions = {
...
export type ContactEditRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type ContactEditDependencies = {
...
Usage:
  pnpm test:workflows --contacts:edit --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --contacts:edit --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one contact from tests/data-workflows/fixtures/data/contacts/001_create.json
  2. Edit the same contact into a validation-passing record from tests/data-workflows/fixtures/data/contacts/002_check_data_success.json
  3. Edit the same contact into a validation-failing record from tests/data-workflows/fixtures/data/contacts/003_edit_data_validate_false.json
...
Flags:
  ...
  --create-data-file <path>        Defaults to tests/data-workflows/fixtures/data/contacts/001_create.json
  --success-data-file <path>       Defaults to tests/data-workflows/fixtures/data/contacts/002_check_data_success.json
  --edit-data-file <path>          Defaults to tests/data-workflows/fixtures/data/contacts/003_edit_data_validate_false.json
  ...
  --keep-data                      Keep the workflow contact after validation (default)
  --keep-created                   Alias of --keep-data
  --no-keep-data                   Delete the workflow contact after validation
...
export function parseEditCliArgs(argv: string[], cwd = process.cwd()): ContactEditCliOptions {
  let runtimeRecordFileExplicit = false;
  const options: ContactEditCliOptions = {
    createDataFile: path.resolve(cwd, DEFAULT_CONTACT_FIXTURE_PATH),
    createExpectedFile: path.resolve(cwd, DEFAULT_CONTACT_EXPECTED_PATH),
    editDataFile: path.resolve(cwd, DEFAULT_CONTACT_EDIT_FAILURE_FIXTURE_PATH),
    editExpectedFile: path.resolve(cwd, DEFAULT_CONTACT_EDIT_FAILURE_EXPECTED_PATH),
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_CONTACT_ROLE,
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_CONTACT_EDIT_FAILURE_FIXTURE_PATH),
      cwd,
    ),
    successDataFile: path.resolve(cwd, DEFAULT_CONTACT_CHECK_DATA_FIXTURE_PATH),
    successExpectedFile: path.resolve(cwd, DEFAULT_CONTACT_CHECK_DATA_EXPECTED_PATH),
...
export function buildContactEditRuntimeRecord(
  options: Pick<
    ContactEditCliOptions,
    | 'createDataFile'
    | 'createExpectedFile'
    | 'editDataFile'
    | 'editExpectedFile'
...
export async function runContactEditSmoke(
  options: ContactEditCliOptions,
  dependencies: ContactEditDependencies = {},
  ...
  const successFixture = await loadContactCheckDataFixture(options.successDataFile);
  const editFixture = await loadContactCheckDataFixture(options.editDataFile);
  ...
  if (!rawSuccessJsonOrdered) {
    throw new Error(`Fixture ${options.successDataFile} does not contain ordered contact JSON.`);
  }

  if (!rawEditJsonOrdered) {
    throw new Error(`Fixture ${options.editDataFile} does not contain ordered contact JSON.`);
  }
  ...
  try {
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildContactEditRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        createStep,
...
async function runEditStage(input: {
  ...
  const preparedJsonOrdered = prepareContactJsonOrderedForRuntime(input.rawJsonOrdered, {
  ...
  if (updateResult.error) {
    throw new Error(`Update contact failed: ${updateResult.error.message}.`);
  }

  const record = await queryContactRecord(input.modules.supabase, input.runtimeId, input.version);
