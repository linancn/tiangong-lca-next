...
import {
  CONTACT_CREATE_VERSION_UPDATE_REFERENCE_DATA_WORKFLOW_HELP,
  parseCreateVersionUpdateReferenceCliArgs,
  runContactCreateVersionUpdateReferenceSmoke,
} from './contacts-create-version-update-reference-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(CONTACT_CREATE_VERSION_UPDATE_REFERENCE_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runContactCreateVersionUpdateReferenceSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Contact create-version-update-reference data workflow');
  ...
  if (!result.passed) {
    throw new Error('Contact create-version-update-reference data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Contact create-version-update-reference data workflow passed.');
  }
}
