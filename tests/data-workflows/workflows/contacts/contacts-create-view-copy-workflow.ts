...
import {
  CONTACT_CREATE_VIEW_COPY_DATA_WORKFLOW_HELP,
  parseCreateViewCopyCliArgs,
  runContactCreateViewCopySmoke,
} from './contacts-create-view-copy-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(CONTACT_CREATE_VIEW_COPY_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runContactCreateViewCopySmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Contact create-view-copy data workflow');
    ...
    console.log(`Runtime source contact ID: ${result.runtimeFixture.runtimeId}`);
    console.log(`Copied contact ID: ${result.copy.copiedId}`);
    console.log(`Copied contact version: ${result.copy.copiedVersion}`);
    console.log(
  ...
  if (!result.passed) {
    throw new Error('Contact create-view-copy data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Contact create-view-copy data workflow passed.');
  }
}
