...
import {
  CONTACT_FULL_TEXT_SEARCH_DATA_WORKFLOW_HELP,
  parseContactFullTextSearchCliArgs,
  runContactFullTextSearchSmoke,
} from './contacts-full-text-search-workflow-lib';

export async function main(argv: string[] = process.argv.slice(2)) {
  const detailResultState = extractDetailResultFlag(argv);
  const options = parseContactFullTextSearchCliArgs(detailResultState.argv);

  if (options.help) {
    printDataWorkflowHelp(CONTACT_FULL_TEXT_SEARCH_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runContactFullTextSearchSmoke(options);
  ...
  if (!showDetailedResult) {
    printCompactSmokeSummary({
      ...
      dataType: 'contact full-text search',
      passed: result.passed,
      version: result.runtimeFixture.version,
    });
  } else {
    console.log('Contact full-text-search data workflow');
  ...
  if (!result.passed) {
    throw new Error('Contact full-text-search data workflow failed.');
  }
}
