...
import {
  TEAMS_EDIT_DATA_WORKFLOW_HELP,
  parseTeamsEditCliArgs,
  runTeamsEditSmoke,
} from './teams-edit-workflow-lib';

export async function main(argv: string[] = process.argv.slice(2)) {
  const detailResultState = extractDetailResultFlag(argv);
  const options = parseTeamsEditCliArgs(detailResultState.argv);

  if (options.help) {
    printDataWorkflowHelp(TEAMS_EDIT_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runTeamsEditSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Teams edit data workflow');
  ...
  if (!result.passed) {
    throw new Error('Teams edit data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Teams edit data workflow passed.');
  }
}
