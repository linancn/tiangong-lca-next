...
import {
  DEFAULT_PROCESS_CHECK_DATA_EXPECTED_PATH,
...
export const DEFAULT_PROCESS_EDIT_FAILURE_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/processes/003_edit_data_validate_false.json';
export const DEFAULT_PROCESS_EDIT_FAILURE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/processes/003_edit_data_validate_false.md';
...
export type ProcessEditCliOptions = {
...
export type ProcessEditRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type ProcessEditDependencies = {
...
Usage:
  pnpm test:workflows --processes:edit --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --processes:edit --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one process from tests/data-workflows/fixtures/data/processes/001_create.json
  2. Edit the same process into a validation-passing record from tests/data-workflows/fixtures/data/processes/002_check_data_success.json
  3. Edit the same process into a validation-failing record from tests/data-workflows/fixtures/data/processes/003_edit_data_validate_false.json
...
Flags:
  ...
  --success-data-file <path>       Defaults to tests/data-workflows/fixtures/data/processes/002_check_data_success.json
  --edit-data-file <path>          Defaults to tests/data-workflows/fixtures/data/processes/003_edit_data_validate_false.json
  ...
  --keep-data                      Keep the workflow process after validation (default)
  --keep-created                   Alias of --keep-data
  --no-keep-data                   Delete the workflow process after validation
...
export function parseEditCliArgs(argv: string[], cwd = process.cwd()): ProcessEditCliOptions {
  let runtimeRecordFileExplicit = false;
  const options: ProcessEditCliOptions = {
    ...
    editDataFile: path.resolve(cwd, DEFAULT_PROCESS_EDIT_FAILURE_FIXTURE_PATH),
    editExpectedFile: path.resolve(cwd, DEFAULT_PROCESS_EDIT_FAILURE_EXPECTED_PATH),
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_PROCESS_ROLE,
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_PROCESS_EDIT_FAILURE_FIXTURE_PATH),
      cwd,
    ),
    successDataFile: path.resolve(cwd, DEFAULT_PROCESS_CHECK_DATA_FIXTURE_PATH),
    successExpectedFile: path.resolve(cwd, DEFAULT_PROCESS_CHECK_DATA_EXPECTED_PATH),
...
export function buildProcessEditRuntimeRecord(
  options: Pick<
    ProcessEditCliOptions,
    | 'createDataFile'
    | 'createExpectedFile'
    | 'editDataFile'
    | 'editExpectedFile'
...
export async function runProcessEditSmoke(
  options: ProcessEditCliOptions,
  dependencies: ProcessEditDependencies = {},
): Promise<ProcessEditResult> {
  ...
  const successFixture = await loadProcessCheckDataFixture(options.successDataFile);
  const editFixture = await loadProcessCheckDataFixture(options.editDataFile);
  ...
  if (!rawSuccessJsonOrdered) {
    throw new Error(`Fixture ${options.successDataFile} does not contain ordered process JSON.`);
  }

  if (!rawEditJsonOrdered) {
    throw new Error(`Fixture ${options.editDataFile} does not contain ordered process JSON.`);
  }
  ...
  try {
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildProcessEditRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        createStep,
...
async function runEditStage(input: {
  ...
  const preparedJsonOrdered = prepareProcessJsonOrderedForRuntime(input.rawJsonOrdered, {
  ...
  const record = await queryProcessRecord(input.modules.supabase, input.runtimeId, input.version);
