...
import {
  DEFAULT_PROCESS_CONTRIBUTE_TEAM_ROLE,
  DEFAULT_USERS_PATH,
...
export const DEFAULT_PROCESS_CREATE_CONTRIBUTE_TEAM_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/processes/005_create_contribute_team.json';
export const DEFAULT_PROCESS_CREATE_CONTRIBUTE_TEAM_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/processes/005_create_contribute_team.md';
export const DEFAULT_PROCESS_CREATE_CONTRIBUTE_TEAM_CREATE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/processes/005_create_contribute_team_create.md';
...
export type ProcessCreateContributeTeamCliOptions = {
...
export type ProcessCreateContributeTeamResult = {
...
export type ProcessCreateContributeTeamRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type ProcessCreateContributeTeamDependencies = {
...
Usage:
  pnpm test:workflows --processes:create-contribute-team --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --processes:create-contribute-team --role user --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna
...
Workflow:
  ...
  3. Contribute that process through the real contributeSource('processes', id, version) process
...
Flags:
  ...
  --create-data-file <path>        Defaults to tests/data-workflows/fixtures/data/processes/005_create_contribute_team.json
  ...
  --keep-data                      Keep the contributed process after validation (default)
...
export function parseCreateContributeTeamCliArgs(
  argv: string[],
  cwd = process.cwd(),
): ProcessCreateContributeTeamCliOptions {
  let runtimeRecordFileExplicit = false;
  const defaultFollowUpExpectedFile = path.resolve(
    cwd,
    DEFAULT_PROCESS_CREATE_CONTRIBUTE_TEAM_EXPECTED_PATH,
  );
  const options: ProcessCreateContributeTeamCliOptions = {
    createDataFile: path.resolve(cwd, DEFAULT_PROCESS_CREATE_CONTRIBUTE_TEAM_FIXTURE_PATH),
    createExpectedFile: path.resolve(
      cwd,
      DEFAULT_PROCESS_CREATE_CONTRIBUTE_TEAM_CREATE_EXPECTED_PATH,
    ),
    followUpExpectedFile: defaultFollowUpExpectedFile,
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_PROCESS_CONTRIBUTE_TEAM_ROLE,
...
export function buildProcessCreateContributeTeamRuntimeRecord(
  options: Pick<
    ProcessCreateContributeTeamCliOptions,
    'createDataFile' | 'createExpectedFile' | 'followUpExpectedFile' | 'frontendUrl' | 'keepData'
  >,
  result: Pick<
    ProcessCreateContributeTeamResult,
    | 'cleanupAttempted'
    | 'cleanupPassed'
    | 'contribute'
    | 'contributeStep'
    ...
    | 'supabaseTarget'
  > & { passed: boolean },
): ProcessCreateContributeTeamRuntimeRecord {
...
export async function runProcessCreateContributeTeamSmoke(
  options: ProcessCreateContributeTeamCliOptions,
  dependencies: ProcessCreateContributeTeamDependencies = {},
): Promise<ProcessCreateContributeTeamResult> {
  ...
  try {
    ...
    if (contributeError) {
      throw new Error(`Contribute process failed: ${extractErrorMessage(contributeError)}`);
    }

    const contributedRecord = await queryProcessRecord(
      modules.supabase,
      createdRecord.id,
      createdRecord.version,
    );
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildProcessCreateContributeTeamRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        contribute,
        contributeStep,
