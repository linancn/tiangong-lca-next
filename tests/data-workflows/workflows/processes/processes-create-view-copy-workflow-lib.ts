...
export const DEFAULT_PROCESS_CREATE_VIEW_COPY_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/processes/001_create_view_copy.json';
export const DEFAULT_PROCESS_CREATE_VIEW_COPY_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/processes/001_create_view_copy.md';
...
export type ProcessCreateViewCopyCliOptions = {
...
export type ProcessCreateViewCopyResult = {
...
type StepRuntimeRecord = {
  expectationResults: ExpectationResult[];
  persistedRecord: Omit<ProcessWorkflowRecord, 'json' | 'json_ordered'>;
  submittedRuleVerification: boolean | null;
};

export type ProcessCreateViewCopyRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type ProcessCreateViewCopyDependencies = {
...
Usage:
  pnpm test:workflows --processes:create-view-copy --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --processes:create-view-copy --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one process from tests/data-workflows/fixtures/data/processes/001_create_view_copy.json
  2. View the created process through getProcessDetail
  3. Copy that process through the real createProcess process
  4. View the copied process through getProcessDetail
...
Flags:
  ...
  --create-data-file <path>        Defaults to tests/data-workflows/fixtures/data/processes/001_create_view_copy.json
  ...
  --keep-data                      Keep both source and copied processes after validation (default)
  --keep-created                   Alias of --keep-data
  --no-keep-data                   Delete the workflow processes after validation
...
export function parseCreateViewCopyCliArgs(
  argv: string[],
  cwd = process.cwd(),
): ProcessCreateViewCopyCliOptions {
  let runtimeRecordFileExplicit = false;
  const defaultFollowUpExpectedFile = path.resolve(
    cwd,
    DEFAULT_PROCESS_CREATE_VIEW_COPY_EXPECTED_PATH,
  );
  const options: ProcessCreateViewCopyCliOptions = {
    createDataFile: path.resolve(cwd, DEFAULT_PROCESS_CREATE_VIEW_COPY_FIXTURE_PATH),
...
export function normalizeJsonForCopyComparison(jsonOrdered: Record<string, unknown>) {
  ...
  deleteNestedValue(clonedJson, PROCESS_UUID_PATH);
...
export function buildProcessCreateViewCopyRuntimeRecord(
  options: Pick<
    ProcessCreateViewCopyCliOptions,
    'createDataFile' | 'createExpectedFile' | 'followUpExpectedFile' | 'frontendUrl' | 'keepData'
  >,
  result: Pick<
    ProcessCreateViewCopyResult,
    | 'cleanupAttempted'
    | 'cleanupPassed'
    | 'copy'
    | 'copyStep'
    ...
    | 'viewCopyStep'
  > & { passed: boolean },
): ProcessCreateViewCopyRuntimeRecord {
  const {
    json: _createJson,
...
export async function runProcessCreateViewCopySmoke(
  options: ProcessCreateViewCopyCliOptions,
  dependencies: ProcessCreateViewCopyDependencies = {},
): Promise<ProcessCreateViewCopyResult> {
  const supabaseTarget = normalizeSupabaseTarget(options);
  const runtimeFixture = prepareRuntimeFixture(await loadProcessFixture(options.createDataFile), {
  ...
  try {
    const submittedCreateRuleVerification = await (
      dependencies.computeCreateRuleVerificationFn ?? computeProcessRuleVerification
    ...
    const sourceDetailResponse = await modules.processesApi.getProcessDetail(
      createdRecord.id,
      createdRecord.version,
    );

    if (!sourceDetailResponse?.success || !sourceDetailResponse.data?.json) {
      throw new Error('View source process failed: getProcessDetail returned no data.');
    }

    const sourceFormData = modules.processesUtil.genProcessFromData(
      (sourceDetailResponse.data.json as Record<string, any>)?.processDataSet ?? {},
    );

    if (!sourceFormData) {
      throw new Error('Failed to derive process form data from the viewed source detail.');
    }

    const copiedId = (dependencies.generateIdFn ?? randomUUID)();
    if (copiedId === createdRecord.id) {
      throw new Error('Copy process failed: generated id matches the source process id.');
    }

    const copyCreateResult = await modules.processesApi.createProcess(copiedId, sourceFormData);
    if (copyCreateResult.error) {
      throw new Error(`Copy process failed: ${copyCreateResult.error.message ?? 'unknown error'}`);
    }
    ...
    const copiedRecord = await queryProcessRecord(modules.supabase, copiedId, copiedVersion);
    createdRecords.push({
      id: copiedRecord.id,
      version: copiedRecord.version,
    });

    const copiedDetailResponse = await modules.processesApi.getProcessDetail(
      copiedRecord.id,
      copiedRecord.version,
    );

    if (!copiedDetailResponse?.success || !copiedDetailResponse.data?.json) {
      throw new Error('View copied process failed: getProcessDetail returned no data.');
    }
    ...
    const actuals: WorkflowActuals = {
      ...
      'copy.uuidMatchesCopiedId':
        getNestedValue(copiedRecord.json_ordered as Record<string, any>, PROCESS_UUID_PATH) ===
        copiedId,
      'copy.permanentUriMatchesCopiedIdAndVersion':
    ...
    if (!options.keepData) {
      ...
      cleanupPassed = await cleanupCreatedProcesses(modules.supabase, accessToken, createdRecords);
    }
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildProcessCreateViewCopyRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        copy,
        copyStep,
...
async function loadWorkflowModules(supabaseTarget: SupabaseTarget): Promise<LoadedWorkflowModules> {
  ...
  const processesApiModule = await import('../../../../src/services/processes/api');
  ...
  const processesApiExports =
    extractModuleExports<LoadedWorkflowModules['processesApi']>(processesApiModule);
  const processesUtilExports =
    extractModuleExports<LoadedWorkflowModules['processesUtil']>(processesUtilModule);

  return {
    processesApi: {
      createProcess: processesApiExports.createProcess,
      getProcessDetail: processesApiExports.getProcessDetail,
    },
    processesUtil: {
...
async function cleanupCreatedProcesses(
  supabase: SupabaseClient,
  accessToken: string,
