...
export const DEFAULT_PROCESS_CHECK_DATA_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/processes/002_check_data_success.json';
export const DEFAULT_PROCESS_CHECK_DATA_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/processes/002_check_data_success.md';
...
export type ProcessCheckDataFixture = {
...
export type ProcessCheckDataResult = {
...
export type ProcessCheckDataRuntimeRecord = {
  check: {
    expectedFile: string;
    expectationResults: ExpectationResult[];
    fixtureFile: string;
...
export type ProcessCheckDataDependencies = {
...
Usage:
  pnpm test:workflows --processes:check-data --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --processes:check-data --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one process from tests/data-workflows/fixtures/data/processes/001_create.json
  2. Update the same process with tests/data-workflows/fixtures/data/processes/002_check_data_success.json
...
Flags:
  ...
  --check-data-file <path>         Defaults to tests/data-workflows/fixtures/data/processes/002_check_data_success.json
...
export function parseCheckDataCliArgs(argv: string[], cwd = process.cwd()): CheckDataCliOptions {
  ...
  const options: CheckDataCliOptions = {
    checkDataFile: path.resolve(cwd, DEFAULT_PROCESS_CHECK_DATA_FIXTURE_PATH),
    checkExpectedFile: path.resolve(cwd, DEFAULT_PROCESS_CHECK_DATA_EXPECTED_PATH),
    ...
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_PROCESS_CHECK_DATA_FIXTURE_PATH),
      cwd,
    ),
...
export async function loadProcessCheckDataFixture(
  filePath: string,
): Promise<ProcessCheckDataFixture> {
  const fixture = (await importJsonFile<ProcessCheckDataFixture>(
    filePath,
  )) as ProcessCheckDataFixture | null;

  if (!fixture || typeof fixture !== 'object' || Array.isArray(fixture)) {
    throw new Error(`Invalid process check-data fixture: ${filePath}`);
  }

  if (fixture.table && fixture.table !== 'processes') {
    throw new Error(`Check-data fixture table must be "processes": ${filePath}`);
  }
...
export function getFixtureOrderedJson(fixture: ProcessCheckDataFixture) {
...
export function buildProcessCheckDataRuntimeRecord(
  options: Pick<
    CheckDataCliOptions,
    | 'checkDataFile'
    | 'checkExpectedFile'
...
export async function runProcessCheckDataSmoke(
  options: CheckDataCliOptions,
  dependencies: ProcessCheckDataDependencies = {},
): Promise<ProcessCheckDataResult> {
  ...
  const checkFixture = await loadProcessCheckDataFixture(options.checkDataFile);
  ...
  try {
    ...
    const preparedCheckJsonOrdered = prepareProcessJsonOrderedForRuntime(rawCheckOrderedJson, {
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildProcessCheckDataRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        createStep,
        frontendProbe,
