...
import {
  PROCESS_CHECK_DATA_WORKFLOW_HELP,
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(PROCESS_CHECK_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runProcessCheckDataSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Process check-data data workflow');
  ...
  if (!result.passed) {
    throw new Error('Process check-data data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Process check-data data workflow passed.');
  }
}
