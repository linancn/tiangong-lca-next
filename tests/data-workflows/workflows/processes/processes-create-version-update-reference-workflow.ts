...
import {
  PROCESS_CREATE_VERSION_UPDATE_REFERENCE_DATA_WORKFLOW_HELP,
  parseCreateVersionUpdateReferenceCliArgs,
  runProcessCreateVersionUpdateReferenceSmoke,
} from './processes-create-version-update-reference-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(PROCESS_CREATE_VERSION_UPDATE_REFERENCE_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runProcessCreateVersionUpdateReferenceSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Process create-version-update-reference data workflow');
  ...
  if (!result.passed) {
    throw new Error('Process create-version-update-reference data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Process create-version-update-reference data workflow passed.');
  }
}
