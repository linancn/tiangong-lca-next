...
export const DEFAULT_PROCESS_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/processes/001_create.json';
export const DEFAULT_PROCESS_EXPECTED_PATH =
...
export const DEFAULT_PROCESS_ROLE = 'user';
export const DEFAULT_PROCESS_CONTRIBUTE_TEAM_ROLE = 'team-member';
export const TEST_USERS_JSON_ENV = SHARED_TEST_USERS_JSON_ENV;
export const RUNTIME_PROCESS_ID_PLACEHOLDER = '__RUNTIME_PROCESS_ID__';

const PROCESS_PERMANENT_URI_BASE = 'https://lcdn.tiangong.earth/datasetdetail/process.xhtml';
...
export type ProcessCreateFixture = {
  id: string;
...
export type RuntimeFixture = {
  fixture: ProcessCreateFixture;
  sourceFixtureId: string;
  runtimeId: string;
  version: string;
};

export type PreparedProcessJsonOrdered = {
  jsonOrdered: Record<string, any>;
  version: string;
};
...
Usage:
  pnpm test:workflows --processes:create --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --processes:create --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna --no-keep-data
...
Flags:
  ...
  --data-file <path>               Defaults to tests/data-workflows/fixtures/data/processes/001_create.json
...
export function parseCliArgs(argv: string[], cwd = process.cwd()): CliOptions {
  ...
  const options: CliOptions = {
    dataFile: path.resolve(cwd, DEFAULT_PROCESS_FIXTURE_PATH),
    expectedFile: path.resolve(cwd, DEFAULT_PROCESS_EXPECTED_PATH),
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_PROCESS_ROLE,
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_PROCESS_FIXTURE_PATH),
      cwd,
    ),
...
export async function loadProcessFixture(filePath: string): Promise<ProcessCreateFixture> {
  const fixture = await loadJsonFile<ProcessCreateFixture>(filePath);

  if (!fixture || typeof fixture !== 'object') {
    throw new Error(`Invalid process fixture: ${filePath}`);
  }
  ...
  if (fixture.table !== 'processes') {
    throw new Error(`Fixture table must be "processes": ${filePath}`);
  }
...
export function prepareRuntimeFixture(
  ...
  const preparedJsonOrdered = prepareProcessJsonOrderedForRuntime(clonedFixture.jsonOrdered, {
    runtimeId,
  });
...
export function prepareProcessJsonOrderedForRuntime(
  jsonOrdered: Record<string, any>,
  options: {
    runtimeId: string;
    version?: string;
  },
): PreparedProcessJsonOrdered {
  const clonedJsonOrdered = replaceRuntimeProcessIdPlaceholder(
    deepClone(jsonOrdered),
    options.runtimeId,
  );
  setNestedValue(clonedJsonOrdered, PROCESS_UUID_PATH, options.runtimeId);

  const version = options.version ?? getProcessVersion(clonedJsonOrdered);
  if (!version) {
    throw new Error('Unable to resolve process dataset version from jsonOrdered.');
  }

  setNestedValue(clonedJsonOrdered, PROCESS_VERSION_PATH, version);
  setNestedValue(
    clonedJsonOrdered,
    PROCESS_PERMANENT_URI_PATH,
...
export async function computeProcessRuleVerification(
  jsonOrdered: Record<string, any>,
): Promise<boolean> {
  const sdkValidation = createTidasProcess(jsonOrdered).validateEnhanced();
...
export async function runProcessCreateSmoke(
  options: CliOptions,
  ...
  let submittedRuleVerification = await computeProcessRuleVerification(
  ...
  submittedRuleVerification = await computeProcessRuleVerification(
    runtimeFixture.fixture.jsonOrdered,
  );
  ...
  if (recordResult.error || !recordResult.data) {
    throw new Error(
      `Failed to query created process: ${recordResult.error?.message ?? 'record not found'}`,
    );
  }
...
function getProcessVersion(jsonOrdered: Record<string, any>) {
  const version = getNestedValue<string>(jsonOrdered, PROCESS_VERSION_PATH);
...
export function buildPermanentDataSetUri(id: string, version: string) {
  return `${PROCESS_PERMANENT_URI_BASE}?uuid=${id}&version=${version}`;
}
...
function replaceRuntimeProcessIdPlaceholder<T>(value: T, runtimeId: string): T {
  if (typeof value === 'string') {
    return value.split(RUNTIME_PROCESS_ID_PLACEHOLDER).join(runtimeId) as T;
  }

  if (Array.isArray(value)) {
    return value.map((item) => replaceRuntimeProcessIdPlaceholder(item, runtimeId)) as T;
  }
  ...
  return Object.entries(value as Record<string, unknown>).reduce<Record<string, unknown>>(
    (accumulator, [key, nestedValue]) => {
      accumulator[key] = replaceRuntimeProcessIdPlaceholder(nestedValue, runtimeId);
      return accumulator;
    },
    {},
  ) as T;
