...
import {
  PROCESS_CREATE_CONTRIBUTE_TEAM_DATA_WORKFLOW_HELP,
  parseCreateContributeTeamCliArgs,
  runProcessCreateContributeTeamSmoke,
} from './processes-create-contribute-team-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(PROCESS_CREATE_CONTRIBUTE_TEAM_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runProcessCreateContributeTeamSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Process create-contribute-team data workflow');
    ...
    console.log(`Runtime process ID: ${result.runtimeFixture.runtimeId}`);
    console.log(`Runtime process version: ${result.runtimeFixture.version}`);
  ...
  if (!result.passed) {
    throw new Error('Process create-contribute-team data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Process create-contribute-team data workflow passed.');
  }
}
