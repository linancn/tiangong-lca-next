...
import {
  PROCESS_EDIT_DATA_WORKFLOW_HELP,
  parseEditCliArgs,
  runProcessEditSmoke,
} from './processes-edit-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(PROCESS_EDIT_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runProcessEditSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Process edit data workflow');
  ...
  if (!result.passed) {
    throw new Error('Process edit data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Process edit data workflow passed.');
  }
}
