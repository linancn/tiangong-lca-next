...
import {
  SOURCE_CREATE_VERSION_UPDATE_REFERENCE_DATA_WORKFLOW_HELP,
  parseCreateVersionUpdateReferenceCliArgs,
  runSourceCreateVersionUpdateReferenceSmoke,
} from './sources-create-version-update-reference-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(SOURCE_CREATE_VERSION_UPDATE_REFERENCE_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runSourceCreateVersionUpdateReferenceSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Source create-version-update-reference data workflow');
    ...
    console.log(`Runtime source ID: ${result.runtimeFixture.runtimeId}`);
  ...
  if (!result.passed) {
    throw new Error('Source create-version-update-reference data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Source create-version-update-reference data workflow passed.');
  }
}
