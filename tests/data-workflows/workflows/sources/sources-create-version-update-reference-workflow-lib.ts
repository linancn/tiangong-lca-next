...
export const DEFAULT_SOURCE_CREATE_VERSION_UPDATE_REFERENCE_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/sources/004_create_version_update_reference.json';
export const DEFAULT_SOURCE_CREATE_VERSION_UPDATE_REFERENCE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/sources/004_create_version_update_reference.md';
...
type LoadedWorkflowModules = {
  sourcesUtil: {
    ...
    genSourceJsonOrdered: (id: string, data: any) => Record<string, any>;
  };
  generalApi: {
...
export type SourceCreateVersionUpdateReferenceCliOptions = {
  dataFile: string;
  expectedFile: string;
  frontendUrl?: string;
  generateId: boolean;
...
export type SourceCreateVersionUpdateReferenceResult = {
...
export type SourceCreateVersionUpdateReferenceRuntimeRecord = {
  cleanupAttempted: boolean;
  cleanupPassed: boolean;
...
export type SourceCreateVersionUpdateReferenceDependencies = {
...
Usage:
  pnpm test:workflows --sources:create-version-update-reference --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --sources:create-version-update-reference --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna

Workflow:
  1. Create one source from tests/data-workflows/fixtures/data/sources/004_create_version_update_reference.json
  2. Create a new version on the same source id
...
Flags:
  ...
  --data-file <path>               Defaults to tests/data-workflows/fixtures/data/sources/004_create_version_update_reference.json
...
const SOURCE_DATASET_FORMAT_REFERENCE_PATH = [
  'sourceDataSet',
  'administrativeInformation',
  'dataEntryBy',
...
export function parseCreateVersionUpdateReferenceCliArgs(
  argv: string[],
  cwd = process.cwd(),
): SourceCreateVersionUpdateReferenceCliOptions {
  let runtimeRecordFileExplicit = false;
  const options: SourceCreateVersionUpdateReferenceCliOptions = {
    dataFile: path.resolve(cwd, DEFAULT_SOURCE_CREATE_VERSION_UPDATE_REFERENCE_FIXTURE_PATH),
    expectedFile: path.resolve(cwd, DEFAULT_SOURCE_CREATE_VERSION_UPDATE_REFERENCE_EXPECTED_PATH),
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_SOURCE_ROLE,
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_SOURCE_CREATE_VERSION_UPDATE_REFERENCE_FIXTURE_PATH),
      cwd,
    ),
...
export function normalizeJsonForCreateVersionComparison(jsonOrdered: Record<string, unknown>) {
  ...
  deleteNestedValue(clonedJson, SOURCE_VERSION_PATH);
...
export function buildSourceCreateVersionUpdateReferenceRuntimeRecord(
  options: Pick<
    SourceCreateVersionUpdateReferenceCliOptions,
    'dataFile' | 'expectedFile' | 'frontendUrl' | 'keepData'
  >,
  result: Pick<
    SourceCreateVersionUpdateReferenceResult,
    | 'cleanupAttempted'
    | 'cleanupPassed'
    | 'createStep'
    | 'createVersionStep'
    ...
    | 'versionCreation'
  > & { passed: boolean },
): SourceCreateVersionUpdateReferenceRuntimeRecord {
...
export async function runSourceCreateVersionUpdateReferenceSmoke(
  options: SourceCreateVersionUpdateReferenceCliOptions,
  dependencies: SourceCreateVersionUpdateReferenceDependencies = {},
): Promise<SourceCreateVersionUpdateReferenceResult> {
  const supabaseTarget = normalizeSupabaseTarget(options);
  const runtimeFixture = prepareRuntimeFixture(await loadSourceFixture(options.dataFile), {
    generateId: options.generateId,
  });
  runtimeFixture.fixture.jsonOrdered = alignSourceDataSetFormatReferenceVersion(
  ...
  try {
    ...
    const createVersionFormData = modules.sourcesUtil.genSourceFromData(
      (createdRecord.json_ordered as Record<string, any>)?.sourceDataSet ?? {},
    );

    if (!createVersionFormData) {
      throw new Error('Failed to derive source form data from the created record.');
    }
    ...
    const createVersionJsonOrdered = prepareSourceJsonOrderedForRuntime(
      modules.sourcesUtil.genSourceJsonOrdered(runtimeFixture.runtimeId, createVersionFormData),
      {
    ...
    if (createVersionResult.error) {
      throw new Error(`Create source version failed: ${createVersionResult.error.message}.`);
    }

    const createdVersionRecord = await querySourceRecord(
      modules.supabase,
      runtimeFixture.runtimeId,
      createVersion,
    );
    ...
    const updateReferenceFormData = modules.sourcesUtil.genSourceFromData(
      (createdVersionRecord.json_ordered as Record<string, any>)?.sourceDataSet ?? {},
    );

    if (!updateReferenceFormData) {
      throw new Error('Failed to derive source form data from the created version record.');
    }
    ...
    const updatedJsonOrdered = prepareSourceJsonOrderedForRuntime(
      modules.sourcesUtil.genSourceJsonOrdered(runtimeFixture.runtimeId, updatedReferenceFormData),
      {
        runtimeId: runtimeFixture.runtimeId,
    ...
    if (updateReferenceResult.error) {
      throw new Error(`Update source reference failed: ${updateReferenceResult.error.message}.`);
    }
    ...
    const updatedShortName = normalizeLangTextValue(
      getNestedValue(updatedRecord.json_ordered as Record<string, any>, [
        'sourceDataSet',
        'sourceInformation',
        'dataSetInformation',
        'common:shortName',
    ...
    const actuals: WorkflowActuals = {
      ...
      'create.ownerRefIdMatchesRuntimeId':
        createOwnerReference.refObjectId === runtimeFixture.runtimeId,
      'create.ownerRefUriMatchesRuntimeId':
        createOwnerReference.uri === buildSourceReferenceUri(runtimeFixture.runtimeId),
      ...
      'createVersion.ownerRefUriMatchesRuntimeId':
        createVersionOwnerReference.uri === buildSourceReferenceUri(runtimeFixture.runtimeId),
      'createVersion.ownerRefVersionStillEqualsCreateVersion':
      ...
      'updateReference.ownerRefIdMatchesRuntimeId':
        updatedOwnerReference.refObjectId === runtimeFixture.runtimeId,
      'updateReference.ownerRefUriMatchesRuntimeId':
        updatedOwnerReference.uri === buildSourceReferenceUri(runtimeFixture.runtimeId),
      'updateReference.ownerRefVersionEqualsCreatedVersion': updatedOwnerReference.version ?? null,
      'updateReference.ownerRefShortDescriptionMatchesCurrentShortName':
        normalizedLangTextEntriesEqual(updatedOwnerReference.shortDescription, updatedShortName),
      'updateReference.savedVersionUnchanged':
    ...
    if (!options.keepData) {
      ...
      cleanupPassed = await cleanupCreatedSourceVersions(
        modules.supabase,
        accessToken,
        runtimeFixture.runtimeId,
        createdVersions,
    ...
    const referenceUpdate: UpdateReferenceStageSummary = {
      ...
      currentShortName: updatedShortName,
    ...
    if (options.writeRuntime) {
      ...
      const runtimeRecord = buildSourceCreateVersionUpdateReferenceRuntimeRecord(options, {
        cleanupAttempted,
        cleanupPassed,
        createStep,
        createVersionStep,
...
async function loadWorkflowModules(supabaseTarget: SupabaseTarget): Promise<LoadedWorkflowModules> {
  ...
  return {
    sourcesUtil: {
      ...
      genSourceJsonOrdered: sourcesUtilExports.genSourceJsonOrdered,
    },
    generalApi: {
...
function buildSourceReferenceUri(id: string) {
  return `../sources/${id}.xml`;
}
...
function selectCreatedVersionReferenceCandidate(
  ...
  return (
    newRefs.find(
      (reference) =>
        reference.id === runtimeId &&
        reference.type === 'source data set' &&
        reference.currentVersion === currentVersion &&
        reference.newVersion === createdVersion,
    ) ?? null
  );
}

function alignSourceDataSetFormatReferenceVersion(
  ...
  if (
    reference &&
    reference['@refObjectId'] === runtimeId &&
    reference['@type'] === 'source data set'
  ) {
    reference['@uri'] = buildSourceReferenceUri(runtimeId);
    reference['@version'] = version;
  }

  return clonedJsonOrdered;
}

async function cleanupCreatedSourceVersions(
  supabase: SupabaseClient,
  accessToken: string,
  runtimeId: string,
  createdVersions: string[],
