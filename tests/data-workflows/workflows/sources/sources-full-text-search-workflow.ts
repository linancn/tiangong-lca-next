...
import {
  SOURCE_FULL_TEXT_SEARCH_DATA_WORKFLOW_HELP,
  parseSourceFullTextSearchCliArgs,
  runSourceFullTextSearchSmoke,
} from './sources-full-text-search-workflow-lib';

export async function main(argv: string[] = process.argv.slice(2)) {
  const detailResultState = extractDetailResultFlag(argv);
  const options = parseSourceFullTextSearchCliArgs(detailResultState.argv);

  if (options.help) {
    printDataWorkflowHelp(SOURCE_FULL_TEXT_SEARCH_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runSourceFullTextSearchSmoke(options);
  ...
  if (!showDetailedResult) {
    printCompactSmokeSummary({
      ...
      dataType: 'source full-text search',
      passed: result.passed,
      version: result.runtimeFixture.version,
    });
  } else {
    console.log('Source full-text-search data workflow');
  ...
  if (!result.passed) {
    throw new Error('Source full-text-search data workflow failed.');
  }
}
