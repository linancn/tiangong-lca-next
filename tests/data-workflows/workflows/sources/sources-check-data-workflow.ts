...
import {
  SOURCE_CHECK_DATA_WORKFLOW_HELP,
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(SOURCE_CHECK_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runSourceCheckDataSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Source check-data data workflow');
  ...
  if (!result.passed) {
    throw new Error('Source check-data data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Source check-data data workflow passed.');
  }
}
