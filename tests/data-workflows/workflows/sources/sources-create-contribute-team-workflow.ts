...
import {
  SOURCE_CREATE_CONTRIBUTE_TEAM_DATA_WORKFLOW_HELP,
  parseCreateContributeTeamCliArgs,
  runSourceCreateContributeTeamSmoke,
} from './sources-create-contribute-team-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(SOURCE_CREATE_CONTRIBUTE_TEAM_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runSourceCreateContributeTeamSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Source create-contribute-team data workflow');
    ...
    console.log(`Runtime source version: ${result.runtimeFixture.version}`);
  ...
  if (!result.passed) {
    throw new Error('Source create-contribute-team data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Source create-contribute-team data workflow passed.');
  }
}
