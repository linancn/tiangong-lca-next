...
import {
  SOURCE_CREATE_DATA_WORKFLOW_HELP,
  parseCliArgs,
  runSourceCreateSmoke,
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(SOURCE_CREATE_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runSourceCreateSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Source create data workflow');
  ...
  if (!result.passed) {
    throw new Error('Source create data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Source create data workflow passed.');
  }
}
