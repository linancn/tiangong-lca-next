...
export const DEFAULT_SOURCE_FIXTURE_PATH =
  'tests/data-workflows/fixtures/data/sources/001_create.json';
export const DEFAULT_SOURCE_EXPECTED_PATH =
  'tests/data-workflows/fixtures/result/sources/001_create.md';
...
export const DEFAULT_SOURCE_ROLE = 'user';
export const DEFAULT_SOURCE_CONTRIBUTE_TEAM_ROLE = 'team-member';
export const TEST_USERS_JSON_ENV = SHARED_TEST_USERS_JSON_ENV;
export const RUNTIME_SOURCE_ID_PLACEHOLDER = '__RUNTIME_SOURCE_ID__';

const SOURCE_PERMANENT_URI_BASE = 'https://lcdn.tiangong.earth/datasetdetail/source.xhtml';
...
export type SourceCreateFixture = {
  id: string;
...
export type RuntimeFixture = {
  fixture: SourceCreateFixture;
  sourceFixtureId: string;
  runtimeId: string;
  version: string;
};

export type PreparedSourceJsonOrdered = {
  jsonOrdered: Record<string, any>;
  version: string;
};
...
Usage:
  pnpm test:workflows --sources:create --frontend-url http://127.0.0.1:8000 --supabase-url https://submidrhbtknjxfympna.supabase.co
  pnpm test:workflows --sources:create --role admin --frontend-url https://lca.tiangong.earth --supabase-url https://supabase.com/dashboard/project/submidrhbtknjxfympna --no-keep-data
...
Flags:
  ...
  --data-file <path>               Defaults to tests/data-workflows/fixtures/data/sources/001_create.json
  ...
  --keep-data                      Keep the created source after validation (default)
  --keep-created                   Alias of --keep-data
  --no-keep-data                   Delete the created source after validation
...
export function parseCliArgs(argv: string[], cwd = process.cwd()): CliOptions {
  ...
  const options: CliOptions = {
    dataFile: path.resolve(cwd, DEFAULT_SOURCE_FIXTURE_PATH),
    expectedFile: path.resolve(cwd, DEFAULT_SOURCE_EXPECTED_PATH),
    generateId: true,
    help: false,
    keepData: true,
    role: DEFAULT_SOURCE_ROLE,
    runtimeRecordFile: resolveRuntimeRecordFilePath(
      path.resolve(cwd, DEFAULT_SOURCE_FIXTURE_PATH),
      cwd,
    ),
...
export async function loadSourceFixture(filePath: string): Promise<SourceCreateFixture> {
  const fixture = await loadJsonFile<SourceCreateFixture>(filePath);

  if (!fixture || typeof fixture !== 'object') {
    throw new Error(`Invalid source fixture: ${filePath}`);
  }
  ...
  if (fixture.table !== 'sources') {
    throw new Error(`Fixture table must be "sources": ${filePath}`);
  }
...
export function prepareRuntimeFixture(
  ...
  const preparedJsonOrdered = prepareSourceJsonOrderedForRuntime(clonedFixture.jsonOrdered, {
    runtimeId,
  });
...
export function prepareSourceJsonOrderedForRuntime(
  jsonOrdered: Record<string, any>,
  options: {
    runtimeId: string;
    version?: string;
  },
): PreparedSourceJsonOrdered {
  const clonedJsonOrdered = replaceRuntimeSourceIdPlaceholder(
    deepClone(jsonOrdered),
    options.runtimeId,
  );
  setNestedValue(clonedJsonOrdered, SOURCE_UUID_PATH, options.runtimeId);

  const version = options.version ?? getSourceVersion(clonedJsonOrdered);
  if (!version) {
    throw new Error('Unable to resolve source dataset version from jsonOrdered.');
  }

  setNestedValue(clonedJsonOrdered, SOURCE_VERSION_PATH, version);
  setNestedValue(
    clonedJsonOrdered,
    SOURCE_PERMANENT_URI_PATH,
...
export async function computeSourceRuleVerification(
  jsonOrdered: Record<string, any>,
): Promise<boolean> {
  const sdkValidation = createTidasSource(jsonOrdered).validateEnhanced();
...
export async function runSourceCreateSmoke(
  options: CliOptions,
  ...
  let submittedRuleVerification = await computeSourceRuleVerification(
  ...
  submittedRuleVerification = await computeSourceRuleVerification(
    runtimeFixture.fixture.jsonOrdered,
  );
  ...
  if (recordResult.error || !recordResult.data) {
    throw new Error(
      `Failed to query created source: ${recordResult.error?.message ?? 'record not found'}`,
    );
  }
...
function getSourceVersion(jsonOrdered: Record<string, any>) {
  const version = getNestedValue<string>(jsonOrdered, SOURCE_VERSION_PATH);
...
export function buildPermanentDataSetUri(id: string, version: string) {
  return `${SOURCE_PERMANENT_URI_BASE}?uuid=${id}&version=${version}`;
}
...
function replaceRuntimeSourceIdPlaceholder<T>(value: T, runtimeId: string): T {
  if (typeof value === 'string') {
    return value.split(RUNTIME_SOURCE_ID_PLACEHOLDER).join(runtimeId) as T;
  }

  if (Array.isArray(value)) {
    return value.map((item) => replaceRuntimeSourceIdPlaceholder(item, runtimeId)) as T;
  }
  ...
  return Object.entries(value as Record<string, unknown>).reduce<Record<string, unknown>>(
    (accumulator, [key, nestedValue]) => {
      accumulator[key] = replaceRuntimeSourceIdPlaceholder(nestedValue, runtimeId);
      return accumulator;
    },
    {},
  ) as T;
