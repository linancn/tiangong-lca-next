...
import {
  SOURCE_CREATE_VIEW_COPY_DATA_WORKFLOW_HELP,
  parseCreateViewCopyCliArgs,
  runSourceCreateViewCopySmoke,
} from './sources-create-view-copy-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(SOURCE_CREATE_VIEW_COPY_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runSourceCreateViewCopySmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Source create-view-copy data workflow');
    ...
    console.log(`Copied source ID: ${result.copy.copiedId}`);
    console.log(`Copied source version: ${result.copy.copiedVersion}`);
    console.log(
  ...
  if (!result.passed) {
    throw new Error('Source create-view-copy data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Source create-view-copy data workflow passed.');
  }
}
