...
import {
  SOURCE_EDIT_DATA_WORKFLOW_HELP,
  parseEditCliArgs,
  runSourceEditSmoke,
} from './sources-edit-workflow-lib';
...
export async function main(argv: string[] = process.argv.slice(2)) {
  ...
  if (options.help) {
    printDataWorkflowHelp(SOURCE_EDIT_DATA_WORKFLOW_HELP);
    return;
  }

  const result = await runSourceEditSmoke(options);
  ...
  if (!showDetailedResult) {
    ...
    console.log('Source edit data workflow');
  ...
  if (!result.passed) {
    throw new Error('Source edit data workflow failed.');
  }

  if (showDetailedResult) {
    console.log('Source edit data workflow passed.');
  }
}
