...
import {
  buildFlowEditRuntimeRecord,
  buildParsedValidationExpectationResults,
  buildValidationExpectations,
  parseEditCliArgs,
  runFlowEditSmoke,
} from '../../workflows/flows/flows-edit-workflow-lib';
...
describe('flows-edit-workflow-lib', () => {
  ...
  it('builds a runtime record with success and failing edit summaries', () => {
    const runtimeRecord = buildFlowEditRuntimeRecord(
      {
        ...
        editDataFile:
          '/repo/tests/data-workflows/fixtures/data/flows/003_edit_data_validate_false.json',
        editExpectedFile:
          '/repo/tests/data-workflows/fixtures/result/flows/003_edit_data_validate_false.md',
  ...
  it('runs the create-success-fail edit workflow on a single flow record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'flows-edit-workflow-'));
...
`,
      ...
      const result = await runFlowEditSmoke(
        {
          createDataFile,
          createExpectedFile,
          editDataFile,
      ...
      expect(result.successEditStep.record.id).toBe('fixture-flow-id');
      expect(result.failingEditStep.record.id).toBe('fixture-flow-id');
      ...
      expect(
        functionsInvoke.mock.calls[2]?.[1]?.body?.jsonOrdered?.flowDataSet?.flowInformation
          ?.dataSetInformation?.name?.baseName?.['#text'],
      ).toBe('updated-flow-fail');
