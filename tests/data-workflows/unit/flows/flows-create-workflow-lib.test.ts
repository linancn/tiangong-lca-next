...
import type { FlowCreateFixture } from '../../workflows/flows/flows-create-workflow-lib';
import {
  ...
  writeRuntimeRecord,
} from '../../workflows/flows/flows-create-workflow-lib';

describe('flows-create-workflow-lib', () => {
  it('parses cli arguments with defaults and explicit flags', () => {
    ...
    expect(options.dataFile).toBe('/repo/tests/data-workflows/fixtures/data/flows/001_create.json');
    expect(options.usersFile).toBe('/repo/tests/data-workflows/fixtures/data/users.json');
    expect(options.runtimeRecordFile).toBe(
      '/repo/tests/data-workflows/runtime/flows/001_create.last-run.json',
    );
  });
  ...
  it('uses an explicit runtime record path and enables writing when requested', () => {
    const options = parseCliArgs(
      [
        '--role=user',
        '--data-file',
        'tests/data-workflows/fixtures/data/flows/001_create.json',
        '--runtime-record-file',
        'output/custom-flow-runtime.json',
      ],
      '/repo',
    );

    expect(options.writeRuntime).toBe(true);
    expect(options.runtimeRecordFile).toBe('/repo/output/custom-flow-runtime.json');
  });
  ...
  it('loads users from .env.users.local before falling back to a users file', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'flows-create-env-file-'));
  ...
  it('prepares a runtime fixture with a generated id and updates the embedded uuid fields', () => {
    const fixture: FlowCreateFixture = {
      id: 'fixture-id',
      table: 'flows',
      jsonOrdered: {
        flowDataSet: {
    ...
    expect(
      runtimeFixture.fixture.jsonOrdered.flowDataSet.flowInformation.dataSetInformation[
        'common:UUID'
      ],
    ).toBe('runtime-id');
    expect(
      runtimeFixture.fixture.jsonOrdered.flowDataSet.administrativeInformation
        .publicationAndOwnership['common:permanentDataSetURI'],
    ).toBe(
      'https://lcdn.tiangong.earth/datasetdetail/productFlow.xhtml?uuid=runtime-id&version=01.01.000',
    );
  ...
  it('derives the default runtime record path from tests/data-workflows/fixtures/data fixtures', () => {
    expect(
      resolveRuntimeRecordFilePath(
        '/repo/tests/data-workflows/fixtures/data/flows/001_create.json',
        '/',
      ),
    ).toBe('/repo/tests/data-workflows/runtime/flows/001_create.last-run.json');
  });
  ...
  it('writes a runtime record file without mutating the source fixture', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'flows-runtime-record-'));
    ...
    try {
      ...
      const runtimeRecord = buildRuntimeRecord(
        {
          dataFile: '/repo/tests/data-workflows/fixtures/data/flows/001_create.json',
          expectedFile: '/repo/tests/data-workflows/fixtures/result/flows/001_create.md',
