...
import {
  ...
  buildFlowCreateVersionUpdateReferenceRuntimeRecord,
  incrementDatasetVersion,
  normalizeJsonForCreateVersionComparison,
  parseCreateVersionUpdateReferenceCliArgs,
  runFlowCreateVersionUpdateReferenceSmoke,
} from '../../workflows/flows/flows-create-version-update-reference-workflow-lib';
...
function buildVersionComparisonFixture(id = '__RUNTIME_FLOW_ID__') {
  return {
    id: '87d48aa7-35cb-4e15-8f32-449d8bd1f569',
    table: 'flows',
    jsonOrdered: buildFlowJsonOrdered(
      id,
      '01.01.000',
      '01.01.000',
...
describe('flows-create-version-update-reference-workflow-lib', () => {
  ...
  it('normalizes create-version comparison JSON by excluding versioned runtime fields', () => {
    const createJson = buildFlowJsonOrdered(
      'flow-1',
      '01.01.000',
      '01.01.000',
      buildOwnerShortDescription(),
    ...
    const createVersionJson = buildFlowJsonOrdered(
      'flow-1',
      '01.01.001',
      '01.01.000',
      buildOwnerShortDescription(),
  ...
  it('builds a runtime record with version and update-reference summaries', () => {
    const runtimeRecord = buildFlowCreateVersionUpdateReferenceRuntimeRecord(
      {
        dataFile:
          '/repo/tests/data-workflows/fixtures/data/flows/004_create_version_update_reference.json',
        expectedFile:
          '/repo/tests/data-workflows/fixtures/result/flows/004_create_version_update_reference.md',
      ...
      },
      {
        ...
        versionCreation: {
          ...
          permanentDataSetUri: buildPermanentUri('flow-1', '01.01.001'),
        },
      },
    );
  ...
  it('runs the create-version-update-reference workflow on the same flow id', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'flows-version-reference-'));
    ...
    try {
      ...
      const result = await runFlowCreateVersionUpdateReferenceSmoke(
        {
          dataFile,
          expectedFile,
          generateId: false,
          ...
          modulesLoader: async () =>
            ({
              flowsUtil: {
                ...
                genFlowJsonOrdered: (id: string, data: any) => {
                  const version =
