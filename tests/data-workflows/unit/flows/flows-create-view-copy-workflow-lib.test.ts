...
import {
  ...
  buildFlowCreateViewCopyRuntimeRecord,
  normalizeJsonForCopyComparison,
  parseCreateViewCopyCliArgs,
  resolveCreateViewCopyRuntimeRecordFilePath,
  runFlowCreateViewCopySmoke,
} from '../../workflows/flows/flows-create-view-copy-workflow-lib';
...
1. 调用\`getFlowDetail(id, version)\`成功
...
15. 调用\`getFlowDetail(copyId, version)\`成功
16. 返回的\`id\`和复制后的数据\`id\`一致
17. 返回的\`version\`为\`01.01.000\`
18. 返回的\`json\`和数据库中的\`json\`一致
`;

describe('flows-create-view-copy-workflow-lib', () => {
  ...
  it('defaults role to user when --role is omitted', () => {
    ...
    expect(options.createDataFile).toBe(
      '/repo/tests/data-workflows/fixtures/data/flows/001_create_view_copy.json',
    );
    expect(options.role).toBe('user');
    expect(options.keepData).toBe(true);
    expect(options.generateId).toBe(true);
  ...
  it('builds code-owned workflow expectations and resolves the default runtime record path', () => {
    ...
    expect(
      resolveCreateViewCopyRuntimeRecordFilePath(
        '/repo/tests/data-workflows/fixtures/result/flows/001_create_view_copy.md',
        '/',
      ),
    ).toBe('/repo/tests/data-workflows/runtime/flows/001_create_view_copy.last-run.json');
  });

  it('normalizes copy-comparison JSON by excluding runtime-only fields', () => {
    const sourceJson = buildFlowJsonOrdered('source-id', '01.01.000', '2026-04-16T05:31:26.289Z');
    const copiedJson = buildFlowJsonOrdered('copy-id', '01.01.000', '2026-04-19T00:00:00.000Z');
  ...
  it('treats missing empty objects and arrays as equivalent during copy comparison', () => {
    const sourceJson = buildFlowJsonOrdered('source-id', '01.01.000', '2026-04-16T05:31:26.289Z');
    const copiedJson = buildFlowJsonOrdered('copy-id', '01.01.000', '2026-04-19T00:00:00.000Z');
    const flowInfo = sourceJson.flowDataSet.flowInformation.dataSetInformation as Record<
      string,
      unknown
    >;

    flowInfo.classificationInformation = {};
    flowInfo['common:generalComment'] = [];
    flowInfo['common:synonyms'] = [];
  ...
  it('builds a workflow runtime record without persisting json/json_ordered', () => {
    const runtimeRecord = buildFlowCreateViewCopyRuntimeRecord(
      {
        createDataFile: '/repo/tests/data-workflows/fixtures/data/flows/001_create_view_copy.json',
        createExpectedFile: '/repo/tests/data-workflows/fixtures/result/flows/001_create.md',
  ...
  it('runs the create-view-copy workflow and writes a runtime record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'flows-create-view-copy-'));
    ...
    try {
      ...
      await writeJson(createDataFile, {
        id: sourceId,
        table: 'flows',
        jsonOrdered: buildFlowJsonOrdered(sourceId, '01.01.000', '2026-04-16T05:31:26.289Z'),
      });
      ...
      const functionsInvoke = jest.fn(async (functionName: string, request: any) => {
        if (functionName === 'app_dataset_create') {
          const version =
            request.body.jsonOrdered.flowDataSet.administrativeInformation.publicationAndOwnership[
      ...
      const getFlowDetail = jest.fn(async (id: string, version: string) => {
      ...
      const createFlows = jest.fn(async (id: string, formData: any) => {
        const version =
          formData.administrativeInformation?.publicationAndOwnership?.['common:dataSetVersion'] ??
          '01.01.000';
        const baseName = deepClone(
          formData.flowInformation?.dataSetInformation?.name?.baseName ?? {
            '#text': 'test-flow',
            '@xml:lang': 'en',
          },
        );
        const jsonOrdered = buildFlowJsonOrdered(id, version, '2026-04-19T00:00:00.000Z', baseName);
      ...
      const result = await runFlowCreateViewCopySmoke(
        {
          createDataFile,
          createExpectedFile,
          followUpExpectedFile,
        ...
        },
        {
          ...
          modulesLoader: async () =>
            ({
              ...
              flowsUtil: {
                genFlowFromData: (flowDataSet: any) => ({
                  administrativeInformation: deepClone(flowDataSet.administrativeInformation),
                  flowInformation: deepClone(flowDataSet.flowInformation),
                }),
              },
              supabase: {
                auth: {
      ...
      expect(result.sourceViewStep.detail.id).toBe(sourceId);
      expect(result.copy.copiedId).toBe(copiedId);
      expect(result.copyStep.record.id).toBe(copiedId);
      expect(result.viewCopyStep.detail.id).toBe(copiedId);
      ...
      expect(getFlowDetail).toHaveBeenCalledWith(sourceId, '01.01.000');
      expect(createFlows).toHaveBeenCalledWith(
        copiedId,
        expect.objectContaining({
          administrativeInformation: expect.any(Object),
          flowInformation: expect.any(Object),
        }),
      );
