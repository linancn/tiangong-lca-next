...
function buildFlowJsonOrdered(id: string, version: string, timestamp: string) {
  return {
    flowDataSet: {
      flowInformation: {
        dataSetInformation: {
...
1. 调用\`contributeSource("flows", id, version)\`成功
...
describe('flows-create-contribute-team-workflow-lib', () => {
  ...
  it('defaults role to team-member when --role is omitted', () => {
    ...
    expect(options.createDataFile).toBe(
      '/repo/tests/data-workflows/fixtures/data/flows/005_create_contribute_team.json',
    );
  ...
  it('builds code-owned workflow expectations and resolves the default runtime record path', () => {
    ...
    expect(
      resolveCreateContributeTeamRuntimeRecordFilePath(
        '/repo/tests/data-workflows/fixtures/result/flows/005_create_contribute_team.md',
        '/',
      ),
    ).toBe('/repo/tests/data-workflows/runtime/flows/005_create_contribute_team.last-run.json');
  });

  it('builds a workflow runtime record without persisting json_ordered', () => {
    const runtimeRecord = buildFlowCreateContributeTeamRuntimeRecord(
      {
        createDataFile:
          '/repo/tests/data-workflows/fixtures/data/flows/005_create_contribute_team.json',
        createExpectedFile:
          '/repo/tests/data-workflows/fixtures/result/flows/005_create_contribute_team_create.md',
  ...
  it('runs the create-contribute-team workflow and writes a runtime record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'flows-create-contribute-team-'));
    ...
    try {
      ...
      await writeJson(createDataFile, {
        id: flowId,
        table: 'flows',
        jsonOrdered: buildFlowJsonOrdered(flowId, '01.01.000', '2026-04-16T05:31:26.289Z'),
      });
      ...
      const result = await runFlowCreateContributeTeamSmoke(
        {
          createDataFile,
          createExpectedFile,
          followUpExpectedFile,
      ...
      expect(contributeSource).toHaveBeenCalledWith('flows', flowId, '01.01.000');
