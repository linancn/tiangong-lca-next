...
import {
  buildFlowCheckDataRuntimeRecord,
  buildValidationExpectationResults,
  getExpectedRuleVerification,
  getFixtureOrderedJson,
  parseCheckDataCliArgs,
  runFlowCheckDataSmoke,
  type FlowCheckDataFixture,
} from '../../workflows/flows/flows-check-data-workflow-lib';
import { buildCreateExpectations } from '../../workflows/flows/flows-create-workflow-lib';
...
describe('flows-check-data-workflow-lib', () => {
  ...
  it('prefers jsonOrdered and falls back to json_ordered then json in validation fixtures', () => {
    const withJsonOrdered: FlowCheckDataFixture = {
      json: { from: 'json' },
      json_ordered: { from: 'json_ordered' },
      jsonOrdered: { from: 'jsonOrdered' },
      table: 'flows',
    };
    const withOrderedJson: FlowCheckDataFixture = {
      json: { from: 'json' },
      json_ordered: { from: 'json_ordered' },
      table: 'flows',
    };
    const withJsonOnly: FlowCheckDataFixture = {
      json: { from: 'json' },
      table: 'flows',
    };
  ...
  it('builds a workflow runtime record without persisting json_ordered', () => {
    const runtimeRecord = buildFlowCheckDataRuntimeRecord(
      {
        checkDataFile: '/repo/tests/data-workflows/fixtures/data/flows/002_check_data_success.json',
        checkExpectedFile:
          '/repo/tests/data-workflows/fixtures/result/flows/002_check_data_success.md',
        createDataFile: '/repo/tests/data-workflows/fixtures/data/flows/001_create.json',
    ...
    expect(runtimeRecord.create.runtimeId).toBe('flow-1');
  ...
  it('runs the create-then-update workflow against one flow record and writes a runtime record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'flows-check-data-workflow-'));
            ...
            administrativeInformation: {
              publicationAndOwnership: {
                ...
                'common:permanentDataSetURI':
                  'https://lcdn.tiangong.earth/datasetdetail/productFlow.xhtml?uuid=fixture-flow-id&version=01.01.000',
              },
            },
          },
        },
...
`,
      ...
      const validateDatasetRuleVerification = jest
        ...
        .mockResolvedValueOnce({
          ...
          nonExistentRef: [],
          ruleVerification: false,
          unRuleVerification: [{ path: 'missing-base-name' }],
        })
        .mockResolvedValueOnce({
          datasetSdkValid: true,
          nonExistentRef: [],
      ...
      const result = await runFlowCheckDataSmoke(
        {
          checkDataFile,
          checkExpectedFile,
          createDataFile,
      ...
      expect(result.runtimeFixture.runtimeId).toBe('fixture-flow-id');
      expect(result.createStep.record.id).toBe('fixture-flow-id');
      expect(result.updateStep.record.id).toBe('fixture-flow-id');
      ...
      expect(
        functionsInvoke.mock.calls[1]?.[1]?.body?.jsonOrdered?.flowDataSet?.flowInformation
          ?.dataSetInformation?.['common:UUID'],
      ).toBe('fixture-flow-id');
      expect(
        functionsInvoke.mock.calls[1]?.[1]?.body?.jsonOrdered?.flowDataSet
      ...
      expect(writtenRuntimeRecord.create.runtimeId).toBe('fixture-flow-id');
