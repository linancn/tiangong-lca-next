...
import {
  buildContactCheckDataRuntimeRecord,
  buildValidationExpectationResults,
  getExpectedRuleVerification,
  getFixtureOrderedJson,
  parseCheckDataCliArgs,
  runContactCheckDataSmoke,
  type ContactCheckDataFixture,
} from '../../workflows/contacts/contacts-check-data-workflow-lib';
import { buildCreateExpectations } from '../../workflows/contacts/contacts-create-workflow-lib';
...
describe('contacts-check-data-workflow-lib', () => {
  ...
  it('prefers jsonOrdered and falls back to json_ordered then json in validation fixtures', () => {
    const withJsonOrdered: ContactCheckDataFixture = {
      json: { from: 'json' },
      json_ordered: { from: 'json_ordered' },
      jsonOrdered: { from: 'jsonOrdered' },
      table: 'contacts',
    };
    const withOrderedJson: ContactCheckDataFixture = {
      json: { from: 'json' },
      json_ordered: { from: 'json_ordered' },
      table: 'contacts',
    };
    const withJsonOnly: ContactCheckDataFixture = {
      json: { from: 'json' },
      table: 'contacts',
    };
  ...
  it('builds a workflow runtime record without persisting json_ordered', () => {
    const runtimeRecord = buildContactCheckDataRuntimeRecord(
      {
        checkDataFile:
        ...
        createDataFile: '/repo/tests/data-workflows/fixtures/data/contacts/001_create.json',
  ...
  it('runs the create-then-update workflow against one contact record and writes a runtime record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'contacts-check-data-workflow-'));
      ...
      await writeJson(createDataFile, {
        ...
        jsonOrdered: {
          contactDataSet: {
            contactInformation: {
              dataSetInformation: {
                'common:UUID': 'fixture-contact-id',
              },
            },
            administrativeInformation: {
              publicationAndOwnership: {
                ...
                'common:permanentDataSetURI':
                  'https://lcdn.tiangong.earth/datasetdetail/contact.xhtml?uuid=fixture-contact-id&version=01.01.000',
              },
            },
          },
        },
...
`,
      ...
      const result = await runContactCheckDataSmoke(
        {
          checkDataFile,
          checkExpectedFile,
          createDataFile,
      ...
      expect(result.runtimeFixture.runtimeId).toBe('fixture-contact-id');
      expect(result.createStep.record.id).toBe('fixture-contact-id');
      expect(result.updateStep.record.id).toBe('fixture-contact-id');
      ...
      expect(
        functionsInvoke.mock.calls[1]?.[1]?.body?.jsonOrdered?.contactDataSet?.contactInformation
          ?.dataSetInformation?.['common:UUID'],
      ).toBe('fixture-contact-id');
      expect(
        functionsInvoke.mock.calls[1]?.[1]?.body?.jsonOrdered?.contactDataSet
      ...
      expect(writtenRuntimeRecord.create.runtimeId).toBe('fixture-contact-id');
