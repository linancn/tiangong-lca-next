...
import {
  buildContactEditRuntimeRecord,
  buildParsedValidationExpectationResults,
  buildValidationExpectations,
  parseEditCliArgs,
  runContactEditSmoke,
} from '../../workflows/contacts/contacts-edit-workflow-lib';
...
describe('contacts-edit-workflow-lib', () => {
  ...
  it('builds a runtime record with success and failing edit summaries', () => {
    const runtimeRecord = buildContactEditRuntimeRecord(
      {
        ...
        editDataFile:
          '/repo/tests/data-workflows/fixtures/data/contacts/003_edit_data_validate_false.json',
        editExpectedFile:
          '/repo/tests/data-workflows/fixtures/result/contacts/003_edit_data_validate_false.md',
  ...
  it('runs the create-success-fail edit workflow on a single contact record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'contacts-edit-workflow-'));
            ...
            administrativeInformation: {
              publicationAndOwnership: {
                ...
                'common:referenceToOwnershipOfDataSet': {
                  '@refObjectId': 'existing-contact',
                  '@type': 'contact data set',
                  '@uri': '../contacts/existing-contact.xml',
                  '@version': '01.01.001',
                },
              },
            },
            ...
            administrativeInformation: {
              publicationAndOwnership: {
                ...
                'common:permanentDataSetURI':
                  'https://lcdn.tiangong.earth/datasetdetail/contact.xhtml?uuid=__RUNTIME_CONTACT_ID__&version=01.01.000',
                'common:referenceToOwnershipOfDataSet': {
                  '@refObjectId': 'missing-contact',
                  '@type': 'contact data set',
                  '@uri': '../contacts/missing-contact.xml',
                  '@version': '99.99.999',
                },
              },
            },
...
`,
      ...
      const result = await runContactEditSmoke(
        {
          createDataFile,
          createExpectedFile,
          editDataFile,
      ...
      expect(result.successEditStep.record.id).toBe('fixture-contact-id');
      expect(result.failingEditStep.record.id).toBe('fixture-contact-id');
      ...
      expect(
        functionsInvoke.mock.calls[2]?.[1]?.body?.jsonOrdered?.contactDataSet
