...
import type { ContactCreateFixture } from '../../workflows/contacts/contacts-create-workflow-lib';
import {
  ...
  writeRuntimeRecord,
} from '../../workflows/contacts/contacts-create-workflow-lib';

describe('contacts-create-workflow-lib', () => {
  it('parses cli arguments with defaults and explicit flags', () => {
    ...
    expect(options.dataFile).toBe(
      '/repo/tests/data-workflows/fixtures/data/contacts/001_create.json',
    );
    expect(options.usersFile).toBe('/repo/tests/data-workflows/fixtures/data/users.json');
    expect(options.runtimeRecordFile).toBe(
      '/repo/tests/data-workflows/runtime/contacts/001_create.last-run.json',
    );
  });
  ...
  it('uses an explicit runtime record path and enables writing when requested', () => {
    const options = parseCliArgs(
      [
        ...
        '--runtime-record-file',
        'output/custom-contact-runtime.json',
      ],
      '/repo',
    );

    expect(options.writeRuntime).toBe(true);
    expect(options.runtimeRecordFile).toBe('/repo/output/custom-contact-runtime.json');
  });
  ...
  it('loads users from .env.users.local before falling back to a users file', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'contacts-create-env-file-'));
  ...
  it('prepares a runtime fixture with a generated id and updates the embedded uuid fields', () => {
    const fixture: ContactCreateFixture = {
      id: 'fixture-id',
      table: 'contacts',
      jsonOrdered: {
        contactDataSet: {
    ...
    expect(
      runtimeFixture.fixture.jsonOrdered.contactDataSet.contactInformation.dataSetInformation[
        'common:UUID'
      ],
    ).toBe('runtime-id');
    expect(
      runtimeFixture.fixture.jsonOrdered.contactDataSet.administrativeInformation
        .publicationAndOwnership['common:permanentDataSetURI'],
    ).toBe(
      'https://lcdn.tiangong.earth/datasetdetail/contact.xhtml?uuid=runtime-id&version=01.01.000',
    );
    expect(
      runtimeFixture.fixture.jsonOrdered.contactDataSet.administrativeInformation
  ...
  it('derives the default runtime record path from tests/data-workflows/fixtures/data fixtures', () => {
    ...
    ).toBe('/repo/tests/data-workflows/runtime/contacts/001_create.last-run.json');
  });
  ...
  it('writes a runtime record file without mutating the source fixture', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'contacts-runtime-record-'));
    ...
    try {
      ...
      const runtimeRecord = buildRuntimeRecord(
        {
          dataFile: '/repo/tests/data-workflows/fixtures/data/contacts/001_create.json',
          expectedFile: '/repo/tests/data-workflows/fixtures/result/contacts/001_create.md',
