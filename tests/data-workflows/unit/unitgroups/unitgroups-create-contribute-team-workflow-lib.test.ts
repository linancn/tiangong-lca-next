...
import {
  ...
  buildUnitGroupCreateContributeTeamRuntimeRecord,
  parseCreateContributeTeamCliArgs,
  resolveCreateContributeTeamRuntimeRecordFilePath,
  runUnitGroupCreateContributeTeamSmoke,
} from '../../workflows/unitgroups/unitgroups-create-contribute-team-workflow-lib';
...
function buildUnitGroupJsonOrdered(id: string, version: string, timestamp: string) {
  return {
    unitGroupDataSet: {
...
1. 调用\`contributeSource("unitgroups", id, version)\`成功
...
describe('unitgroups-create-contribute-team-workflow-lib', () => {
  ...
  it('builds code-owned workflow expectations and resolves the default runtime record path', () => {
    ...
    expect(
      resolveCreateContributeTeamRuntimeRecordFilePath(
        '/repo/tests/data-workflows/fixtures/result/unitgroups/005_create_contribute_team.md',
        '/',
      ),
    ).toBe(
      '/repo/tests/data-workflows/runtime/unitgroups/005_create_contribute_team.last-run.json',
    );
  });

  it('builds a workflow runtime record without persisting json_ordered', () => {
    const runtimeRecord = buildUnitGroupCreateContributeTeamRuntimeRecord(
      {
        createDataFile:
          '/repo/tests/data-workflows/fixtures/data/unitgroups/005_create_contribute_team.json',
        createExpectedFile:
          '/repo/tests/data-workflows/fixtures/result/unitgroups/005_create_contribute_team_create.md',
  ...
  it('runs the create-contribute-team workflow and writes a runtime record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'unitgroups-create-contribute-team-'));
    ...
    try {
      ...
      const result = await runUnitGroupCreateContributeTeamSmoke(
        {
          createDataFile,
          createExpectedFile,
          followUpExpectedFile,
      ...
      expect(contributeSource).toHaveBeenCalledWith('unitgroups', sourceId, '01.01.000');
