...
import {
  ...
  buildUnitGroupEditRuntimeRecord,
  buildValidationExpectations,
  parseEditCliArgs,
  runUnitGroupEditSmoke,
} from '../../workflows/unitgroups/unitgroups-edit-workflow-lib';
...
describe('unitgroups-edit-workflow-lib', () => {
  ...
  it('builds a runtime record with success and failing edit summaries', () => {
    const runtimeRecord = buildUnitGroupEditRuntimeRecord(
      {
        ...
        editDataFile:
          '/repo/tests/data-workflows/fixtures/data/unitgroups/003_edit_data_validate_false.json',
        editExpectedFile:
          '/repo/tests/data-workflows/fixtures/result/unitgroups/003_edit_data_validate_false.md',
  ...
  it('runs the create-success-fail edit workflow on a single unit group record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'unitgroups-edit-workflow-'));
          ...
          unitGroupDataSet: {
            unitGroupInformation: {
              dataSetInformation: {
                ...
                'common:shortName': {
                  '#text': 'updated-unitgroup-success',
                  '@xml:lang': 'en',
                },
              },
            },
...
`,
      ...
      const result = await runUnitGroupEditSmoke(
        {
          createDataFile,
          createExpectedFile,
          editDataFile,
      ...
      expect(result.successEditStep.record.id).toBe('fixture-unitgroup-id');
      expect(result.failingEditStep.record.id).toBe('fixture-unitgroup-id');
      ...
      expect(
        functionsInvoke.mock.calls[2]?.[1]?.body?.jsonOrdered?.unitGroupDataSet
