...
import {
  ...
  buildUnitGroupCreateViewCopyRuntimeRecord,
  normalizeJsonForCopyComparison,
  parseCreateViewCopyCliArgs,
  resolveCreateViewCopyRuntimeRecordFilePath,
  runUnitGroupCreateViewCopySmoke,
} from '../../workflows/unitgroups/unitgroups-create-view-copy-workflow-lib';
...
1. 调用\`getUnitGroupDetail(id, version)\`成功
2. 返回的\`id\`和创建数据的\`id\`一致
3. 返回的\`version\`为\`01.01.000\`
4. 返回的\`json\`和创建数据的\`json\`一致
...
11. 复制后的\`rule_verification\`和原数据一致
...
14. 复制后的\`json_ordered\`中不存在\`common:permanentDataSetURI\`

# view copy

15. 调用\`getUnitGroupDetail(copyId, version)\`成功
16. 返回的\`id\`和复制后的数据\`id\`一致
17. 返回的\`version\`为\`01.01.000\`
18. 返回的\`json\`和复制后的\`json\`一致
`;

describe('unitgroups-create-view-copy-workflow-lib', () => {
  ...
  it('builds code-owned workflow expectations and resolves the default runtime record path', () => {
    ...
    expect(
      resolveCreateViewCopyRuntimeRecordFilePath(
        '/repo/tests/data-workflows/fixtures/result/unitgroups/001_create_view_copy.md',
        '/',
      ),
    ).toBe('/repo/tests/data-workflows/runtime/unitgroups/001_create_view_copy.last-run.json');
  });

  it('normalizes copy-comparison JSON by excluding runtime-only fields', () => {
    const sourceJson = buildUnitGroupJsonOrdered(
      'source-id',
      '01.01.000',
      '2026-04-16T05:31:26.289Z',
    );
    const copiedJson = buildUnitGroupJsonOrdered(
      'copy-id',
      '01.01.000',
  ...
  it('treats missing empty objects and arrays as equivalent during copy comparison', () => {
    ...
    const sourceDataSetInformation = sourceJson.unitGroupDataSet.unitGroupInformation
  ...
  it('builds a workflow runtime record without persisting json/json_ordered', () => {
    const runtimeRecord = buildUnitGroupCreateViewCopyRuntimeRecord(
      {
        createDataFile:
          '/repo/tests/data-workflows/fixtures/data/unitgroups/001_create_view_copy.json',
  ...
  it('runs the create-view-copy workflow and writes a runtime record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'unitgroups-create-view-copy-'));
    ...
    try {
      ...
      await writeJson(createDataFile, {
        id: sourceId,
        table: 'unitgroups',
        jsonOrdered: buildUnitGroupJsonOrdered(sourceId, '01.01.000', '2026-04-16T05:31:26.289Z'),
      });
      ...
      const getUnitGroupDetail = jest.fn(async (id: string, version: string) => {
      ...
      const createUnitGroup = jest.fn(async (id: string, formData: any) => {
        const version =
          formData.administrativeInformation?.publicationAndOwnership?.['common:dataSetVersion'] ??
          '01.01.000';
        const shortName = deepClone(
          formData.unitGroupInformation?.dataSetInformation?.['common:shortName'] ?? {
            '#text': 'test-unitgroup',
            '@xml:lang': 'en',
          },
        );
        const jsonOrdered = buildUnitGroupJsonOrdered(
          id,
          version,
          '2026-04-19T00:00:00.000Z',
          shortName,
      ...
      const result = await runUnitGroupCreateViewCopySmoke(
        {
          createDataFile,
          createExpectedFile,
          followUpExpectedFile,
      ...
      expect(createUnitGroup).toHaveBeenCalledWith(
        copiedId,
        expect.objectContaining({
          administrativeInformation: expect.any(Object),
          unitGroupInformation: expect.any(Object),
        }),
      );
