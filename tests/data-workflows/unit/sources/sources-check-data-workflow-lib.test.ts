...
import {
  buildSourceCheckDataRuntimeRecord,
  buildValidationExpectationResults,
  getExpectedRuleVerification,
  getFixtureOrderedJson,
  parseCheckDataCliArgs,
  runSourceCheckDataSmoke,
  type SourceCheckDataFixture,
} from '../../workflows/sources/sources-check-data-workflow-lib';
import { buildCreateExpectations } from '../../workflows/sources/sources-create-workflow-lib';
...
describe('sources-check-data-workflow-lib', () => {
  ...
  it('prefers jsonOrdered and falls back to json_ordered then json in validation fixtures', () => {
    const withJsonOrdered: SourceCheckDataFixture = {
      json: { from: 'json' },
      json_ordered: { from: 'json_ordered' },
      jsonOrdered: { from: 'jsonOrdered' },
      table: 'sources',
    };
    const withOrderedJson: SourceCheckDataFixture = {
      json: { from: 'json' },
      json_ordered: { from: 'json_ordered' },
      table: 'sources',
    };
    const withJsonOnly: SourceCheckDataFixture = {
      json: { from: 'json' },
      table: 'sources',
    };
  ...
  it('builds a workflow runtime record without persisting json_ordered', () => {
    const runtimeRecord = buildSourceCheckDataRuntimeRecord(
      {
        checkDataFile:
  ...
  it('runs the create-then-update workflow against one source record and writes a runtime record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'sources-check-data-workflow-'));
...
`,
      ...
      const result = await runSourceCheckDataSmoke(
        {
          checkDataFile,
          checkExpectedFile,
          createDataFile,
      ...
      expect(result.runtimeFixture.runtimeId).toBe('fixture-source-id');
      expect(result.createStep.record.id).toBe('fixture-source-id');
      expect(result.updateStep.record.id).toBe('fixture-source-id');
      ...
      expect(
        functionsInvoke.mock.calls[1]?.[1]?.body?.jsonOrdered?.sourceDataSet?.sourceInformation
          ?.dataSetInformation?.['common:UUID'],
      ).toBe('fixture-source-id');
      expect(
        functionsInvoke.mock.calls[1]?.[1]?.body?.jsonOrdered?.sourceDataSet
      ...
      expect(writtenRuntimeRecord.create.runtimeId).toBe('fixture-source-id');
