...
import type { SourceCreateFixture } from '../../workflows/sources/sources-create-workflow-lib';
import {
  ...
  writeRuntimeRecord,
} from '../../workflows/sources/sources-create-workflow-lib';

describe('sources-create-workflow-lib', () => {
  it('parses cli arguments with defaults and explicit flags', () => {
    ...
    expect(options.dataFile).toBe(
      '/repo/tests/data-workflows/fixtures/data/sources/001_create.json',
    );
    expect(options.usersFile).toBe('/repo/tests/data-workflows/fixtures/data/users.json');
    expect(options.runtimeRecordFile).toBe(
      '/repo/tests/data-workflows/runtime/sources/001_create.last-run.json',
    );
  });
  ...
  it('uses an explicit runtime record path and enables writing when requested', () => {
    const options = parseCliArgs(
      [
        ...
        '--runtime-record-file',
        'output/custom-source-runtime.json',
      ],
      '/repo',
    );

    expect(options.writeRuntime).toBe(true);
    expect(options.runtimeRecordFile).toBe('/repo/output/custom-source-runtime.json');
  });
  ...
  it('loads users from .env.users.local before falling back to a users file', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'sources-create-env-file-'));
  ...
  it('prepares a runtime fixture with a generated id and updates the embedded uuid fields', () => {
    const fixture: SourceCreateFixture = {
      id: 'fixture-id',
      table: 'sources',
      jsonOrdered: {
        sourceDataSet: {
    ...
    expect(
      runtimeFixture.fixture.jsonOrdered.sourceDataSet.sourceInformation.dataSetInformation[
        'common:UUID'
      ],
    ).toBe('runtime-id');
    expect(
      runtimeFixture.fixture.jsonOrdered.sourceDataSet.administrativeInformation
        .publicationAndOwnership['common:permanentDataSetURI'],
    ).toBe(
      'https://lcdn.tiangong.earth/datasetdetail/source.xhtml?uuid=runtime-id&version=01.01.000',
    );
    expect(
      runtimeFixture.fixture.jsonOrdered.sourceDataSet.administrativeInformation.dataEntryBy[
        'common:referenceToDataSetFormat'
      ]['@refObjectId'],
    ).toBe('runtime-id');
    expect(
    ...
    ).toBe('../sources/runtime-id.xml');
    expect(runtimeFixture.sourceFixtureId).toBe('fixture-id');
  });
  ...
  it('derives the default runtime record path from tests/data-workflows/fixtures/data fixtures', () => {
    ...
    ).toBe('/repo/tests/data-workflows/runtime/sources/001_create.last-run.json');
  });
  ...
  it('writes a runtime record file without mutating the source fixture', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'sources-runtime-record-'));
    ...
    try {
      ...
      const runtimeRecord = buildRuntimeRecord(
        {
          dataFile: '/repo/tests/data-workflows/fixtures/data/sources/001_create.json',
          expectedFile: '/repo/tests/data-workflows/fixtures/result/sources/001_create.md',
