...
import {
  ...
  buildSourceCreateVersionUpdateReferenceRuntimeRecord,
  incrementDatasetVersion,
  parseCreateVersionUpdateReferenceCliArgs,
  runSourceCreateVersionUpdateReferenceSmoke,
} from '../../workflows/sources/sources-create-version-update-reference-workflow-lib';
...
function buildSourceUri(id: string) {
  return `../sources/${id}.xml`;
}

function buildSourceJsonOrdered(
  id: string,
  version: string,
  ownerVersion: string,
  ownerShortName: any,
...
function buildVersionComparisonFixture(id = '__RUNTIME_SOURCE_ID__') {
  return {
...
8. \`reviews\`为\`NULL\`
9. \`json_ordered\`中的\`common:referenceToDataSetFormat.@refObjectId\`和刚创建数据的\`id\`一致
10. \`json_ordered\`中的\`common:referenceToDataSetFormat.@uri\`和刚创建数据的\`id\`一致
11. \`json_ordered\`中的\`common:referenceToDataSetFormat.@version\`为\`01.01.000\`
...
17. \`json_ordered\`中的\`common:referenceToDataSetFormat.@version\`仍为\`01.01.000\`
...
21. \`json_ordered\`中的\`common:referenceToDataSetFormat.@version\`更新为\`01.01.001\`
22. \`json_ordered\`中的\`common:referenceToDataSetFormat.common:shortDescription\`和当前数据的\`common:shortName\`一致
`;

describe('sources-create-version-update-reference-workflow-lib', () => {
  ...
  it('builds a runtime record with version and update-reference summaries', () => {
    const runtimeRecord = buildSourceCreateVersionUpdateReferenceRuntimeRecord(
      {
        dataFile:
          '/repo/tests/data-workflows/fixtures/data/sources/004_create_version_update_reference.json',
        expectedFile:
          '/repo/tests/data-workflows/fixtures/result/sources/004_create_version_update_reference.md',
      ...
      },
      {
        ...
        referenceUpdate: {
          ...
          currentShortName: [{ lang: 'en', text: 'Version Source' }],
          ownerReference: {
            refObjectId: 'source-1',
            shortDescription: [{ lang: 'en', text: 'Version Source' }],
        ...
        versionCreation: {
          ...
          permanentDataSetUri:
            'https://lcdn.tiangong.earth/datasetdetail/source.xhtml?uuid=source-1&version=01.01.001',
        },
      },
    );
  ...
  it('runs the create-version-update-reference workflow on the same source id', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'sources-version-reference-'));
    ...
    try {
      ...
      const functionsInvoke = jest.fn(async (_functionName: string, request: any) => {
        if (_functionName === 'app_dataset_create') {
          const version =
            request.body.jsonOrdered.sourceDataSet.administrativeInformation
      ...
      const result = await runSourceCreateVersionUpdateReferenceSmoke(
        {
          dataFile,
          expectedFile,
          generateId: true,
        ...
        },
        {
          modulesLoader: async () =>
            ({
              sourcesUtil: {
                genSourceFromData: (sourceDataSet: any) => ({
                  administrativeInformation: deepClone(sourceDataSet.administrativeInformation),
                  sourceInformation: deepClone(sourceDataSet.sourceInformation),
                }),
                genSourceJsonOrdered: (id: string, data: any) => {
                  const version =
