...
import {
  ...
  buildSourceCreateContributeTeamRuntimeRecord,
  parseCreateContributeTeamCliArgs,
  resolveCreateContributeTeamRuntimeRecordFilePath,
  runSourceCreateContributeTeamSmoke,
} from '../../workflows/sources/sources-create-contribute-team-workflow-lib';
...
function buildSourceJsonOrdered(id: string, version: string, timestamp: string) {
  return {
    sourceDataSet: {
      sourceInformation: {
        dataSetInformation: {
...
1. 调用\`contributeSource("sources", id, version)\`成功
...
describe('sources-create-contribute-team-workflow-lib', () => {
  ...
  it('defaults role to team-member when --role is omitted', () => {
    ...
    expect(options.createDataFile).toBe(
      '/repo/tests/data-workflows/fixtures/data/sources/005_create_contribute_team.json',
    );
  ...
  it('builds code-owned workflow expectations and resolves the default runtime record path', () => {
    ...
    expect(
      resolveCreateContributeTeamRuntimeRecordFilePath(
        '/repo/tests/data-workflows/fixtures/result/sources/005_create_contribute_team.md',
        '/',
      ),
    ).toBe('/repo/tests/data-workflows/runtime/sources/005_create_contribute_team.last-run.json');
  });

  it('builds a workflow runtime record without persisting json_ordered', () => {
    const runtimeRecord = buildSourceCreateContributeTeamRuntimeRecord(
      {
        createDataFile:
          '/repo/tests/data-workflows/fixtures/data/sources/005_create_contribute_team.json',
        createExpectedFile:
          '/repo/tests/data-workflows/fixtures/result/sources/005_create_contribute_team_create.md',
  ...
  it('runs the create-contribute-team workflow and writes a runtime record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'sources-create-contribute-team-'));
    ...
    try {
      ...
      const result = await runSourceCreateContributeTeamSmoke(
        {
          createDataFile,
          createExpectedFile,
          followUpExpectedFile,
      ...
      expect(contributeSource).toHaveBeenCalledWith('sources', sourceId, '01.01.000');
