...
import {
  ...
  buildProcessCreateViewCopyRuntimeRecord,
...
} from '../../workflows/processes/processes-create-view-copy-workflow-lib';
...
1. 调用\`getProcessDetail(id, version)\`成功
...
15. 调用\`getProcessDetail(copyId, version)\`成功
16. 返回的\`id\`和复制后的数据\`id\`一致
17. 返回的\`version\`为\`01.01.000\`
18. 返回的\`json\`和数据库中的\`json\`一致
`;

describe('processes-create-view-copy-workflow-lib', () => {
  ...
  it('defaults role to user when --role is omitted', () => {
    ...
    expect(options.createDataFile).toBe(
      '/repo/tests/data-workflows/fixtures/data/processes/001_create_view_copy.json',
    );
    expect(options.role).toBe('user');
    expect(options.keepData).toBe(true);
    expect(options.generateId).toBe(true);
  ...
  it('builds code-owned workflow expectations and resolves the default runtime record path', () => {
    ...
    expect(
      resolveCreateViewCopyRuntimeRecordFilePath(
        '/repo/tests/data-workflows/fixtures/result/processes/001_create_view_copy.md',
        '/',
      ),
    ).toBe('/repo/tests/data-workflows/runtime/processes/001_create_view_copy.last-run.json');
  });

  it('normalizes copy-comparison JSON by excluding runtime-only fields', () => {
    const sourceJson = buildProcessJsonOrdered(
      'source-id',
      '01.01.000',
      '2026-04-16T05:31:26.289Z',
    );
    const copiedJson = buildProcessJsonOrdered('copy-id', '01.01.000', '2026-04-19T00:00:00.000Z');
  ...
  it('treats missing empty objects and arrays as equivalent during copy comparison', () => {
    ...
    const copiedJson = buildProcessJsonOrdered('copy-id', '01.01.000', '2026-04-19T00:00:00.000Z');
    const flowInfo = sourceJson.processDataSet.processInformation.dataSetInformation as Record<
      string,
      unknown
    >;
  ...
  it('builds a workflow runtime record without persisting json/json_ordered', () => {
    const runtimeRecord = buildProcessCreateViewCopyRuntimeRecord(
      {
        createDataFile:
          '/repo/tests/data-workflows/fixtures/data/processes/001_create_view_copy.json',
  ...
  it('runs the create-view-copy workflow and writes a runtime record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'processes-create-view-copy-'));
    ...
    try {
      ...
      await writeJson(createDataFile, {
        id: sourceId,
        table: 'processes',
        jsonOrdered: buildProcessJsonOrdered(sourceId, '01.01.000', '2026-04-16T05:31:26.289Z'),
      });
      ...
      const getProcessDetail = jest.fn(async (id: string, version: string) => {
      ...
      const createProcess = jest.fn(async (id: string, formData: any) => {
        const version =
          formData.administrativeInformation?.publicationAndOwnership?.['common:dataSetVersion'] ??
          '01.01.000';
        const baseName = deepClone(
          formData.processInformation?.dataSetInformation?.name?.baseName ?? {
            '#text': 'test-process',
            '@xml:lang': 'en',
          },
        );
        const jsonOrdered = buildProcessJsonOrdered(
          id,
          version,
      ...
      const result = await runProcessCreateViewCopySmoke(
        {
          createDataFile,
          createExpectedFile,
          followUpExpectedFile,
      ...
      expect(getProcessDetail).toHaveBeenCalledWith(sourceId, '01.01.000');
      expect(createProcess).toHaveBeenCalledWith(
        copiedId,
        expect.objectContaining({
          administrativeInformation: expect.any(Object),
          processInformation: expect.any(Object),
        }),
      );
