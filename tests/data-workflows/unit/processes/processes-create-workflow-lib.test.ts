...
import type { ProcessCreateFixture } from '../../workflows/processes/processes-create-workflow-lib';
import {
  ...
  writeRuntimeRecord,
} from '../../workflows/processes/processes-create-workflow-lib';

describe('processes-create-workflow-lib', () => {
  it('parses cli arguments with defaults and explicit flags', () => {
    ...
    expect(options.runtimeRecordFile).toBe(
      '/repo/tests/data-workflows/runtime/processes/001_create.last-run.json',
    );
  });
  ...
  it('uses an explicit runtime record path and enables writing when requested', () => {
    const options = parseCliArgs(
      [
        ...
        '--runtime-record-file',
        'output/custom-process-runtime.json',
      ],
      '/repo',
    );

    expect(options.writeRuntime).toBe(true);
    expect(options.runtimeRecordFile).toBe('/repo/output/custom-process-runtime.json');
  });
  ...
  it('loads users from .env.users.local before falling back to a users file', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'processes-create-env-file-'));
  ...
  it('prepares a runtime fixture with a generated id and updates the embedded uuid fields', () => {
    const fixture: ProcessCreateFixture = {
      id: 'fixture-id',
      table: 'processes',
      jsonOrdered: {
        processDataSet: {
    ...
    expect(
      runtimeFixture.fixture.jsonOrdered.processDataSet.processInformation.dataSetInformation[
        'common:UUID'
      ],
    ).toBe('runtime-id');
    expect(
      runtimeFixture.fixture.jsonOrdered.processDataSet.administrativeInformation
        .publicationAndOwnership['common:permanentDataSetURI'],
    ).toBe(
      'https://lcdn.tiangong.earth/datasetdetail/process.xhtml?uuid=runtime-id&version=01.01.000',
    );
  ...
  it('derives the default runtime record path from tests/data-workflows/fixtures/data fixtures', () => {
    ...
    ).toBe('/repo/tests/data-workflows/runtime/processes/001_create.last-run.json');
  });
  ...
  it('writes a runtime record file without mutating the source fixture', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'processes-runtime-record-'));
    ...
    try {
      ...
      const runtimeRecord = buildRuntimeRecord(
        {
          dataFile: '/repo/tests/data-workflows/fixtures/data/processes/001_create.json',
          expectedFile: '/repo/tests/data-workflows/fixtures/result/processes/001_create.md',
