...
import {
  buildProcessCheckDataRuntimeRecord,
  buildValidationExpectationResults,
  getExpectedRuleVerification,
  getFixtureOrderedJson,
  parseCheckDataCliArgs,
  runProcessCheckDataSmoke,
  type ProcessCheckDataFixture,
} from '../../workflows/processes/processes-check-data-workflow-lib';
import { buildCreateExpectations } from '../../workflows/processes/processes-create-workflow-lib';
...
describe('processes-check-data-workflow-lib', () => {
  ...
  it('prefers jsonOrdered and falls back to json_ordered then json in validation fixtures', () => {
    const withJsonOrdered: ProcessCheckDataFixture = {
      json: { from: 'json' },
      json_ordered: { from: 'json_ordered' },
      jsonOrdered: { from: 'jsonOrdered' },
      table: 'processes',
    };
    const withOrderedJson: ProcessCheckDataFixture = {
      json: { from: 'json' },
      json_ordered: { from: 'json_ordered' },
      table: 'processes',
    };
    const withJsonOnly: ProcessCheckDataFixture = {
      json: { from: 'json' },
      table: 'processes',
    };
  ...
  it('builds a workflow runtime record without persisting json_ordered', () => {
    const runtimeRecord = buildProcessCheckDataRuntimeRecord(
      {
        checkDataFile:
          '/repo/tests/data-workflows/fixtures/data/processes/002_check_data_success.json',
        checkExpectedFile:
          '/repo/tests/data-workflows/fixtures/result/processes/002_check_data_success.md',
    ...
    expect(runtimeRecord.create.runtimeId).toBe('process-1');
  ...
  it('runs the create-then-update workflow against one process record and writes a runtime record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'processes-check-data-workflow-'));
      ...
      await writeJson(createDataFile, {
        ...
        jsonOrdered: {
          processDataSet: {
            processInformation: {
              dataSetInformation: {
                'common:UUID': 'fixture-process-id',
              },
            },
            administrativeInformation: {
              publicationAndOwnership: {
                ...
                'common:permanentDataSetURI':
                  'https://lcdn.tiangong.earth/datasetdetail/process.xhtml?uuid=fixture-process-id&version=01.01.000',
              },
            },
          },
        },
...
`,
      ...
      const result = await runProcessCheckDataSmoke(
        {
          checkDataFile,
          checkExpectedFile,
          createDataFile,
      ...
      expect(result.runtimeFixture.runtimeId).toBe('fixture-process-id');
      expect(result.createStep.record.id).toBe('fixture-process-id');
      expect(result.updateStep.record.id).toBe('fixture-process-id');
      ...
      expect(
        functionsInvoke.mock.calls[1]?.[1]?.body?.jsonOrdered?.processDataSet?.processInformation
          ?.dataSetInformation?.['common:UUID'],
      ).toBe('fixture-process-id');
      expect(
        functionsInvoke.mock.calls[1]?.[1]?.body?.jsonOrdered?.processDataSet
      ...
      expect(writtenRuntimeRecord.create.runtimeId).toBe('fixture-process-id');
