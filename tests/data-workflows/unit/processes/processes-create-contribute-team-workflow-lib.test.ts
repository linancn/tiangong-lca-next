...
import {
  ...
  buildProcessCreateContributeTeamRuntimeRecord,
  parseCreateContributeTeamCliArgs,
  resolveCreateContributeTeamRuntimeRecordFilePath,
  runProcessCreateContributeTeamSmoke,
} from '../../workflows/processes/processes-create-contribute-team-workflow-lib';
...
function buildProcessJsonOrdered(id: string, version: string, timestamp: string) {
  return {
    processDataSet: {
      processInformation: {
        dataSetInformation: {
...
1. 调用\`contributeSource("processes", id, version)\`成功
...
describe('processes-create-contribute-team-workflow-lib', () => {
  ...
  it('defaults role to team-member when --role is omitted', () => {
    ...
    expect(options.createDataFile).toBe(
      '/repo/tests/data-workflows/fixtures/data/processes/005_create_contribute_team.json',
    );
  ...
  it('builds code-owned workflow expectations and resolves the default runtime record path', () => {
    ...
    expect(
      resolveCreateContributeTeamRuntimeRecordFilePath(
        '/repo/tests/data-workflows/fixtures/result/processes/005_create_contribute_team.md',
        '/',
      ),
    ).toBe('/repo/tests/data-workflows/runtime/processes/005_create_contribute_team.last-run.json');
  });

  it('builds a workflow runtime record without persisting json_ordered', () => {
    const runtimeRecord = buildProcessCreateContributeTeamRuntimeRecord(
      {
        createDataFile:
          '/repo/tests/data-workflows/fixtures/data/processes/005_create_contribute_team.json',
        createExpectedFile:
          '/repo/tests/data-workflows/fixtures/result/processes/005_create_contribute_team_create.md',
  ...
  it('runs the create-contribute-team workflow and writes a runtime record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'processes-create-contribute-team-'));

    try {
      const flowId = '96926fa6-658a-465e-8f2e-022259ef9e6d';
      ...
      await writeJson(createDataFile, {
        id: flowId,
        table: 'processes',
        jsonOrdered: buildProcessJsonOrdered(flowId, '01.01.000', '2026-04-16T05:31:26.289Z'),
      });
      ...
      const result = await runProcessCreateContributeTeamSmoke(
        {
          createDataFile,
          createExpectedFile,
          followUpExpectedFile,
      ...
      expect(result.runtimeFixture.runtimeId).toBe(flowId);
      expect(result.createStep.record.id).toBe(flowId);
      expect(result.contribute.currentTeamId).toBe('team-123');
      expect(result.contribute.commandSucceeded).toBe(true);
      expect(result.contributeStep.record.id).toBe(flowId);
      ...
      expect(contributeSource).toHaveBeenCalledWith('processes', flowId, '01.01.000');
      expect(state.deletedKeys).toEqual([`${flowId}|01.01.000`]);
      expect(signOut).toHaveBeenCalled();

      const runtimeRecord = JSON.parse(await readFile(runtimeRecordFile, 'utf8'));
      expect(runtimeRecord.create.runtimeId).toBe(flowId);
      expect(runtimeRecord.contribute.summary.currentTeamId).toBe('team-123');
