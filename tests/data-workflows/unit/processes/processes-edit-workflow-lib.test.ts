...
import {
  ...
  buildProcessEditRuntimeRecord,
  buildValidationExpectations,
  parseEditCliArgs,
  runProcessEditSmoke,
} from '../../workflows/processes/processes-edit-workflow-lib';
...
describe('processes-edit-workflow-lib', () => {
  ...
  it('builds a runtime record with success and failing edit summaries', () => {
    const runtimeRecord = buildProcessEditRuntimeRecord(
      {
        ...
        editDataFile:
          '/repo/tests/data-workflows/fixtures/data/processes/003_edit_data_validate_false.json',
        editExpectedFile:
          '/repo/tests/data-workflows/fixtures/result/processes/003_edit_data_validate_false.md',
  ...
  it('runs the create-success-fail edit workflow on a single process record', async () => {
    const tempRoot = await mkdtemp(path.join(os.tmpdir(), 'processes-edit-workflow-'));
...
`,
      ...
      const result = await runProcessEditSmoke(
        {
          createDataFile,
          createExpectedFile,
          editDataFile,
      ...
      expect(result.successEditStep.record.id).toBe('fixture-process-id');
      expect(result.failingEditStep.record.id).toBe('fixture-process-id');
      ...
      expect(
        functionsInvoke.mock.calls[2]?.[1]?.body?.jsonOrdered?.processDataSet?.processInformation
          ?.dataSetInformation?.name?.baseName?.['#text'],
      ).toBe('updated-process-fail');
