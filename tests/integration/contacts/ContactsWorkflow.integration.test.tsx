...
describe('Contacts workflow', () => {
  ...
  it('supports create, edit, delete, and search workflows', async () => {
    ...
    // Edit
    const newContactRow = screen.getByTestId(`pro-table-row-${createdContact.id}`);
    ...
    // Delete
    const updatedRow = screen.getByTestId(`pro-table-row-${createdContact.id}`);
    const deleteButton = within(updatedRow).getByRole('button', { name: 'Delete' });
    await user.click(deleteButton);

    const confirmModal = await screen.findByRole('alertdialog', { name: 'Delete Contact' });
