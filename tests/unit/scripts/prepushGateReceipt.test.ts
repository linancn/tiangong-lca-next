...
describe('bounded checked-push transport receipt', () => {
  ...
  it('keeps gate and retry activation inside repo-owned entrypoints', () => {
    ...
    expect(packageJson.scripts['test:coverage:collect']).toBe(
      'cross-env NODE_OPTIONS=--max-old-space-size=8192 node scripts/test-runner.cjs --stage coverage --maxWorkers=2 --workerIdleMemoryLimit=64MB --testTimeout=20000 --coverage --testPathIgnorePatterns="<rootDir>/tests/unit/scripts/prepushGateReceipt[.]test[.]ts$"',
    );
