...
describe('bounded checked-push transport receipt', () => {
  ...
  it('uses an already-active Node 24.20.0 without requiring an NVM-managed install', () => {
  ...
  it('fails clearly when neither PATH nor NVM can provide Node 24.20.0', () => {
    const current = fixture();
    const fakeHome = path.join(current.container, 'home');
    ...
    expect(result.stderr).toContain('Node.js 24.20.0 is required for the pre-push gate.');
