...
describe('docs screenshot capture contracts', () => {
  ...
  test('requires every capture to declare the shared browser locale', () => {
    const root = path.resolve('/tmp/docs-screenshot-target');
    const invalid = validPlan();
    delete (invalid.captures[0] as Partial<(typeof invalid.captures)[number]>).locale;
    expect(() => validateVisualPlan(invalid, [root])).toThrow(
      'captures[0].locale must be a non-empty string',
    );
  });
