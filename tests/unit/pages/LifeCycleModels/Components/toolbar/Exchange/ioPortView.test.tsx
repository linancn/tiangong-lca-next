...
const mockGetFolwypeOfDataSetOptions = jest.fn((value: string) => `type:${value}`);
...
jest.mock('@/pages/LifeCycleModels/Components/toolbar/Exchange/ioPortSelect', () => ({
  __esModule: true,
  getFolwypeOfDataSetOptions: (...args: any[]) => mockGetFolwypeOfDataSetOptions(...args),
}));
...
describe('LifeCycleModelIoPortView', () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it('loads process exchanges and renders lifecycle-specific columns when the drawer is opened', async () => {
    ...
    expect(mockGetProcessExchange).toHaveBeenCalledWith('table-data', 'Input', {
      current: 1,
      pageSize: 10,
    });
