// @ts-nocheck
import SourceCreate from '@/pages/Sources/Components/create';
      ...
      return (
        <div>
          ...
          <button
            type='button'
            onClick={() => {
              setFileList?.([{ uid: 'file-existing', name: 'existing.pdf' }]);
              setLoadFiles?.([{ uid: 'file-existing', name: 'existing.pdf' }]);
            }}
          >
            mock-file
          </button>
...
jest.mock('@/services/supabase/storage', () => ({
  __esModule: true,
  getThumbFileUrls: jest.fn(() => Promise.resolve([])),
...
describe('SourceCreate component', () => {
  ...
  it('submits new source and triggers reload', async () => {
    ...
    await user.type(shortNameInput, 'My Source');
