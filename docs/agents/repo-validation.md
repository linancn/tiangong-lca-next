...
lastReviewedNote: 'Reviewed for Issue #630 against v0.0.50: route-view localization proof now preserves the anonymous login boundary and validates protected views under their existing access context.'
related:
  - ../AGENTS.md
  - ../.docpact/config.yaml
...
| locale bundles, message IDs, or localized runtime copy | `npm run i18n:audit`; `npm run i18n:locale:audit -- --locale <canonical-locale>`; `npm run i18n:context:check -- --locale <canonical-locale>`; `npm run i18n:locale:quality:check -- --locale <canonical-locale>`; `npm run i18n:corrections:check`; `npm run i18n:locale:activation:check -- --locale <canonical-locale>`; focused locale/runtime tests; `npm run lint`; `npm run build` | `npm run prepush:gate`; route-view browser smoke for selector, persistence, framework copy, declared fallbacks, route/static views under their existing access context, and long-text layout | every registry locale must match the canonical topology/key/ICU contract; dynamic IDs require an audited family and unknown fallback; route-view/context manifests must have zero blocked or unowned content and must not grant anonymous access; active commands never read translation confirmation files |
...
Treat `npm run i18n:audit` as the deterministic structural proof for locale ownership and runtime message references. For autonomous language delivery, pair it with the registry-driven context, quality, correction, route-view, and activation checks; these tracked gates prove the configured automated quality model without creating a human translation-approval state.
