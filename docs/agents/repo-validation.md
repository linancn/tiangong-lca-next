...
An authenticated browser rerun may be avoided only for exact non-browser-semantic release-harness changes recorded in `docs/plans/i18n/semantic-e2e-digest-compatibility.json`. Each entry binds one evidence commit, one evidence digest, one reviewed compatible digest, an owner Issue, proof commands, and the sunset condition `next-verified-evidence-for-compatible-sha`. The production check accepts only the exact pair; changing that file again, changing any unlisted test/source/runtime input, or replacing the verified evidence fails closed. Never use this mechanism for route, browser assertion, backend, source-copy, runtime-asset, package dependency, authorization, production-data, or cleanup behavior.
