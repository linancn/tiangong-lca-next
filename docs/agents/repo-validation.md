...
| Data Product closure/task failure presentation | `npm run lint`; focused Data Processing, global Task Center, data-product task decoder, and locale tests; `npm run i18n:audit`; `npm run build` | `npm run prepush:gate`; smoke a failed and a blocked closure check against a matching non-production Database/Edge/Worker deployment | execution failure must not render as an empty closure-issue result; expose only the safe task-summary error, stable error code, and job identity, while passed/blocked checks retain the curated paginated issue list |
