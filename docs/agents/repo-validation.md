...
lastReviewedNote: 'Reviewed the staged German local Markdown confirmation, reviewer dossier, scope/body digests, evidence diagnosis, and full-catalog enforcement workflow.'
related:
  - ../AGENTS.md
  - ../.docpact/config.yaml
...
| staged German context, pilot, or pre-activation leaf translation work | refresh with `npm run i18n:de:audit:write` then `npm run i18n:de:pilot:write`; generate/check the ignored pilot form with `npm run i18n:de:review:generate` and `npm run i18n:de:review:check`; after the complete catalog exists, use `npm run i18n:de:review:catalog:generate` and `npm run i18n:de:review:catalog:check`; inspect with candidate report and `npm run i18n:de:pilot:report`; also run the locale-bundle baseline above | final `npm run i18n:de:pilot` and `npm run i18n:de:audit` only after the applicable local confirmation and all 30 leaf modules exist | completed forms never enter Git/GitHub; repository-local forms outside the ignored private directory are rejected; canonical body binding prevents self-hashed sparse forms; pilot approval covers only 90 messages; human approval never clears invalid/missing context proposals; Issue #601 must not activate `de-DE` |
