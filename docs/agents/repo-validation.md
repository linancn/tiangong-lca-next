...
| unified seven-type Root/Reference Review UI | `npm run tsc`; focused ESLint for changed pages/services/locales; `npx jest tests/unit/services/reviews/api.test.ts tests/unit/components/DataNotification.test.tsx --runInBand --no-watchman`; `npm run build` | final `npm run prepush:gate`; smoke all seven edit-page submit actions plus Admin/Member queues against matching Database and Edge revisions | Process alone keeps the asynchronous numeric Gate. Verify the other six types send no Gate fields, simple approval has no opinion, rejection requires a reason, Root rows expand to Reference rows, Reviewer rejections count as completed opinions, dataset detail pages do not gain rejection displays, and owner notifications route to the correct edit page. |
...
A passing feature-branch or `dev`-relative Docpact gate proves only that configured comparison range. Before a `dev -> main` promotion, check out the intended candidate head and run `DOCPACT_BASE_REF=origin/main npm run docpact:gate`; the main-target PR Release Gate checks that complete base/head range before merge, and the post-merge Release Gate verifies the exact release commit again. If that range reports `missing-review`, genuinely review and update every required governed document before promotion; do not hide active findings with a baseline, waiver, or narrower comparison base.
