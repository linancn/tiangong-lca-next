...
lastReviewedNote: 'Reviewed the staged German onboarding, reviewer-dossier, four-hash queue, detailed evidence-diagnosis, and full-catalog enforcement workflow.'
related:
  - ../AGENTS.md
  - ../.docpact/config.yaml
...
| staged German context, pilot, or pre-activation leaf translation work | onboard available reviewer identities with `npm run i18n:de:review:onboard -- <options>`; refresh with `npm run i18n:de:audit:write` then `npm run i18n:de:pilot:write`; inspect with `node scripts/i18n/audit-german-candidate.mjs --mode report --check` and `npm run i18n:de:pilot:report`; also run the locale-bundle baseline above | final `npm run i18n:de:pilot` and `npm run i18n:de:audit` only after required named reviews and all 30 leaf modules exist | onboarding never posts comments or decisions; report mode must preserve honest blockers; enforce mode intentionally fails while context, independent native-German review, domain review, or translations are incomplete; Issue #601 must not activate `de-DE` |
...
The local `pre-push` hook always runs `npm run docpact:gate` first, then runs the full `npm run prepush:gate` local test gate on every branch. The docpact gate defaults to `origin/dev` and can be redirected with `DOCPACT_BASE_REF=<ref>` for promote or hotfix branches.
...
For the staged German workflow, regenerate in dependency order: canonical manifest, German context ledger, then pilot review pack. Reviewer onboarding stores a pending roster first and changes all roles to assigned only after the three maintainer-authored Issue comments verify; the tool never posts those comments. `i18n:de:pilot` first verifies ledger freshness and pins approvals to `contextHash`, `translationHash`, `dossierHash`, `reviewScopeHash`, and a canonical producer actor; the generated pack provides three blank role-specific queues but never supplies human decisions. GitHub humans are keyed by API-verified immutable numeric user ID rather than a login/display-name alias. The final candidate gate requires the approved pilot and independently verifies production plus native/domain evidence for every candidate. Issue #601 assignment and scope attestations bind all review-gate policy sources and the declared policy; changed source context, policy implementation, producer, dossier, pilot risk scope, or German copy invalidates the applicable evidence. Reserved messages without runtime evidence remain `BLOCKED_CONTEXT` until an assigned human product/domain reviewer records concrete hash-pinned evidence. Do not weaken counts or use AI/arbitrary local identities to make report-mode findings pass.
