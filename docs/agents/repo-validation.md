...
| source-bound docs capture profile, login/identity test IDs, runtime/readiness contract, or access-denied marker | `npm run docs:capture-profile:test`; focused `npm run lint`; `npm run build` when shipped markup changes | workspace v2 compiler tests plus one authorized end-to-end canary capture, privacy review, and paired `next-docs` asset validation | Next owns only facts that must evolve with the rendered commit. The workspace engine owns plan/result/access schemas, credentials, read-only enforcement, dynamic origin, and Draft/evidence policy. A missing or incompatible profile is a blocker, never verified access denial. |
