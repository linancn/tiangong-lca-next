...
| staged German context, pilot, or pre-activation leaf translation work | refresh with `npm run i18n:de:audit:write` then `npm run i18n:de:pilot:write`; inspect with `node scripts/i18n/audit-german-candidate.mjs --mode report --check` and `npm run i18n:de:pilot:report`; also run the locale-bundle baseline above | final `npm run i18n:de:pilot` and `npm run i18n:de:audit` only after required named reviews and all 30 leaf modules exist | report mode must preserve honest blockers; enforce mode intentionally fails while context, independent native-German review, domain review, or translations are incomplete; Issue #601 must not activate `de-DE` |
