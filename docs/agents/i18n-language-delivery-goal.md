---
title: TianGong Next 新增单一产品语言——最终交付 Goal
docType: goal
scope: repo
status: template
authoritative: false
...
whenToUpdate:
  ...
  - when public or static routes, query-driven views, or their user-visible copy ownership changes
...
# TianGong Next 新增单一产品语言——最终交付 Goal

## 0. Goal 指令

本 Goal 用于把一种新语言完整交付到 TianGong Next：从 locale 契约、上下文建模、全量翻译、公共/静态页面及其全部 URL 状态、已有语言质量修订、运行时接入、测试、`dev` 集成，一直推进到 `main`、生产发布和根 workspace 精确指针集成。
...
> 以本文件为完整执行目标。对所有翻译和既有译文修订进行自主、上下文完备的判断；把公共/静态路由、query/hash/path 驱动的每个可见视图状态和组件内文案一并纳入翻译范围；不得创建或等待任何人工翻译审阅。持续推进到真正 production-effective action 之前的唯一最终发布确认点。确认后自主执行该动作及剩余的发布监控、生产 smoke、根 workspace 集成和清理。复用所有仍有效的检查点，不重复翻译、验证、门禁或远端工作。
...
| 数据/服务 fallback | `<DATA_FALLBACK_LOCALE>` | UI 语言不等于 TIDAS/数据集内容语言 |
...
| Next 可执行 Issue | `<NEXT_ISSUE_URL_OR_ID>` | 一个完整 Issue 通常足够；不存在时可填 `CREATE_IF_MISSING` |
...
| smoke matrix | `<SMOKE_ROLE_MATRIX>` | 匿名、共享登录路径、真实角色分支及凭据可用性 |
...
### 2.5 公共/静态页面与 URL 状态覆盖合同

“静态页面”不表示可以跳过 i18n。所有无需登录即可访问的公共、欢迎、介绍、指南、帮助和 onboarding 页面，以及登录后以固定内容为主的页面，都属于全量翻译范围。同一个 React 页面只要因 URL 或交互状态显示不同内容，就必须按不同 view state 独立建模、翻译和验证。
...
观察基线至少包含以下三个入口/状态；未来发现的其他静态路由或 view 参数必须自动追加，不能把此列表当作封闭白名单：

- `/` redirect 后的最终页面；
- `/welcome` 的默认 overview；
- `/welcome?view=carbon-footprint` 的 carbon-footprint guide。
...
观察基线 `v0.0.49@2a523ea5` 有 `zh-CN`、`en-US`、`de-DE` 三种活跃语言，每种 30 个 leaf modules、2,737 条消息，并包含 17 个 dynamic families/39 个动态调用点。当前选择器精确显示 `🇨🇳 简体中文`、`🇺🇸 English`、`🇩🇪 Deutsch`。未来执行时必须重新生成当前数字和菜单项，不得硬编码历史规模。

### 5.2 必查入口

- `src/locales/<locale>.ts` 与同名 leaf modules；
- `config/routes.ts`、redirect、页面内 `history.push`/link 和 URL 参数解析；
- `src/pages/Welcome.tsx`、`src/locales/*/pages_home.ts`，以及 route-view matrix 发现的其他公共/静态页面；
- `src/services/general/runtimeLocale.ts`；
...
### 5.3 单一 typed locale registry

新增下一种语言时，把以下事实收敛到一个 typed registry：

- canonical locale；
- native label；只在 Umi 默认 label 含国家/地区限定或不符合产品原生名称时做最小规范化；
- aliases；
- writing direction；
- Ant Design/Day.js/Intl adapter；
- docs/legal/data/service fallback；
- environment title/subtitle key；
- import/export/report adapter。

`SupportedAppLocale`、选择器顺序、alias normalization、文档链接、格式化和服务分支都从 registry 派生。选择器国旗继续由 Umi locale metadata 所有，不在 registry 中复制第二份自定义图片或 emoji 映射。不得继续复制 `if (locale === 'de-DE')` 或新的语言专用特判。
...
实际命令以实现后的 `package.json` 为准。共享 inventory、dynamic-family registry 和 parser 只能有一个 source of truth；语言特有内容仅保留 glossary、style guide、必要 context override、locale files 和紧凑 quality/activation manifest。
...
- 登录/欢迎入口允许使用局部、可访问的紧凑 action frame，但不得改变下拉菜单的原生 locale icon/label 数据；
- 在未登录、登录/欢迎和登录后的共享 Header 中验证真实可见菜单、图标对齐、hover frame 和键盘交互。

### 5.7 fallback contract

为文档、法律、数据集、服务错误、导入/导出 schema、环境标题和报告下载生成可观察表：
...
`/welcome` 与 `/welcome?view=carbon-footprint` 即使复用同一组件，也必须是两个独立 coverage rows。只验证 `pages.welcome.*` key parity 不足以证明完成：组件本地 content map、媒体状态、动作文案和辅助技术文本也必须进入同一审计。新增语言实现完成后，任何用户可达的静态状态都不得出现未声明的 English/Chinese fallback 或只在目标 locale 下缺失的内容分支。
...
| 检查点 | 完成证据 | 失效时只重做 |
| --- | --- | --- |
| C0 输入/契约 | 参数、registry decision digest | 契约影响范围 |
| C1 source/context | source SHA、message inventory、route-view matrix、context schema/digest | source/route delta/context 闭包 |
| C2 autonomous catalog | target digest、零 blocked、quality report | 变化 message/规则闭包 |
| C3 existing-locale repair | correction ledger/digest | 新发现或变化闭包 |
| C4 runtime activation | activation manifest、focused proof | 运行时影响范围 |
| C5 delivery freeze | immutable SHA、clean proof、full gate | 受控 tracked 变化 |
| C6 dev delivery | feature PR/dev SHA | 远端实际未完成阶段 |
...
4. fetch/prune 后查询同 locale 的 Project、Issue、branch、PR、version、tag、release、deployment 和 root integration。
5. 去重键按阶段补齐，最终为 `(locale, issue, sourceSha, catalogDigest, deliveryTree, version, rootTargetSha)`。
...
9. pin source SHA 和 English/Chinese/current-locale inventory，不让浮动 `origin/dev` 反复重写基线。
10. 更新 Project/Issue 到实际开始状态，不请求人工开始确认。

阶段输出：唯一任务记录、基线 SHA/digest、分支/发布模型、已完成检查点和去重结果。

### 阶段 B：共享 i18n 能力最小通用化

1. 建立 typed locale registry，并迁移可复用 German 特判。
2. 让 locale audit/tests 从 registry 发现语言，而不是硬编码三语言或 German 路径。
3. 提取新语言实际复用的 manifest、context、quality 和 activation 公共能力。
...
6. 证明新语言和 active German 的 context/quality/activation/full-gate 命令都不读取 `.local/**confirmation*`；历史 checker 只能在 frozen-snapshot 模式中使用。
...
2. 从 routes、redirect、页面导航和 URL/交互状态分支生成第 2.5 节的 route-view coverage matrix；观察基线必须明确包含 `/`、`/welcome` 和 `/welcome?view=carbon-footprint`。
...
8. 发现 source locale 自身问题时，只有证据明确才直接修复；当前 audit 干净时不重复全仓 cleanup。
9. 标记并通过进一步研究消除 `BLOCKED_CONTEXT`；缺失 route/view coverage 同样视为上下文未完成。
...
5. 每个 batch 运行 exact key、ICU、token、glossary、context、dynamic-family 和 route-view coverage checks。
6. 高风险 batch 由不同自动化 reviewer 做独立证据复核；分歧回到 callsite/术语证据解决。
7. 交叉检查相邻状态、成功/失败、按钮/标题、单复数、页面不同 view 和模块间术语。
8. source baseline 之后的新消息、静态内容或 route/view state 形成自动 delta，只翻译和验证变化闭包。
9. 达到第 3.3 节全部自动质量门槛。
...
1. 在阶段 C/D 的跨语言对照中识别已有译文异常。
2. 按第 4.2 节判断是否证据明确。
3. 对明确问题直接修改对应 Chinese/German/其他 locale；German post-baseline change 必须写入第 5.5 节的自动 correction overlay，不得回到 private human delta review。
...
2. 将目标语言加入 registry，接入 Umi、Ant Design、Day.js、Intl、storage 和 service runtime。
3. 所有 `xx`/`xx-*`/underscore/POSIX aliases 归一到唯一 canonical locale。
4. 接入 docs/legal/data/service/import/export/environment fallback contract。
...
7. 对直接打开和语言切换后的 `/welcome`、`/welcome?view=carbon-footprint` 及所有发现的静态 view 验证 URL/view state 保持、文案切换和零未声明 fallback。
8. RTL 时完成 direction、logical layout、portal 和 bidi 接入。
9. 在 activation manifest 记录精确 CI-safe structure/activation/route-view/focused clean commands。
...
- route-view coverage audit，覆盖所有公共/静态 path、redirect 和 query/hash/path/交互状态；
...
- 匿名入口和一个代表性已登录 shared Header browser smoke；匿名静态页面至少逐项访问 `/`、`/welcome` 和 `/welcome?view=carbon-footprint`；
- 对 Welcome overview 与 carbon-footprint guide 验证标题、正文、动作、步骤/schema、modal、媒体 loading/error/fallback、语言切换及带 query 刷新；
- 对 route-view matrix 发现的其他静态页面执行同级 focused browser proof，不能只验证示例 URL；
...
2. 冻结 locale、已有语言修订、runtime、tests、docs 和 manifests。
3. 选择/确认唯一 package version，只 bump 一次。
4. 最后一次生成 source/route-view/quality/correction/activation manifests，并运行 exact focused checks。
...
1. 创建/更新 feature PR 到当前 routine target（观察模型通常是 `dev`），关联 Issue。
2. PR 说明目标 locale、context contract、自动质量模型、已有语言修订、runtime/fallback、验证和回滚。
...
4. required checks 通过后自主合并到 `dev`，不请求翻译或合并确认。
5. 执行 dev smoke；完整覆盖 route-view matrix，特别是 `/welcome` 与 `/welcome?view=carbon-footprint`；若角色走同一 selector 代码路径，只做代表性 smoke。
...
- 自动翻译质量门槛、已有语言 correction audit、runtime/browser smoke 已通过；
...
- 静态页面：<ROUTE/VIEW COUNT + `/welcome`/`carbon-footprint` PROOF>
...
7. 在生产执行目标语言选择、持久化、formatting、fallback 和 Umi 原生国旗 selector smoke；按冻结的 route-view matrix 逐项验证公共/静态页面，至少包含 `/` redirect、`/welcome` overview、`/welcome?view=carbon-footprint` guide 及其关键正常/加载/失败/fallback 状态。
...
| 公共/静态 route、query view 或组件本地文案变化 | 更新 route-view matrix + 受影响状态翻译/浏览器 proof | 永不需要 | 冻结后一次 | 若 tuple 已确认且 tracked tree 变化则失效 |
...
- 不把 locale key/module parity 当成静态页面翻译完成；必须同时审计 route-view matrix、组件内 content 和条件状态。
...
- 不重复当前已干净的 English/Chinese canonical cleanup，除非 audit 发现真实 drift。
...
- 不只 smoke 一个 selector 入口；至少覆盖匿名和代表性登录共享路径。
...
- [ ] route-view coverage matrix 覆盖所有公共/静态路由、redirect 和 query/hash/path/交互状态；`/`、`/welcome`、`/welcome?view=carbon-footprint` 均有独立完整记录。
...
- [ ] Ant Design、Day.js、Intl、storage、service 和 environment adapter 正确。
- [ ] docs/legal/data/service/import/export fallback contract 全部通过。
- [ ] selector 在所有共享入口保留 Umi 原生国旗，并精确显示“国旗 + native label”；无国家/地区限定文本、地球 fallback 或自定义图片/SVG/emoji 映射。
- [ ] 登录后 Header 使用原生 Umi/ProLayout action 行为；登录/欢迎入口的局部 action frame 与相邻图标等高、对齐且可键盘操作。
- [ ] 切换、刷新、浏览器首选、旧缓存、未知 locale fallback 正确。
- [ ] 直接打开或切换语言后，所有静态页面的 path/query/view state 保持不变；Welcome overview 与 carbon-footprint guide 的正常、加载、失败/fallback 和关键交互均显示目标语言。
...
- [ ] active German 和新 locale 的 full-gate 路径都不读取 private confirmation；Next 治理文档和测试已反映该边界。
...
- 静态页面范围：<ROUTE COUNT> 条路由 / <VIEW STATE COUNT> 个状态 / <ROUTE-VIEW DIGEST>，含 `/welcome` 与 `/welcome?view=carbon-footprint`
...
- 如何发现并证明 `/welcome`、`/welcome?view=carbon-footprint` 及其他静态/query view 都已翻译，而不只依赖 locale key parity？
...
当且仅当第 11 节全部满足、唯一最终线上发布确认已执行、阶段 K 完成并生成最终报告，Goal 才能标记为 `complete`。
