...
lastReviewedNote: 'Bound the reusable Goal to the implemented UI/content/service/reference registry split, registry-complete activation evidence, and exact anti-hardcoding gate.'
...
4. 最后一次生成 source/route-view/quality/correction/capability/reference-resource/activation manifests，并运行 exact focused checks；随后运行 `npm run i18n:locale:all:production:check`，任何 owned blocker 都必须使最终 release gate 非零退出。
