...
lastReviewedNote: 'Expanded the reusable Goal to cover content languages, official-first classification/location localization, registry-derived capabilities, all-active-language backfill, and anti-hardcoding gates.'
...
npm run i18n:capabilities:audit
npm run i18n:reference-data:audit
```
...
CI 必须包含反硬编码门禁：扫描业务代码中的 locale/language 二元条件、固定 supported-locale union、手写语言下拉数组、缓存资产数组和“非 zh 即 en”归一逻辑。允许项必须落在最小 allowlist，并说明它是外部 adapter、canonical source 或历史冻结验证；新增语言后，不修改业务页面即可让 registry 驱动的测试发现其所有必需能力。
...
门禁输出精确文件和行号；allowlist 必须最小、可解释、可测试。新增 registry locale 后，catalog、内容能力、参考资源或参数化测试任一缺失都必须 fail closed。
...
| C4 runtime activation | activation manifest、capability/reference-data gates、focused proof | 运行时影响范围 |
...
4. 最后一次生成 source/route-view/quality/correction/capability/reference-resource/activation manifests，并运行 exact focused checks。
...
| registry/content capability 变化 | 重算全部 active locale 能力闭包、参数化 proof 和 hardcoding audit | 永不需要 | 冻结后一次 | 若 tuple 已确认则失效 |
...
- [ ] 同版官方译本优先；不存在时已有完整项目译本和独立自动证据复核，最终状态为 `project-reviewed`。
