...
lastReviewedCommit: 790d68165ea8f7103f0cff147c4808eca498e516
lastReviewedNote: 'Reviewed for Issue #670: the separate docs-impact screenshot capture profile is non-mutating and does not become i18n semantic E2E evidence; locale, route, production-data, and credential-free CI semantics remain unchanged.'
...
上述截图禁令只约束本 Goal 的 i18n semantic E2E 证据边界。`scripts/docs-screenshots/**` 与 `playwright.docs-capture.config.ts` 是独立的 docs-impact 只读截图入口，受其自身的无写请求、无 storage state、秘密文件隔离和产物隐私门禁约束；其 PNG 不得被复用为本 Goal 的 semantic E2E 或生产数据闭包证据。
