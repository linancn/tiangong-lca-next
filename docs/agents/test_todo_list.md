...
- semantic E2E GitHub Actions is credential-free/read-only: the exact release SHA requires the content-addressed hermetic qualification, including the complete Chromium matrix and Firefox/WebKit critical scenarios; the separate dev-server public matrix is optional through `workflow_dispatch` and absent from routine PR/dev/release triggers, while authenticated candidate-local/production-backend closure is restricted to an explicitly authorized local operator session with authenticated mode, both production-write guards, and an explicit verified-evidence opt-in
...
- exact marker-bound dev Release PRs run the aggregate reusable Release Gate against their exact base/head and emit a 30-day proof only after static/full tests and the hermetic semantic qualification succeed; generated candidate/promotion local pushes use restricted Docpact-plus-static profiles
