...
- deterministic release-command coverage proves JSON purity, non-mutating dry-run, static preflight before transport, rejection of unexpected untracked files, exact version fields, checked-push delegation, idempotent PR reuse, immutable promotion identity, independent version-candidate and cumulative `main`-to-candidate Docpact preflight, bounded automatic review, and the invariant that release-to-dev never runs browsers or writes tracked proof
