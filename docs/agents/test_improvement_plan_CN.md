...
- 当前全量运行规模：276 suites / 2350 tests
- 当前 Jest 全量覆盖率：
  - Statements: 91.48%
  - Branches: 78.17%
  - Functions: 87.36%
  - Lines: 91.78%
- 当前逐文件库存：追踪 300 个源码文件，已全满 115 个，仍有缺口 185 个。
- 当前 branch 分桶：`<50 = 2`、`50-70 = 46`、`70-90 = 101`、`90-<100 = 25`。
- `jest.config.cjs` 当前全局门槛：50%（branches/functions/lines/statements）。
- 眼下主要问题：**测试工程已经远超过“救门禁”阶段，当前战略重点变成了按有序文件队列持续收口，而剩余债务仍主要集中在页面层簇**。
...
- [x] 已将 branch 覆盖恢复到全局门槛以上，并建立更高的安全余量（当前 78.17%）。
...
- [ ] 先按严格队列顺序关掉当前仅剩的两个 `<50% branch` 文件：`src/components/TableFilter/index.tsx`，然后是 `src/pages/Unitgroups/Components/edit.tsx`。
- [ ] 再按队列顺序清理当前 `50%-70%` 桶，从 `docs/agents/test_todo_list.md` 的队头快照开始。
...
- [ ] 从当前 78.17% branch 基线继续往 100% 收口，方式是持续缩短有序队列，而不是临时改优先级。
