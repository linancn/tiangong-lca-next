...
- release-risk gates should shift left without weakening the final boundary: local pushes run static/full non-browser gates, and every main-target PR runs the aggregate static/full, content-addressed qualification, and public three-browser gate. A successful main-target PR emits exact base/head/tree/run proof; post-merge reuses it only for the matching two-parent unchanged-tree merge and otherwise reruns the aggregate gate
...
- each production release candidate should have one aggregate proof owner: the main-target PR gate (or post-merge fallback) runs `prepush:gate` once, semantic qualification once per behavior key, and the public browser workflow once. Immutable tag publication waits for that aggregate result
