...
lastReviewedNote: 'Reviewed for Issue #693: source-bound capture-profile proof remains separate from the workspace-owned generic screenshot engine and release matrices.'
---
...
- documentation screenshot capture is a separate, on-demand product-evidence workflow: Next tests only the source-bound product profile and stable locators, while workspace owns plan/security/access, run-scoped base-origin logic, and the synthetic Chromium canary outside the semantic localization proof and release matrix
