...
- release-risk gates should shift to the earliest immutable candidate without weakening the final boundary: the generated release-candidate push runs only structural/static proof, and the exact dev Release PR runs the aggregate static/full, content-addressed qualification, and public three-browser gate. A successful dev Release PR emits proof bound to the main/dev bases, candidate head/tree/version, PR, run attempt, and artifact; immutable promotion/main stages verify it instead of repeating the aggregate
...
- each production release candidate should have one aggregate proof owner: the exact dev Release PR runs `prepush:gate` once, semantic qualification once per behavior key, and the public browser workflow once. Normal main PR/push paths fail closed on absent or mismatched proof rather than creating a late fallback; explicit recovery events may run a fresh aggregate
