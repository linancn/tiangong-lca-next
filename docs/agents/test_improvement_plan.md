...
lastReviewedNote: 'Reviewed for Next Issue #819: the testing strategy preserves the full quality boundary while eliminating an exact duplicate post-merge execution.'
---
...
- each production release candidate should have one full-suite owner: `prepush:gate`, executed once by the main-target PR and reused through exact proof or executed once by the post-merge fallback, with at most one coverage worker active at a time; the reusable browser semantic E2E matrix remains a separate exact-release-SHA prerequisite without duplicating Jest coverage, and immutable tag publication follows both successful qualification paths
