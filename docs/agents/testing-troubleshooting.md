...
lastReviewedNote: 'Reviewed for Next Issue #867: troubleshooting distinguishes deployment-only drift from actual qualification input changes.'
---
...
| a canonical `main` release runs the full Release Gate even though its promotion PR passed | the exact proof was absent, expired, ambiguous, mismatched, unavailable, or the release commit was not the expected unchanged-tree two-parent merge | inspect the `Release Context` proof resolver JSON and `Release Qualification` summary for `gate_reason`; preserve the full-gate fallback, and repair proof production or merge policy in a new workflow PR instead of forcing reuse or bypassing qualification |
