...
| the deterministic dev Release PR returns `semantic_qualification_failed`, `semantic_qualification_scope_invalid`, or qualification provenance mismatch | no valid content-addressed proof matched and the fresh closed-simulator qualification failed or did not bind the exact candidate | inspect the Release Gate artifact and qualification report; fix the first candidate/harness failure on `dev`, then create a new patch candidate without hand-editing hashes or proof records |
...
| `release:proof:verify` rejects a restored qualification proof | the proof key, candidate inputs, environment contract, or closure result does not match the current checkout | let CI run a fresh qualification or rerun `e2e:qualify -- --proof <ignored-path>` locally; do not copy proof into tracked source or edit its hashes |
...
| the optional public semantic workflow reports missing login controls, boot-timeout maintenance redirects, detached menus, or lost URL state after cold startup | the dev server was still compiling/remounting routes while the strict public diagnostic ran; this workflow is not release proof | diagnose that dev-server behavior only when relevant, keep `failOnFlakyTests` enabled, and do not add a fixed sleep or make this diagnostic a release prerequisite; release acceptance comes from the prebuilt hermetic qualification |
...
| a deterministic dev Release PR fails content-addressed semantic qualification | no valid cached proof matched the behavior key, and the fresh closed-simulator browser run failed | inspect the semantic qualification controller report and browser logs, fix the candidate or harness on `dev`, and create a new patch candidate; version-only or deployment `.env` changes should keep the key, while source/public/qualification-config/runtime/test/shared-helper or Git mode/type drift intentionally creates a new key |
...
| a release gate fails but a matching new tag already exists | tag publication ran before qualification or the tag was created manually | stop publication and inspect the tag owner/SHA; never move an immutable release tag. The canonical workflow must create a missing tag only after exact Release Gate qualification and exact-SHA semantic E2E pass, and any corrected candidate uses a new patch version |
