...
| Jest exits non-zero without a failure or final summary and macOS writes a Node `.ips` report with `ClearStaleLeftTrimmedPointerVisitor` | native Node/V8 GC crash, not a Jest assertion failure; if it reappears under the qualified two-worker pool, concurrency versus Node/V8 regression must be discriminated | confirm the crash signature once; keep `prepushGateReceipt.test.ts` in its repo-owned no-coverage process and retain the `64MB` recycle boundary, then run one bounded `--maxWorkers=1` diagnostic on the same Node/checkpoint before changing the checked-in two-worker contract; do not repeatedly rerun the unchanged crashing gate |
